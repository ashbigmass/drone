...
class moduleModel extends module {
	...
	function getModuleInfoByMid($mid, $site_srl = 0, $columnList = array()) {
		if(!$mid || ($mid && !preg_match("/^[a-z][a-z0-9_]+$/i", $mid))) return;
		$args = new stdClass();
		$args->mid = $mid;
	...
	function getModuleInfoByModuleSrl($module_srl, $columnList = array()) {
		...
		if($mid_info && count($columnList)) {
			...
			foreach($mid_info as $key => $item) {
				if(in_array($key, $columnList)) $module_info->$key = $item;
			}
		}
	...
	private function applyDefaultSkin(&$moduleInfo) {
		if($moduleInfo->is_skin_fix == 'N') $moduleInfo->skin = '/USE_DEFAULT/';
		if($moduleInfo->is_mskin_fix == 'N') $moduleInfo->mskin = '/USE_DEFAULT/';
	}
	...
	function getModulesInfoByLayout($layout_srl, $columnList = array()) {
		...
		$modules = array();
		for($i=0;$i<$count;$i++) $modules[] = $output->data[$i];
	...
	function getMidList($args = null, $columnList = array()) {
		...
		if($list === false) {
			if($oCacheHandler->isSupport() && count($args) === 1 && isset($args->site_srl)) $columnList = array();
			$output = executeQuery('module.getMidList', $args, $columnList);
			if(!$output->toBool()) return $output;
			$list = $output->data;
			if($oCacheHandler->isSupport() && count($args) === 1 && isset($args->site_srl)) $oCacheHandler->put($cache_key, $list);
		}
		if(!$list) return;
		if(!is_array($list)) $list = array($list);
		foreach($list as $val) $mid_list[$val->mid] = $val;
		return $mid_list;
	}
	...
	function getActionForward($act) {
		...
		if($action_forward === false) {
			...
			foreach($output->data as $item) $action_forward[$item->act] = $item;
			if($oCacheHandler->isSupport()) $oCacheHandler->put($cache_key, $action_forward);
		}
		if($action_forward[$act]) return $action_forward[$act];
	...
	function getTriggers($trigger_name, $called_position) {
		if(is_null($GLOBALS['__triggers__'])) {
			...
			if($triggers === FALSE) {
				...
				if($output->toBool() && $oCacheHandler->isSupport()) $oCacheHandler->put($cache_key, $triggers);
			}
			foreach($triggers as $item) $GLOBALS['__triggers__'][$item->trigger_name][$item->called_position][] = $item;
		}
	...
	function getTrigger($trigger_name, $module, $type, $called_method, $called_position) {
		...
		if($triggers && is_array($triggers)) {
			foreach($triggers as $item) {
				if($item->module == $module && $item->type == $type && $item->called_method == $called_method) return $item;
			}
		}
		return NULL;
	}
	...
	function getModuleExtend($parent_module, $type, $kind='') {
		...
		if(array_key_exists($key, $module_extend_info)) return $module_extend_info[$key];
		return false;
	}
	...
	function loadModuleExtends() {
		...
		if(!isset($GLOBALS['__MODULE_EXTEND__'])) {
			if(file_exists(FileHandler::getRealPath('./files')) && !file_exists($cache_file)) {
				...
				if($output->data) {
					foreach($output->data as $v) $arr[] = sprintf("'%s.%s.%s' => '%s'", $v->parent_module, $v->kind, $v->type, $v->extend_module);
				}
				$str = '<?PHP return array(%s); ?>';
				$str = sprintf($str, join(',',$arr));
				FileHandler::writeFile($cache_file, $str);
			}
			if(file_exists($cache_file)) $GLOBALS['__MODULE_EXTEND__'] = include($cache_file);
			else $GLOBALS['__MODULE_EXTEND__'] = array();
		}
	...
	function getModuleActionXml($module) {
		...
		if(!file_exists($cache_file) || filemtime($cache_file) < filemtime($xml_file) || $re_cache) {
			...
			$grants = $xml_obj->module->grants->grant;
			$permissions = $xml_obj->module->permissions->permission;
			$menus = $xml_obj->module->menus->menu;
			$actions = $xml_obj->module->actions->action;
	...
	function getSkins($path, $dir = 'skins') {
		...
		foreach($list as $skin_name) {
			if(!is_dir($skin_path . $skin_name)) continue;
			unset($skin_info);
		...
		$moduleName = $module;
		if($moduleName === 'page') $moduleName = 'ARTICLE';
	...
	function getModulePartConfigs($module, $site_srl = 0) {
		...
		foreach($output->data as $key => $val) $result[$val->module_srl] = unserialize($val->config);
		return $result;
	}
	...
	function getModuleCategories($moduleCategorySrl = array()) {
		...
		if(!$list) return;
		if(!is_array($list)) $list = array($list);
		foreach($list as $val) $category_list[$val->module_category_srl] = $val;
	...
	function checkNeedUpdate($module_name) {
		...
		if($oDummy && method_exists($oDummy, "checkUpdate")) return $oDummy->checkUpdate();
		return false;
	}
	...
	function getModuleList() {
		...
		for($i=0;$i<$searched_count;$i++) {
			...
			$oDummy = null;
			$oDummy = getModule($module_name, 'class');
			if($oDummy && method_exists($oDummy, "checkUpdate")) $info->need_update = $oDummy->checkUpdate();
			else continue;
			$list[] = $info;
		}
		return $list;
	...
	function syncModuleToSite(&$data) {
		if(!$data) return;
		if(is_array($data)) {
			foreach($data as $key => $val) $module_srls[] = $val->module_srl;
		...
		foreach($output->data as $key => $val) $modules[$val->module_srl] = $val;
		if(is_array($data)) {
			foreach($data as $key => $val) $data[$key]->domain = $modules[$val->module_srl]->domain;
		} else {
	...
	function getModuleExtraVars($list_module_srl) {
		...
		if($oCacheHandler->isSupport()) {
			foreach($list_module_srl as $module_srl) {
				...
				if($vars) $extra_vars[$module_srl] = $vars;
				else $get_module_srls[] = $module_srl;
			}
		} else {
		...
		if(count($get_module_srls) > 0) {
			...
			if(!$output->toBool()) return;
			if(!$output->data) {
				foreach($get_module_srls as $module_srl) $extra_vars[$module_srl] = new stdClass;
			}
			foreach($output->data as $key => $val) {
				if(in_array($val->name, array('mid','module')) || $val->value == 'Array') continue;
				if(!isset($extra_vars[$val->module_srl])) $extra_vars[$val->module_srl] = new stdClass();
	...
	function getModuleSkinVars($module_srl) {
		...
		if($skin_vars === false) {
			...
			if(!$output->toBool()) return;
			$skin_vars = array();
			foreach($output->data as $vars) $skin_vars[$vars->name] = $vars;
	...
	function getModuleDefaultSkin($module_name, $skin_type = 'P', $site_srl = 0, $updateCache = true) {
		...
		if(!$skinName) {
			...
			if(is_dir($moduleSkinPath.'default')) $skinName = 'default';
			else if(is_dir($moduleSkinPath.'xe_default')) $skinName = 'xe_default';
			else {
				$skins = FileHandler::readDir($moduleSkinPath);
				if(count($skins) > 0) $skinName = $skins[0];
				else $skinName = NULL;
			}
	...
	function syncSkinInfoToModuleInfo(&$module_info) {
		...
		$oCacheHandler = CacheHandler::getInstance('object', null, true);
		if(Mobile::isFromMobilePhone()) $skin_vars = $this->getModuleMobileSkinVars($module_info->module_srl);
		else $skin_vars = $this->getModuleSkinVars($module_info->module_srl);
		if(!$skin_vars) return;
	...
	function getGrant($module_info, $member_info, $xml_info = '') {
		...
		if(!$module_srl) {
			...
			if($grant->manager) {
				$grant->access = true;
				if(count($grant_info)) {
					foreach($grant_info as $key => $val) $grant->{$key} = true;
				}
			}
		}
		return $grant;
	...
	function unserializeAttributes($module_filebox_list) {
		if(is_array($module_filebox_list->data)) {
			foreach($module_filebox_list->data as &$item) {
				if(empty($item->comment)) continue;
				$attributes = explode(';', $item->comment);
				foreach($attributes as $attribute) {
					$values = explode(':', $attribute);
					if((count($values) % 2) ==1) {
						for($i=2;$i<count($values);$i++) $values[1].=":".$values[$i];
					}
	...
	function getValidatorFilePath($module, $ruleset, $mid=null) {
		if(strpos($ruleset, '@') !== false) {
			...
			$xml_file = sprintf('./files/ruleset/%s.xml', $rulsetFile);
			if(is_readable($xml_file)) return FileHandler::getRealPath($xml_file);
			else $ruleset = str_replace('#', '', $ruleset);
		}
		$class_path = ModuleHandler::getModulePath($module);
		if(!$class_path) return;
