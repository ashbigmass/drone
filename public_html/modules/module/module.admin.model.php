...
class moduleAdminModel extends module
{
	...
	function getModuleAdminModuleList() {
		...
		for($i=0;$i<count($modules);$i++) $module_list[$modules[$i]] = $list[$modules[$i]];
	...
	function getSelectedManageHTML($grantList, $tabChoice = array(), $modulePath = NULL) {
		...
		if(count($tabChoice) == 0) $tabChoice = array('tab1'=>1, 'tab2'=>1, 'tab3'=>1);
	...
	public function getModuleAdminGrant() {
		...
		if(!$targetModule || !$moduleSrl) return new Object(-1, 'msg_invalid_request');
		if($targetModule == '_SHORTCUT') return new Object(0);
		$oModuleModel = getModel('module');
		...
		if(is_object($grantList)) {
			foreach($grantList AS $key=>$value) {
				if(isset($defaultGrant->{$key})) $grantList->{$key}->grant = $defaultGrant->{$key};
				if(isset($selectedGroup->{$key})) $grantList->{$key}->group_srls = $selectedGroup->{$key};
			}
		}
	...
	function _getModuleSkinHTML($module_srl, $mode) {
		...
		if($mode === 'P') {
			if($module_info->is_skin_fix == 'N') $skin = $oModuleModel->getModuleDefaultSkin($module_info->module, 'P', $module_info->site_srl);
			else $skin = $module_info->skin;
		} else {
			if($module_info->is_mskin_fix == 'N') $skin = $oModuleModel->getModuleDefaultSkin($module_info->module, 'M', $module_info->site_srl);
			else $skin = $module_info->mskin;
		}
	...
	function getLangCode($site_srl, $name, $isFullLanguage = FALSE) {
		if($isFullLanguage) $lang_supported = Context::loadLangSupported();
		else $lang_supported = Context::get('lang_supported');
		if(substr($name,0,12)=='$user_lang->') {
			...
			if($output->data) {
				foreach($output->data as $key => $val) $selected_lang[$val->lang_code] = $val->value;
			}
		} else {
			...
			if($tmp) {
				...
				if(is_array($lang_supported)) {
					foreach($lang_supported as $key => $val) $selected_lang[$key] = $tmp[$key]?$tmp[$key]:$rand_name;
				}
			}
		}
		$output = array();
		if(is_array($lang_supported)) {
			foreach($lang_supported as $key => $val) $output[$key] = $selected_lang[$key]?$selected_lang[$key]:$name;
		}
		return $output;
	}
	...
	function getModuleAdminLangListByValue() {
		...
		if($output->toBool() && is_array($output->data)) {
			...
			foreach($output->data as $data) {
				...
				if($outputByName->toBool()) $langList = array_merge($langList, $outputByName->data);
			}
		}
		$this->add('lang_list', $langList);
	}
	...
	function getModuleAdminLangListHtml() {
		...
		$args->page_count = 5;
		if(!$args->langCode) $args->langCode = Context::get('lang_type');
	...
	function getModuleAdminModuleInfo() {
		if(Context::get('search_module_srl')) $module_srl = Context::get('search_module_srl');
		else $module_srl = Context::get('module_srl');
