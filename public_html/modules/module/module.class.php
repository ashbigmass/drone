...
class module extends ModuleObject {
	...
	function moduleUpdate() {
			...
			foreach($modules as $key => $module_info) {
				...
				switch($module) {
					case 'point' :
						...
						break;
					case 'trackback' : case 'rss' : case 'file' : case 'comment' : case 'editor' :
		...
		if(!$oDB->isColumnExists('sites', 'default_language')) $oDB->addColumn('sites','default_language','varchar',255,0,false);
		...
		if(!$oDB->isColumnExists("modules", "use_mobile")) $oDB->addColumn('modules','use_mobile','char',1,'N');
		if(!$oDB->isColumnExists("modules", "mlayout_srl")) $oDB->addColumn('modules','mlayout_srl','number',11, 0);
		if(!$oDB->isColumnExists("modules", "mcontent")) $oDB->addColumn('modules','mcontent','bigtext');
		if(!$oDB->isColumnExists("modules", "mskin")) $oDB->addColumn('modules','mskin','varchar',250);
		...
		if(!$oDB->isColumnExists("module_config", "site_srl")) $oDB->addColumn('module_config', 'site_srl', 'number', 11, 0, true);
		FileHandler::makeDir('./files/ruleset');
