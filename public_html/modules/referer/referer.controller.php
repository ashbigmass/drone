...
class refererController extends referer
{
	...
	function updateRefererStatistics($remote, $host, $uagent, $member_srl, $ref_mid, $ref_document_srl, $location) {
		...
		if($oRefererModel->isInsertedRemote($remote)) $output = executeQuery('referer.updateRemoteStatistics', $args);
		else $output = executeQuery('referer.insertRemoteStatistics', $args);
		if($host != "") {
			$args->host = $host;
			if($oRefererModel->isInsertedHost($host)) $output = executeQuery('referer.updateRefererStatistics', $args);
			else $output = executeQuery('referer.insertRefererStatistics', $args);
		}
		if($uagent != "") {
			$args->uagent = $uagent;
			if($oRefererModel->isInsertedUAgent($uagent)) $output = executeQuery('referer.updateUAgentStatistics', $args);
			else $output = executeQuery('referer.insertUAgentStatistics', $args);
		}
		$args->member_srl = $member_srl;
		if($oRefererModel->isInsertedUser($member_srl)) $output = executeQuery('referer.updateUserStatistics', $args);
		else $output = executeQuery('referer.insertUserStatistics', $args);
		$args->ref_mid = $ref_mid;
		$args->ref_document_srl = $ref_document_srl;
		if($oRefererModel->isInsertedPage($ref_mid, $ref_document_srl)) $output = executeQuery('referer.updatePageStatistics', $args);
		else $output = executeQuery('referer.insertPageStatistics', $args);
		if ($location != NULL && $location->country_code != "") {
			if($oRefererModel->isInsertedCountry($args)) $output = executeQuery('referer.updateCountryStatistics', $args);
			else $output = executeQuery('referer.insertCountryStatistics', $args);
		}
		return $output;
	}
