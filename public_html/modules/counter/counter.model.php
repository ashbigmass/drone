...
class counterModel extends counter
{
	...
	function isLogged($site_srl = 0) {
		...
		if($iplogged && $oCacheHandler->isSupport()) $oCacheHandler->put($cache_key, $iplogged);
		return $iplogged;
	}
	...
	function getStatus($selected_date, $site_srl = 0) {
		...
		$output = array();
		foreach($status as $val) $output[substr($val->regdate, 0, 8)] = $val;
		return $output;
	}
	...
	function getHourlyStatus($type = 'hour', $selected_date, $site_srl = 0, $isPageView = false) {
		...
		switch($type) {
			case 'year' :
				...
				if(!($start_year = substr($output->data->regdate, 0, 4))) $start_year = date("Y");
			...
			case 'week' :
				...
				$w = date("D");
				while(date("D", $time) != "Sun") $time += 60 * 60 * 24;
				$time -= 60 * 60 * 24;
	...
	public function getWeeklyUniqueVisitor() {
		...
		$tmp = array();
		foreach($output1->list as $key => $value) $tmp["'" . $key . "'"] = $value;
		$output1->list = $tmp;
		$date2 = date('Ymd');
		$output2 = $this->getHourlyStatus('week', $date2);
		$tmp = array();
		foreach($output2->list as $key => $value) $tmp["'" . $key . "'"] = $value;
		$output2->list = $tmp;
