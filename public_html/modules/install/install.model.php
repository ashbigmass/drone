...
class installModel extends install
{
	...
	function getInstallFTPList() {
		if(!($ftp_info = Context::getRequestVars()) || !$ftp_info->ftp_user || !$ftp_info->ftp_password)  return new Object(-1, 'msg_ftp_invalid_auth_info');
		$this->pwd = $ftp_info->ftp_root_path;
		if(!$ftp_info->ftp_host) $ftp_info->ftp_host = "127.0.0.1";
		if($ftp_info->sftp == 'Y') return $this->getSFTPList();
		$_list = NULL;
		if(function_exists('ftp_connect')) {
			...
			if($ftp_info->ftp_pasv != "N")  ftp_pasv($connection, true);
