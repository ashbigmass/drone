...
class installAdminController extends install {
	...
	function procInstallAdminUpdate() {
		...
		$output = ($oModule) ? $oModule->moduleUpdate() : new Object(-1, 'invalid_request');
		return $output;
	}
	...
	function procInstallAdminSaveTimeZone() {
		...
		if($admin_ip_list) {
			...
			if(preg_match('/(<\?|<\?php|\?>)/xsm', $admin_ip_list)) $admin_ip_list = '';
			...
			if(!IpFilter::validate($admin_ip_list)) return new Object(-1, 'msg_invalid_ip');
		}
	...
	function procInstallAdminUpdateIndexModule() {
		if(!Context::get('index_module_srl') || !Context::get('menu_item_srl')) return new Object(-1, 'msg_invalid_request');
		$site_args = new stdClass();
		$site_args->site_srl = 0;
		...
		if(!$homeMenuSrl || $homeMenuSrl != $output->menu_srl) $oMenuAdminController->makeHomemenuCacheFile($output->menu_srl);
	...
	function procInstallAdminSaveFTPInfo() {
		...
		$ftp_info->ftp_root_path = ((substr($ftp_root_path, strlen($ftp_root_path)-1) == "/")) ? $ftp_root_path : $ftp_root_path.'/';
		if(ini_get('safe_mode')) $ftp_info->ftp_password = Context::get('ftp_password');
		$buff = '<?php if(!defined("__XE__")) exit();'."\n\$ftp_info = new stdClass;\n";
		foreach($ftp_info as $key => $val) {
			if(!$val) continue;
			if(preg_match('/(<\?|<\?php|\?>|fputs|fopen|fwrite|fgets|fread|file_get_contents|file_put_contents|exec|proc_open|popen|passthru|show_source|phpinfo|system|\/\*|\*\/|chr\()/xsm', preg_replace('/\s/', '', $val))) continue;
	...
	function saveLangSelected($selected_lang) {
		...
		$buff = null;
		for($i=0;$i<count($langs);$i++) $buff .= sprintf("%s,%s\n", $langs[$i], $lang_supported[$langs[$i]]);
	...
	private function updateIcon($iconname, $deleteIcon = false) {
		...
		if(file_exists($tmpicon_filepath)) FileHandler::moveFile($tmpicon_filepath, $icon_filepath);
