...
class memberView extends member
{
	...
	function init() {
		...
		if(!$skin) {
			...
			if (count($config_parse) > 1) $template_path = sprintf('./themes/%s/modules/member/', $config_parse[0]);
	...
	function dispMemberInfo() {
		...
		if(!$member_srl && Context::get('is_logged')) $member_srl = $logged_info->member_srl;
		elseif(!$member_srl) return $this->dispMemberSignUpForm();
	...
	function _getDisplayedMemberInfo($memberInfo, $extendFormInfo, $memberConfig) {
		...
		foreach($memberConfig->signupForm as $no=>$formInfo) {
			...
			if($formInfo->name == 'password' || $formInfo->name == 'find_account_question') continue;
			if($logged_info->is_admin != 'Y' && $memberInfo->member_srl != $logged_info->member_srl && $formInfo->isPublic != 'Y') continue;
			$item = $formInfo;
	...
	function dispMemberLoginForm() {
		...
		if($XE_VALIDATOR_ERROR == -11) Context::set('XE_VALIDATOR_MESSAGE', $XE_VALIDATOR_MESSAGE . $config->limit_day_description);
		if($XE_VALIDATOR_ERROR < -10 && $XE_VALIDATOR_ERROR > -21) Context::set('referer_url', getUrl(''));
		else Context::set('referer_url', htmlspecialchars($_SERVER['HTTP_REFERER'], ENT_COMPAT | ENT_HTML401, 'UTF-8', false));
	...
	function dispMemberLogout() {
		...
		if(!$output->redirect_url) $this->setRedirectUrl(getNotEncodedUrl('act', ''));
		else $this->setRedirectUrl($output->redirect_url);
		return;
	}
	...
	function addExtraFormValidatorMessage() {
		...
		foreach($extraList as $val) {
			...
			if($val->column_type == 'kr_zip' || $val->column_type == 'tel') $js_code[] = sprintf('validator.cast("ADD_MESSAGE", ["%s[]","%s"]);', $val->column_name, $title);
			else $js_code[] = sprintf('validator.cast("ADD_MESSAGE", ["%s","%s"]);', $val->column_name, $title);
