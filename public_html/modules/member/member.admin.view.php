...
class memberAdminView extends member
{
	...
	function dispMemberAdminList() {
		...
		if($output->data) {
			foreach($output->data as $key => $member) $output->data[$key]->group_list = $oMemberModel->getMemberGroups($member->member_srl,0);
		}
	...
	function dispMemberAdminInsert() {
		...
		if(isset($memberInfo)) $memberInfo->signature = $oMemberModel->getSignature($this->memberInfo->member_srl);
	...
	function _getMemberInputTag($memberInfo, $isAdmin = false) {
		...
		if ($memberInfo) $memberInfo = get_object_vars($memberInfo);
		$member_config = $this->memberConfig;
		if(!$this->memberConfig) $member_config = $this->memberConfig = $oMemberModel->getMemberConfig();
		$formTags = array();
		global $lang;
		foreach($member_config->signupForm as $no=>$formInfo) {
			...
			if($isAdmin) if($formInfo->mustRequired) $formTag->title = '<em style="color:red">*</em> '.$formTag->title;
			else if ($formInfo->required && $formInfo->name != 'password') $formTag->title = '<em style="color:red">*</em> '.$formTag->title;
			...
			if($formInfo->isDefaultForm) {
				if($formInfo->imageType) {
					...
					if($extendForm->column_type == 'text') {
						...
						if($krzipModel && method_exists($krzipModel , 'getKrzipCodeSearchHtml' )) $template = $krzipModel->getKrzipCodeSearchHtml($extendForm->column_name, $extendForm->value);
					...
					if($extendForm->description) $inputTag .= '<p class="help-block">'.$extendForm->description.'</p>';
				}
