...
class memberAdminController extends member
{
	...
	function procMemberAdminInsert() {
		...
		if($logged_info->is_admin != 'Y' || !checkCSRF()) return new Object(-1, 'msg_invalid_request');
		...
		foreach($getVars as $val) $args->{$val} = Context::get($val);
		$args->member_srl = Context::get('member_srl');
		if(Context::get('reset_password')) $args->password = Context::get('reset_password');
		...
		if(is_uploaded_file($profile_image['tmp_name'])) $oMemberController->insertProfileImage($args->member_srl, $profile_image['tmp_name']);
		$image_mark = $_FILES['image_mark'];
		if(is_uploaded_file($image_mark['tmp_name'])) $oMemberController->insertImageMark($args->member_srl, $image_mark['tmp_name']);
		$image_name = $_FILES['image_name'];
		if (is_uploaded_file($image_name['tmp_name'])) $oMemberController->insertImageName($args->member_srl, $image_name['tmp_name']);
	...
	public function procMemberAdminInsertDefaultConfig() {
		...
		if(!array_key_exists($args->password_hashing_algorithm, $oPassword->getSupportedAlgorithms())) $args->password_hashing_algorithm = 'md5';
		$args->password_hashing_work_factor = intval($args->password_hashing_work_factor, 10);
		if($args->password_hashing_work_factor < 4) $args->password_hashing_work_factor = 4;
		if($args->password_hashing_work_factor > 16) $args->password_hashing_work_factor = 16;
		if($args->password_hashing_auto_upgrade != 'Y') $args->password_hashing_auto_upgrade = 'N';
		if((!$args->webmaster_name || !$args->webmaster_email) && $args->enable_confirm == 'Y') return new Object(-1, 'msg_mail_authorization');
	...
	public function procMemberAdminInsertSignupConfig() {
		...
		if($args->redirect_url) {
			...
			if(!$redirectModuleInfo) return new Object('-1', 'msg_exist_selected_module');
	...
	public function procMemberAdminInsertDesignConfig() {
		$oModuleController = getController('module');
		$args = Context::gets('layout_srl', 'skin', 'colorset', 'mlayout_srl', 'mskin');
	...
	function createSignupForm($identifier) {
		...
		foreach($items as $key) {
			...
			if($key == 'find_account_question' || $key == 'password') $signupItem->isPublic = 'N';
			...
			if($signupItem->isIdentifier) array_unshift($list_order, $signupItem);
			else $list_order[] = $signupItem;
		}
	...
	function _createSignupRuleset($signupForm, $agreement = null){
		...
		$buff = '<?xml version="1.0" encoding="utf-8"?>' . PHP_EOL.
			...
			'</ruleset>';
		$fields = array();
		if ($agreement) $fields[] = '<field name="accept_agreement"><if test="$act == \'procMemberInsert\'" attr="required" value="true" /></field>';
	...
	function _createFindAccountByQuestion($identifier) {
		...
		$buff = '<?xml version="1.0" encoding="utf-8"?>'.
			...
			'</ruleset>';
		$fields = array();
		if($identifier == 'user_id') $fields[] = '<field name="user_id" required="true" rule="userid" />';
	...
	function procMemberAdminInsertJoinForm() {
		...
		if(in_array($args->column_type, array('checkbox','select','radio')) && count($args->default_value)) $args->default_value = serialize($args->default_value);
		else $args->default_value = '';
		$oMemberModel = getModel('member');
		$config = $oMemberModel->getMemberConfig();
		...
		if($isInsert) {
			...
			foreach($config->signupForm as $key=>$val) {
				if($val->member_join_form_srl == $signupItem->member_join_form_srl) $config->signupForm[$key] = $signupItem;
			}
		}
		$oModuleController = getController('module');
	...
	function procMemberAdminSelectedMemberManage() {
		...
		if($message) {
			...
			foreach($members as $member_srl) $oCommunicationController->sendMessage($sender_member_srl, $member_srl, $title, $message, false);
		}
		$returnUrl = Context::get('success_return_url') ? Context::get('success_return_url') : getNotEncodedUrl('', 'module', 'admin', 'act', 'dispMemberAdminList');
		$this->setRedirectUrl($returnUrl);
	}
	...
	function insertGroup($args) {
		...
		if(!isset($args->list_order) || $args->list_order=='') $args->list_order = $args->group_srl;
	...
	public function procMemberAdminGroupConfig() {
		...
		foreach($group_srls as $order=>$group_srl) {
			...
			if($vars->defaultGroup == $group_srl) $defaultGroupSrl = $update_args->group_srl;
		}
	...
	function _deleteMemberGroupCache($site_srl = 0) {
		...
		if($oCacheHandler->isSupport()) $oCacheHandler->invalidateGroupKey('member');
	}
	...
	function moveJoinFormDown($member_join_form_srl) {
		...
		$join_form_srl_list = array_keys($join_form_list);
		if(count($join_form_srl_list)<2) return new Object();
		for($i=0;$i<count($join_form_srl_list);$i++) if($join_form_srl_list[$i]==$member_join_form_srl) break;
