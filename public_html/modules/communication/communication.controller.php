...
class communicationController extends communication
{
	...
	function procCommunicationSendMessage() {
		...
		if(!$oCommunicationModel->checkGrant($config->grant_write)) return new Object(-1, 'msg_not_permitted');
	...
	function procCommunicationMoveFriend() {
		...
		if(!is_array($friend_srl_list)) $friend_srl_list = explode('|@|', $friend_srl_list);
