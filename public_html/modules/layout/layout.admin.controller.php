...
class layoutAdminController extends layout
{
	...
	function initLayout($layout_srl, $layout_name) {
		...
		if($oLayoutModel->useDefaultLayout($layout_name)) $this->importLayout($layout_srl, $this->module_path.'tpl/faceOff_sample.tar');
		else FileHandler::removeDir($oLayoutModel->getUserLayoutPath($layout_srl));
	}
	...
	function procLayoutAdminUpdate() {
		...
		if($layout_info->menu) $menus = get_object_vars($layout_info->menu);
		if(count($menus)) {
			foreach($menus as $menu_id => $val) {
				...
				if($menu_srl == -1) {
					...
					if(!$output->toBool()) return new Object(-1, 'fail_to_update');
				...
				if($apply_layout=='Y' || $apply_mobile_view=='Y') {
					...
					if($output->data) {
						$modules = array();
						for($i=0;$i<count($output->data);$i++) $modules[] = $output->data[$i]->module_srl;
						if(count($modules)) {
							$update_args = new stdClass();
							$update_args->module_srls = implode(',',$modules);
							if($apply_layout == "Y") $update_args->layout_srl = $args->layout_srl;
							if($layout_info->layout_type == "M") {
								if(Context::get('apply_mobile_view') == "Y") $update_args->use_mobile = "Y";
		...
		if($force) {
			...
			if($layoutInfo) {
				...
				if(count($layoutList) <= 1) {
					...
					if($packageSrl) $output = $oAutoinstallAdminController->uninstallPackageByPackageSrl($packageSrl);
					else $output = $oAutoinstallAdminController->uninstallPackageByPath($path);
	...
	function procLayoutAdminCodeUpdate() {
		...
		if(!$layout_srl || !$code || !$is_post) return new Object(-1, 'msg_invalid_request');
		$oLayoutModel = getModel('layout');
		$layout_file = $oLayoutModel->getUserLayoutHtml($layout_srl);
	...
	function procLayoutAdminUserValueInsert() {
		...
		if($temp =='Y') $oLayoutModel->setUseUserLayoutTemp();
		else $this->deleteUserLayoutTempFile($layout_srl);
	...
	function deleteUserLayoutTempFile($layout_srl) {
		...
		foreach($file_list as $key => $file) FileHandler::removeFile($file);
	}
	...
	function procLayoutAdminUserLayoutExport() {
		...
		foreach($file_list as $file) {
			...
			if(basename($file) == 'faceoff.css' || basename($file) == 'layout.css') $content = preg_replace($pattern, '../', $content);
			else $content = preg_replace($pattern, './', $content);
			foreach($ini_config as $key => $value) $content = str_replace('{$layout_info->faceoff_ini_config[\'' . $key . '\']}', $value, $content);
			FileHandler::writeFile($file, $content);
		}
	...
	function procLayoutAdminCopyLayout() {
		...
		if($sourceArgs->layout == 'faceoff') return $this->stop('not supported');
		if(!$sourceArgs->layout_srl) return $this->stop('msg_empty_origin_layout');
		$oLayoutModel = getModel('layout');
		$layout = $oLayoutModel->getLayout($sourceArgs->layout_srl);
		if(!$sourceArgs->title) $sourceArgs->title = array($layout->title.'_'.$this->_makeRandomMid());
		if(!is_array($sourceArgs->title) || count($sourceArgs->title) == 0) return $this->stop('msg_empty_target_layout');
	...
	function _copyLayoutFile($sourceLayoutSrl, $targetLayoutSrl) {
		...
		foreach($sourceFileList as $key => $file) {
			if(is_readable($sourceLayoutPath.$file)) {
				...
				if($file == 'layout.html' || $file == 'layout.css') $this->_changeFilepathInSource($targetLayoutPath.$file, $sourceImagePath, $targetImagePath);
			}
		}
	}
	...
	function importLayout($layout_srl, $source_file) {
		...
		$user_layout_path = FileHandler::getRealPath($oLayoutModel->getUserLayoutPath($layout_srl));
		$file_list = $oLayoutModel->getUserLayoutFileList($layout_srl);
		foreach($file_list as $key => $file) FileHandler::removeFile($user_layout_path . $file);
		require_once(_XE_PATH_.'libs/tar.class.php');
		...
		foreach($tar->files as $key => $info) FileHandler::writeFile($user_layout_path . $info['name'],str_replace('__LAYOUT_PATH__',$replace_path,$info['file']));
	...
	function procLayoutAdminConfigImageDelete() {
		...
		if($layoutInfo->extra_var_count) {
			foreach($layoutInfo->extra_var as $varId => $val) $newLayoutInfo->{$varId} = $val->value;
		}
