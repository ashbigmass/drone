...
class layoutAdminModel extends layout
{
	...
	public function getLayoutAdminSetInfoView() {
		...
		foreach($pluginList as $plugin) {
			...
			if(!$info) continue;
			foreach($info->jsList as $js) $script .= sprintf('<script src="%s"></script>', $js);
			foreach($info->cssList as $css) $csss .= sprintf('<link rel="stylesheet" href="%s" />', $css);
		}
		$this->add('html', $csss . $script . $html);
		if($isReturn) return $this->get('html');
	}
	...
	public function getSiteDefaultLayout($viewType = 'P', $siteSrl = 0) {
		...
		if(!$designInfo || !$designInfo->{$target}) return 0;
