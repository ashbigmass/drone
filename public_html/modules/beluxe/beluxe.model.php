...
class beluxeModel extends beluxe
{
	...
	function _getModuleInfo($a_modsrl = 0) {
		if (!$this->module_info || !$this->module_info->module_srl) {
			...
			if ($a_modsrl)
				$oMi = $cmModule->getModuleInfoByModuleSrl($a_modsrl);
			else {
				$mid = Context::get('mid');
	...
	function _setCommentItem($a_cmtlst) {
		...
		foreach ($a_cmtlst as $key => $attr) {
			...
			if (!isset($GLOBALS['XE_COMMENT_LIST'][$cmt_srl])) {
				...
				if ($attr->parent_srl > 0 && $acc[$attr->parent_srl] === true && $attr->is_secret == 'Y' && !$oComNew->isAccessible()) $oComNew->setAccessible();
	...
	function _getDocumentColumns($a_docsrl, $a_collst = array(), $is_obj = false) {
		...
		if (!isset($oDocIfo) || count($a_collst) != count($re)) {
			...
			if ($oDocIfo->isExists()) foreach ($a_collst as $tv) $re[$tv] = $oDocIfo->get($tv);
		}
	...
	function _getCommentColumns($a_cmtsrl, $a_collst = array(), $is_obj = false) {
		...
		if (!isset($oCmtIfo) || count($a_collst) != count($re)) {
			...
			if ($oCmtIfo->isExists()) foreach ($a_collst as $tv) $re[$tv] = $oCmtIfo->get($tv);
			}
		}
	...
	function getNavigationList($obj, $a_ectnotice = FALSE, $a_loadextra = TRUE, $a_collst = array()) {
		...
		foreach ($data as $key => $val) $data[$key]->selected = $val->document_srl == $cur_d_srl;
		$out->data = $data;
	...
	function getBestCommentList($a_docsrl) {
		...
		if((int)$oMi->best_c_date !== -1) $start_date = date('YmdHis', time() - (60 * 60 * 24 * (int)$oMi->best_c_date));
	...
	function getDocumentSrlsByAdopt($a_obj, $a_list_order = false) {
		...
		if($a_obj->search_keyword == 'true' || $a_obj->search_keyword == 'Y') $args->like_vars = '\"adopt_srl\"\;';
		else $args->notlike_vars = '\"adopt_srl\"\;';
		...
		if ($out->toBool()) {
			$arr = array();
			foreach ($out->data as $value) $arr[] = $value->document_srl;
			$out->data = $arr;
		}
		return $out;
	}
