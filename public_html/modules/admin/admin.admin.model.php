...
class adminAdminModel extends admin {
	...
	function getSFTPPath() {
		...
		if(!$ftp_info->ftp_port || !is_numeric($ftp_info->ftp_port)) $ftp_info->ftp_port = '22';
		...
		foreach($path_candidate as $path) {
			if(!@ssh2_scp_send($connection, FileHandler::getRealPath('./files/cache/ftp_check'), $path . 'ftp_check.html')) continue;
		...
		if($found_path) $this->add('found_path', $found_path);
	}
	...
	function getFTPPath() {
		...
		if(!$connection) return new Object(-1, sprintf(Context::getLang('msg_ftp_not_connected'), $ftp_host));
		...
		foreach($path_candidate as $path) {
			if(!ftp_put($connection, $path . 'ftp_check.html', FileHandler::getRealPath('./files/cache/ftp_check'), FTP_BINARY)) continue;
			$result = FileHandler::getRemoteResource(getNotencodedFullUrl() . 'ftp_check.html');
	...
	function getAdminFTPPath() {
		...
		if(!$ftp_info->ftp_host) $ftp_info->ftp_host = '127.0.0.1';
		if(!$ftp_info->ftp_port || !is_numeric($ftp_info->ftp_port)) $ftp_info->ftp_port = '21';
		if($ftp_info->sftp == 'Y') {
			if(!function_exists('ssh2_sftp')) return new Object(-1, 'disable_sftp_support');
			return $this->getSFTPPath();
		}
		if($ftp_info->ftp_pasv == 'N') {
			if(function_exists('ftp_connect')) return $this->getFTPPath();
			$ftp_info->ftp_pasv = "Y";
		}
		$oFTP = new ftp();
		...
		if(!$oFTP->ftp_login($ftp_info->ftp_user, $ftp_info->ftp_password)) return new Object(1, 'msg_ftp_invalid_auth_info');
		$pin = $_SERVER['REQUEST_TIME'];
		FileHandler::writeFile('./files/cache/ftp_check', $pin);
		$xe_path = _XE_PATH_;
		$path_info = array_reverse(explode('/', _XE_PATH_));
		...
		foreach($path_candidate as $path) {
			if(!$oFTP->ftp_put($path . 'ftp_check.html', FileHandler::getRealPath('./files/cache/ftp_check'))) continue;
			$result = FileHandler::getRemoteResource(getNotencodedFullUrl() . 'ftp_check.html');
	...
	function getEnv($type = 'WORKING') {
		...
		if($layout_list) foreach($layout_list as $layout) {
			if(in_array($layout->layout, $skip['layout'])) continue;
		...
		if($widget_list) foreach($widget_list as $widget) {
			if(in_array($widget->widget, $skip['widget'])) continue;
		...
		if($widgetstyle_list) foreach($widgetstyle_list as $widgetstyle) {
			if(in_array($widgetstyle->widgetStyle, $skip['widgetstyle'])) continue;
	...
	function getThemeList() {
		...
		$theme_info = array();
		if(count($list) > 0) {
			foreach($list as $val) $theme_info[$val] = $this->getThemeInfo($val);
		}
		return $theme_info;
	}
	...
	function getThemeInfo($theme_name, $layout_list = NULL) {
		...
		switch($layout_parse[1]) {
			case 'themes' : $layout_info->name = $theme_name . '|@|' . $layout_parse[count($layout_parse) - 1]; break;
			case 'layouts' : $layout_info->name = $layout_parse[count($layout_parse) - 1]; break;
		}
	...
	function getAllSitesThatHaveModules($domain = NULL) {
		...
		foreach($siteList as $key => $value) {
			...
			foreach($list as $k => $v) {
				if(!is_dir(_XE_PATH_ . 'modules/' . $v->module)) unset($list[$k]);
			}
			if(!count($list)) unset($siteList[$key]);
		}
		return $siteList;
	}
