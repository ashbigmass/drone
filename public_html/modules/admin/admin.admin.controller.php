...
class adminAdminController extends admin {
	...
	function procAdminRecompileCacheFile() {
		...
		if($oObjectCacheHandler->isSupport()) $truncated[] = $oObjectCacheHandler->truncate();
		if($oTemplateCacheHandler->isSupport()) $truncated[] = $oTemplateCacheHandler->truncate();
		if(count($truncated) && in_array(FALSE, $truncated)) return new Object(-1, 'msg_self_restart_cache_engine');
	...
	public function procAdminInsertDefaultDesignInfo() {
		...
		if(!$vars->site_srl) $vars->site_srl = 0;
	...
	public function updateDefaultDesignInfo($vars) {
		...
		if(!is_dir($siteDesignPath)) FileHandler::makeDir($siteDesignPath);
		...
		if(is_readable($siteDesignFile)) include($siteDesignFile);
		else $designInfo = new stdClass();
		...
		foreach($vars->module_skin as $moduleName => $skinName) {
			if($moduleName == 'ARTICLE') $moduleName = 'page';
	...
	function makeDefaultDesignFile($designInfo, $site_srl = 0) {
		...
		if($designInfo->layout_srl) $buff[] = sprintf('$designInfo->layout_srl = %s; ', $designInfo->layout_srl);
		if($designInfo->mlayout_srl) $buff[] = sprintf('$designInfo->mlayout_srl = %s;', $designInfo->mlayout_srl);
	...
	function cleanFavorite() {
		...
		if(!$favoriteList) return new Object();
		$deleteTargets = array();
		foreach($favoriteList as $favorite) {
			if($favorite->type == 'module') {
				$modulePath = _XE_PATH_ . 'modules/' . $favorite->module;
				if(!is_dir($modulePath)) $deleteTargets[] = $favorite->admin_favorite_srl;
			}
		}
		if(!count($deleteTargets)) return new Object();
		$args = new stdClass();
	...
	function procAdminUpdateConfig() {
		...
		if(!is_object($oAdminConfig)) $oAdminConfig = new stdClass();
		if($file['tmp_name']) {
	...
	function procAdminRemoveIcons() {
		...
		if($file_exist) @FileHandler::removeFile(_XE_PATH_ . 'files/attach/xeicon/' . $virtual_site . $iconname);
		else return new Object(-1, 'fail_to_delete');
	...
	function procAdminUpdateSitelock() {
		...
		if(preg_match('/(<\?|<\?php|\?>)/xsm', $whitelist)) $whitelist = '';
		$whitelist .= ',127.0.0.1,' . $_SERVER['REMOTE_ADDR'];
		$whitelist = explode(',',trim($whitelist, ','));
		$whitelist = array_unique($whitelist);
		if(!IpFilter::validate($whitelist)) return new Object(-1, 'msg_invalid_ip');
		$db_info->sitelock_whitelist = $whitelist;
		$oInstallController = getController('install');
		if(!$oInstallController->makeConfigFile()) return new Object(-1, 'msg_invalid_request');
