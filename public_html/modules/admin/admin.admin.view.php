...
class adminAdminView extends admin 
	...
	function __construct() {
		...
		if(strpos($db_info->default_url, 'xn--') !== FALSE) $xe_default_url = Context::decodeIdna($db_info->default_url);
		else $xe_default_url = $db_info->default_url;
	...
	function makeGnbUrl($module = 'admin') {
		global $lang;
		$oDB = DB::getInstance();
		if(!$oDB->isColumnExists('menu_item', 'is_shortcut')) return;
		...
		foreach((array) $menu->list as $parentKey => $parentMenu) {
			...
			foreach($parentMenu['list'] as $childKey => $childMenu) {
				if($subMenuTitle == $childMenu['text'] && $parentSrl == 0) $parentSrl = $childMenu['parent_srl'];
			}
		}
		...
		if(file_exists($cache_file)) {
			...
			if($item) {
				...
				if(isset($news) && is_array($news)) Context::set('latestVersion', array_shift($news));
			}
	...
	function dispAdminIndex() {
		...
		if(is_array($output)) {
			foreach($output AS $key => $value) $value->content = strip_tags($value->content);
		}
		...
		if(is_array($module_list)) {
			...
			foreach($module_list AS $key => $value) {
				if($value->need_install) $addTables = TRUE;
				if($value->need_update) $needUpdate = TRUE;
			}
		}
		...
		if(is_array($needUpdateList)) {
			foreach($needUpdateList AS $key => $value) {
				...
				switch($value->type) {
					case 'addon': $helpUrl .= 'UMAN_terminology_addon'; break;
					case 'layout': case 'm.layout': $helpUrl .= 'UMAN_terminology_layout'; break;
					case 'module': $helpUrl .= 'UMAN_terminology_module'; break;
					case 'widget': $helpUrl .= 'UMAN_terminology_widget'; break;
					case 'widgetstyle': $helpUrl .= 'UMAN_terminology_widgetstyle'; break;
					default: $helpUrl = ''; break;
				}
	...
	function dispAdminConfigGeneral() {
		...
		if(strpos($db_info->default_url, 'xn--') !== FALSE) $db_info->default_url = Context::decodeIdna($db_info->default_url);
