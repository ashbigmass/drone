...
class admin extends ModuleObject {
	...
	function moduleUpdate() {
		...
		if(!$oDB->isColumnExists("admin_favorite", "type")) {
			...
			if(is_array($favoriteList)) {
				...
				foreach($favoriteList AS $key => $value) $oAdminAdminController->_insertFavorite($value->site_srl, $value->module);
			}
		}
		return new Object();
	}
	...
	public function createXeAdminMenu() {
		...
		foreach($gnbList AS $key => $value) {
			...
			if($value == 'dashboard') $args->url = 'index.php?module=admin';
			else $args->url = '#';
	...
	private function _oldAdminmenuDelete() {
		...
		if($menuSrl) {
			...
			if(is_array($output->data)) {
				...
				foreach($output->data AS $key => $menuItem) {
					if($menuItem->parent_srl != 0) {
						...
						if($result == 'user_added_menu') {
							if($parentMenu[$menuItem->parent_srl] == 'extensions') $newParentItem = $newAdminParentMenuList['advanced'];
							else $newParentItem = $newAdminParentMenuList[$parentMenu[$menuItem->parent_srl]];
				...
				if($isUserAddedMenuMoved) $oMenuAdminController->makeXmlFile($newAdminmenuSrl);
			}
		}
