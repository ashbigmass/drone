...
class spamfilterModel extends spamfilter
{
	...
	function checkLimited($isMessage = FALSE) {
		...
		if($count) {
			if($isMessage) $message = sprintf(Context::getLang('msg_alert_limited_message_by_config'), $interval);
			else $message = sprintf(Context::getLang('msg_alert_limited_by_config'), $interval);
			$oSpamFilterController = getController('spamfilter');
