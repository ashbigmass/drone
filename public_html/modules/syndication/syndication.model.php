...
class syndicationModel extends syndication
{
	...
	function init() {
		...
		if($output->data) {
			foreach($output->data as $key => $val) $this->granted_modules[] = $val->module_srl;
		}
	...
	function getExceptModuleSrls() {
		...
		$module_srls = array();
		if (is_array($output->data)) foreach($output->data as $val) $module_srls[] = $val->module_srl;
		return $module_srls;
	}
	...
	function getLang($key, $site_srl) {
		if(!$this->langs[$site_srl]) {
			...
			foreach($output->data as $value) $this->langs[$site_srl][$value->name] = $value->value;
		}
		if($this->langs[$site_srl][$key]) return $this->langs[$site_srl][$key];
		else return $key;
	}
	...
	function handleLang($title, $site_srl) {
		...
		else return $this->getLang($matches[1], $site_srl);
	}

	function getSyndicationList() {
		$oModuleModel = getModel('module');
		$config = $oModuleModel->getModuleConfig('syndication');
		if(!$config->year || !$config->site_url || !$config->syndication_token) return new Object(-1,'msg_check_syndication_config');
		$id = Context::get('id');
		...
		if($config->syndication_password != Context::get('syndication_password')) return new Object(-1,'msg_invalid_request');
		$url = $matches[1];
		$year = $matches[2];
		$target = $matches[3];
		$id = $matches[4];
			...
			switch($target) {
				case 'site' :
					...
					if($module_srl) {
							...
							if(!$site_info->title) $site_info->title = $site_info->site_title;
					} else {
						...
						if(is_array($except_module_output->data)) {
							...
							foreach($except_module_output->data as $val) $except_module_srls[] = $val->module_srl;
				...
				case 'channel' :
					...
					if($except_module_srls) $args->except_modules = implode(',',$except_module_srls);
					$output = executeQuery('syndication.getSiteUpdatedTime', $args);
				...
				case 'article':
					...
					if(!$channel_info->site_title) $channel_info->site_title = $channel_info->title;
					$channel_info->updated = date("Y-m-d\\TH:i:s").$time_zone;
					$channel_info->self_href = $this->getSelfHref($channel_info->id, $type);
					$channel_info->site_url = getFullSiteUrl($this->uri_scheme . $this->site_url, '');
					$channel_info->alternative_href = $this->getChannelAlternativeHref($module_info->module_srl);
	...
	function getResentPingLogPath() {
		...
		if(!file_exists($target_filename)) FileHandler::writeFile($target_filename, '');
	...
	function setResentPingLog($msg) {
		...
		if(count($list)>=10) array_pop($list);
