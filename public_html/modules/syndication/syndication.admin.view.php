...
class syndicationAdminView extends syndication
{
	...
	public function dispSyndicationAdminConfig() {
		...
		if(!$module_config->syndication_use) $module_config->syndication_use = 'Y';
		if(!$module_config->site_url) $module_config->site_url = Context::getDefaultUrl()?Context::getDefaultUrl():getFullUrl();
		if(!$module_config->year) $module_config->year = date("Y");
		if(!isset($module_config->syndication_password)) $module_config->syndication_password = uniqid();
		...
		if($output->data && count($output->data) > 0) foreach($output->data as $item) $except_module_list[] = $item;
