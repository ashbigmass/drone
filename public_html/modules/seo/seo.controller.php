...
class seoController extends seo
{
	...
	function triggerBeforeDisplay(&$output_content) {
		...
		$piece->image = array();
		$piece->author = null;
		if(stristr($_SERVER['HTTP_USER_AGENT'], 'facebookexternalhit') != FALSE) $single_image = true;
		if ($document_srl) {
			...
			if (is_a($oDocument, 'documentItem') && $document_srl == $oDocument->document_srl) $is_article = true;
		}
		if ($is_article) {
			if (!$oDocument->isSecret()) {
				...
				if($document_images === false && $oDocument->hasUploadedFiles()) {
					...
					foreach ($oDocument->getUploadedFiles() as $file) {
						...
						if($file->cover_image === 'Y') array_unshift($document_images, $image);
						else $document_images[] = $image;
					}
					if($oCacheHandler->isSupport()) $oCacheHandler->put($cache_key_document_images, $document_images);
				}
		...
		if($oCacheHandler->isSupport()) {
			...
			if($site_image) $site_image['url'] = $config->site_image_url;
		...
		if($is_article) {
			if(Context::getLangType() !== $oDocument->getLangCode()) $this->addMeta('og:locale:alternate', $locales[$oDocument->getLangCode()]);
			$this->addMeta('article:published_time', $oDocument->getRegdate('c'));
			$this->addMeta('article:modified_time', $oDocument->getUpdate('c'));
			foreach ($piece->tags as $tag) $this->addMeta('article:tag', $tag);
		}
