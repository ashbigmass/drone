...
class PHPExcel_Reader_CSV implements PHPExcel_Reader_IReader
{
	...
	public function listWorksheetInfo($pFilename) {
		...
		if ($fileHandle === false) throw new Exception("Could not open file " . $pFilename . " for reading.");
	...
	public function loadIntoExisting($pFilename, PHPExcel $objPHPExcel) {
		...
		$escapeEnclosures = array( "\\" . $this->_enclosure,$this->_enclosure . $this->_enclosure);
		$currentRow = 1;
		if ($this->_contiguous) $currentRow = ($this->_contiguousRow == -1) ? $sheet->getHighestRow(): $this->_contiguousRow;
		while (($rowData = fgetcsv($fileHandle, 0, $this->_delimiter, $this->_enclosure)) !== FALSE) {
			$columnLetter = 'A';
			foreach($rowData as $rowDatum) {
				if ($rowDatum != '' && $this->_readFilter->readCell($columnLetter, $currentRow)) {
					...
					if ($this->_inputEncoding !== 'UTF-8') $rowDatum = PHPExcel_Shared_String::ConvertEncoding($rowDatum, 'UTF-8', $this->_inputEncoding);
		...
		fclose($fileHandle);
		if ($this->_contiguous) $this->_contiguousRow = $currentRow;
	...
	public function setContiguous($contiguous = false) {
		...
		if (!$contiguous) $this->_contiguousRow	= -1;
		return $this;
	}
