...
class PHPExcel_Reader_Excel2007_Chart
{
	private static function _getAttribute($component, $name, $format) {
		...
		if (isset($attributes[$name])) {
			if ($format == 'string') return (string) $attributes[$name];
			elseif ($format == 'integer') return (integer) $attributes[$name];
			elseif ($format == 'boolean') return (boolean) ($attributes[$name] == '0') ? false : true;
			else return (float) $attributes[$name];
		}
		return null;
	}

	private static function _readColor($color,$background=false) {
		if (isset($color["rgb"])) return (string)$color["rgb"];
		else if (isset($color["indexed"])) return PHPExcel_Style_Color::indexedColor($color["indexed"]-7,$background)->getARGB();
	}
					...
					foreach($chartElement as $chartDetailsKey => $chartDetails) {
						...
						switch ($chartDetailsKey) {
							case "plotArea":
									...
									if ($plotAreaLayout == NULL) $plotAreaLayout = new PHPExcel_Chart_Layout();
	...
	private static function _chartLayoutDetails($chartDetail,$namespacesChartMeta) {
		...
		$layout = array();
		foreach($details as $detailKey => $detail) $layout[$detailKey] = self::_getAttribute($detail, 'val', 'string');
		...
		foreach($seriesValueSet->lvl as $seriesLevelIdx => $seriesLevel) {
			foreach($seriesLevel as $seriesValueIdx => $seriesValue) {
				switch ($seriesValueIdx) {
					...
					case 'pt':
						$pointVal = self::_getAttribute($seriesValue, 'idx', 'integer');
						if ($dataType == 's') $seriesVal[$pointVal][] = (string) $seriesValue->v;
						else $seriesVal[$pointVal][] = (float) $seriesValue->v;
						break;
				}
			}
		}
	...
	private static function _parseRichText($titleDetailPart = null) {
		...
		foreach($titleDetailPart as $titleDetailElementKey => $titleDetailElement) {
			...
			if (isset($titleDetailElement->rPr)) {
				if (isset($titleDetailElement->rPr->rFont["val"])) $objText->getFont()->setName((string) $titleDetailElement->rPr->rFont["val"]);
				$fontSize = (self::_getAttribute($titleDetailElement->rPr, 'sz', 'integer'));
				if (!is_null($fontSize)) $objText->getFont()->setSize(floor($fontSize / 100));
				$fontColor = (self::_getAttribute($titleDetailElement->rPr, 'color', 'string'));
				if (!is_null($fontColor)) $objText->getFont()->setColor( new PHPExcel_Style_Color( self::_readColor($fontColor) ) );
				$bold = self::_getAttribute($titleDetailElement->rPr, 'b', 'boolean');
				if (!is_null($bold)) $objText->getFont()->setBold($bold);
				$italic = self::_getAttribute($titleDetailElement->rPr, 'i', 'boolean');
				if (!is_null($italic)) $objText->getFont()->setItalic($italic);
				$baseline = self::_getAttribute($titleDetailElement->rPr, 'baseline', 'integer');
				if (!is_null($baseline)) {
					if ($baseline > 0) $objText->getFont()->setSuperScript(true);
					elseif($baseline < 0) $objText->getFont()->setSubScript(true);
				}
				$underscore = (self::_getAttribute($titleDetailElement->rPr, 'u', 'string'));
				if (!is_null($underscore)) {
					if ($underscore == 'sng') $objText->getFont()->setUnderline(PHPExcel_Style_Font::UNDERLINE_SINGLE);
					elseif($underscore == 'dbl') $objText->getFont()->setUnderline(PHPExcel_Style_Font::UNDERLINE_DOUBLE);
					else $objText->getFont()->setUnderline(PHPExcel_Style_Font::UNDERLINE_NONE);
				}
				$strikethrough = (self::_getAttribute($titleDetailElement->rPr, 's', 'string'));
				if (!is_null($strikethrough)) {
					if ($strikethrough == 'noStrike') $objText->getFont()->setStrikethrough(false);
					else $objText->getFont()->setStrikethrough(true);
				}
			}
		}
		return $value;
	...
	private static function _readChartAttributes($chartDetail) {
		...
		if (isset($chartDetail->dLbls)) {
			if (isset($chartDetail->dLbls->howLegendKey)) $plotAttributes['showLegendKey'] = self::_getAttribute($chartDetail->dLbls->showLegendKey, 'val', 'string');
			if (isset($chartDetail->dLbls->showVal)) $plotAttributes['showVal'] = self::_getAttribute($chartDetail->dLbls->showVal, 'val', 'string');
			if (isset($chartDetail->dLbls->showCatName)) $plotAttributes['showCatName'] = self::_getAttribute($chartDetail->dLbls->showCatName, 'val', 'string');
			if (isset($chartDetail->dLbls->showSerName)) $plotAttributes['showSerName'] = self::_getAttribute($chartDetail->dLbls->showSerName, 'val', 'string');
			if (isset($chartDetail->dLbls->showPercent)) $plotAttributes['showPercent'] = self::_getAttribute($chartDetail->dLbls->showPercent, 'val', 'string');
			if (isset($chartDetail->dLbls->showBubbleSize)) $plotAttributes['showBubbleSize'] = self::_getAttribute($chartDetail->dLbls->showBubbleSize, 'val', 'string');
			if (isset($chartDetail->dLbls->showLeaderLines)) $plotAttributes['showLeaderLines'] = self::_getAttribute($chartDetail->dLbls->showLeaderLines, 'val', 'string');
		}
	...
	private static function _setChartAttributes($plotArea,$plotAttributes) {
		foreach($plotAttributes as $plotAttributeKey => $plotAttributeValue) {
			switch($plotAttributeKey) {
				case 'showLegendKey' : $plotArea->setShowLegendKey($plotAttributeValue); break;
				case 'showVal' : $plotArea->setShowVal($plotAttributeValue); break;
				case 'showCatName' : $plotArea->setShowCatName($plotAttributeValue); break;
				case 'showSerName' : $plotArea->setShowSerName($plotAttributeValue); break;
				case 'showPercent' : $plotArea->setShowPercent($plotAttributeValue); break;
				case 'showBubbleSize' : $plotArea->setShowBubbleSize($plotAttributeValue); break;
				case 'showLeaderLines' : $plotArea->setShowLeaderLines($plotAttributeValue); break;
			}
		}
	}
}
