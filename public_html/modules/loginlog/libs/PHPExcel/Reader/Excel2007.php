...
class PHPExcel_Reader_Excel2007 implements PHPExcel_Reader_IReader
{
				...
				foreach ($rels->Relationship as $rel) {
					switch ($rel["Type"]) {
						case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
							if (basename($rel["Target"]) == 'workbook.xml') $xl = true;
						break;
					}
				}
			}
		...
		foreach ($rels->Relationship as $rel) {
			if ($rel["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument") {
				...
				foreach ($relsWorkbook->Relationship as $ele) {
					if ($ele["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet") $worksheets[(string) $ele["Id"]] = $ele["Target"];
				}
	...
	public function _getFromZipArchive(ZipArchive $archive, $fileName = '') 	{
		if (strpos($fileName, '//') !== false) $fileName = substr($fileName, strpos($fileName, '//') + 1);
		$fileName = PHPExcel_Shared_File::realpath($fileName);
		$contents = $archive->getFromName($fileName);
		if ($contents === false) $contents = $archive->getFromName(substr($fileName, 1));
		return $contents;
	}
		...
		foreach ($rels->Relationship as $rel) {
			switch ($rel["Type"]) {
				case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
					$xmlWorkbook = simplexml_load_string($this->_getFromZipArchive($zip, "{$rel['Target']}"));
					if ($xmlWorkbook->sheets) {
						foreach ($xmlWorkbook->sheets->sheet as $eleSheet) $worksheetNames[] = (string) $eleSheet["name"];
					}
			}
		}
		$zip->close();
				...
				case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/theme":
					...
					if (is_object($xmlTheme)) {
						...
						foreach ($colourScheme as $k => $xmlColour) {
							...
							if ($themePos === false) $themePos = $themeOrderAdditional++;
				...
				case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
					...
					if (isset($xmlStrings) && isset($xmlStrings->si)) {
						foreach ($xmlStrings->si as $val) {
							if (isset($val->t)) $sharedStrings[] = PHPExcel_Shared_String::ControlCharacterOOXML2PHP( (string) $val->t );
							elseif (isset($val->r)) $sharedStrings[] = $this->_parseRichText($val);
						}
					}
					$worksheets = array();
					foreach ($relsWorkbook->Relationship as $ele) {
					...
					$numFmts = null;
					if ($xmlStyles && $xmlStyles->numFmts[0]) $numFmts = $xmlStyles->numFmts[0];
					if (isset($numFmts) && ($numFmts !== NULL)) $numFmts->registerXPathNamespace("sml", "http://schemas.openxmlformats.org/spreadsheetml/2006/main");
					if (!$this->_readDataOnly && $xmlStyles) {
						foreach ($xmlStyles->cellXfs->xf as $xf) {
							...
							if ($xf["numFmtId"]) {
								if (isset($numFmts)) {
									...
									if (isset($tmpNumFmt["formatCode"])) $numFmt = (string) $tmpNumFmt["formatCode"];
								}
								if ((int)$xf["numFmtId"] < 164) $numFmt = PHPExcel_Style_NumberFormat::builtInFormatCode((int)$xf["numFmtId"]);
							}
							$style = (object) array(
								"numFmt" => $numFmt,
						...
						foreach ($xmlStyles->cellStyleXfs->xf as $xf) {
							...
							if ($numFmts && $xf["numFmtId"]) {
								$tmpNumFmt = self::array_item($numFmts->xpath("sml:numFmt[@numFmtId=$xf[numFmtId]]"));
								if (isset($tmpNumFmt["formatCode"])) $numFmt = (string) $tmpNumFmt["formatCode"];
								else if ((int)$xf["numFmtId"] < 165) $numFmt = PHPExcel_Style_NumberFormat::builtInFormatCode((int)$xf["numFmtId"]);
							}
					...
					if ($xmlWorkbook->workbookPr) {
						...
						if (isset($xmlWorkbook->workbookPr['date1904'])) {
							...
							if ($date1904 == "true" || $date1904 == "1") PHPExcel_Shared_Date::setExcelCalendar(PHPExcel_Shared_Date::CALENDAR_MAC_1904);
						}
					}
					$sheetId = 0;
					$oldSheetId = -1;
					...
					if ($xmlWorkbook->sheets) {
						foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
							...
							if (isset($xmlSheet->sheetViews) && isset($xmlSheet->sheetViews->sheetView)) {
								if (isset($xmlSheet->sheetViews->sheetView['zoomScale'])) $docSheet->getSheetView()->setZoomScale( intval($xmlSheet->sheetViews->sheetView['zoomScale']) );
								if (isset($xmlSheet->sheetViews->sheetView['zoomScaleNormal'])) $docSheet->getSheetView()->setZoomScaleNormal( intval($xmlSheet->sheetViews->sheetView['zoomScaleNormal']) );
								if (isset($xmlSheet->sheetViews->sheetView['view'])) $docSheet->getSheetView()->setView((string) $xmlSheet->sheetViews->sheetView['view']);
								if (isset($xmlSheet->sheetViews->sheetView['showGridLines'])) $docSheet->setShowGridLines((string)$xmlSheet->sheetViews->sheetView['showGridLines'] ? true : false);
								if (isset($xmlSheet->sheetViews->sheetView['showRowColHeaders'])) $docSheet->setShowRowColHeaders((string)$xmlSheet->sheetViews->sheetView['showRowColHeaders'] ? true : false);
								if (isset($xmlSheet->sheetViews->sheetView['rightToLeft'])) $docSheet->setRightToLeft((string)$xmlSheet->sheetViews->sheetView['rightToLeft'] ? true : false);
								if (isset($xmlSheet->sheetViews->sheetView->pane)) {
									if (isset($xmlSheet->sheetViews->sheetView->pane['topLeftCell'])) {
										...
										$xSplit = 0;
										$ySplit = 0;
										if (isset($xmlSheet->sheetViews->sheetView->pane['xSplit'])) $xSplit = 1 + intval($xmlSheet->sheetViews->sheetView->pane['xSplit']);
										if (isset($xmlSheet->sheetViews->sheetView->pane['ySplit'])) $ySplit = 1 + intval($xmlSheet->sheetViews->sheetView->pane['ySplit']);
							...
							if (isset($xmlSheet->sheetPr) && isset($xmlSheet->sheetPr->tabColor)) {
								if (isset($xmlSheet->sheetPr->tabColor['rgb'])) $docSheet->getTabColor()->setARGB( (string)$xmlSheet->sheetPr->tabColor['rgb'] );
							}
							...
							if (isset($xmlSheet->sheetFormatPr)) {
								...
								if (isset($xmlSheet->sheetFormatPr['defaultColWidth'])) $docSheet->getDefaultColumnDimension()->setWidth( (float)$xmlSheet->sheetFormatPr['defaultColWidth'] );
							...
							if (isset($xmlSheet->cols) && !$this->_readDataOnly) {
								foreach ($xmlSheet->cols->col as $col) {
									for ($i = intval($col["min"]) - 1; $i < intval($col["max"]); ++$i) {
										if ($col["style"] && !$this->_readDataOnly) $docSheet->getColumnDimension(PHPExcel_Cell::stringFromColumnIndex($i))->setXfIndex(intval($col["style"]));
										if ($col["bestFit"]) {}
										if ($col["hidden"]) $docSheet->getColumnDimension(PHPExcel_Cell::stringFromColumnIndex($i))->setVisible(false);
										if ($col["collapsed"]) $docSheet->getColumnDimension(PHPExcel_Cell::stringFromColumnIndex($i))->setCollapsed(true);
										if ($col["outlineLevel"] > 0) $docSheet->getColumnDimension(PHPExcel_Cell::stringFromColumnIndex($i))->setOutlineLevel(intval($col["outlineLevel"]));
										$docSheet->getColumnDimension(PHPExcel_Cell::stringFromColumnIndex($i))->setWidth(floatval($col["width"]));
										if (intval($col["max"]) == 16384) break;
									}
								}
							}
							if (isset($xmlSheet->printOptions) && !$this->_readDataOnly) {
								if ($xmlSheet->printOptions['gridLinesSet'] == 'true' && $xmlSheet->printOptions['gridLinesSet'] == '1') $docSheet->setShowGridlines(true);
								if ($xmlSheet->printOptions['gridLines'] == 'true' || $xmlSheet->printOptions['gridLines'] == '1') $docSheet->setPrintGridlines(true);
								if ($xmlSheet->printOptions['horizontalCentered']) $docSheet->getPageSetup()->setHorizontalCentered(true);
								if ($xmlSheet->printOptions['verticalCentered']) $docSheet->getPageSetup()->setVerticalCentered(true);
							}
							if ($xmlSheet && $xmlSheet->sheetData && $xmlSheet->sheetData->row) {
								foreach ($xmlSheet->sheetData->row as $row) {
									if ($row["ht"] && !$this->_readDataOnly) $docSheet->getRowDimension(intval($row["r"]))->setRowHeight(floatval($row["ht"]));
									if ($row["hidden"] && !$this->_readDataOnly) $docSheet->getRowDimension(intval($row["r"]))->setVisible(false);
									if ($row["collapsed"]) $docSheet->getRowDimension(intval($row["r"]))->setCollapsed(true);
									if ($row["outlineLevel"] > 0) $docSheet->getRowDimension(intval($row["r"]))->setOutlineLevel(intval($row["outlineLevel"]));
									if ($row["s"] && !$this->_readDataOnly) $docSheet->getRowDimension(intval($row["r"]))->setXfIndex(intval($row["s"]));
									foreach ($row->c as $c) {
										$r 					= (string) $c["r"];
										...
										if ($this->getReadFilter() !== NULL) {
											$coordinates = PHPExcel_Cell::coordinateFromString($r);
											if (!$this->getReadFilter()->readCell($coordinates[0], $coordinates[1], $docSheet->getTitle())) continue;
										}
										switch ($cellDataType) {
											case "s":
												if ((string)$c->v != '') {
													$value = $sharedStrings[intval($c->v)];
													if ($value instanceof PHPExcel_RichText) $value = clone $value;
												} else {
													$value = '';
												}
												break;
											...
											case "inlineStr":
												...
												break;
											case "e":
												if (!isset($c->f)) $value = self::_castToError($c);
												else $this->_castToFormula($c,$r,$cellDataType,$value,$calculatedValue,$sharedFormulas,'_castToError');
												break;
											default:
												if (!isset($c->f)) $value = self::_castToString($c);
												else $this->_castToFormula($c,$r,$cellDataType,$value,$calculatedValue,$sharedFormulas,'_castToString');
												break;
										}
										...
										if ($value instanceof PHPExcel_RichText && $this->_readDataOnly) $value = $value->getPlainText();
										$cell = $docSheet->getCell($r);
										if ($cellDataType != '') $cell->setValueExplicit($value, $cellDataType);
										else $cell->setValue($value);
										if ($calculatedValue !== NULL) $cell->setCalculatedValue($calculatedValue);
										if ($c["s"] && !$this->_readDataOnly) $cell->setXfIndex(isset($styles[intval($c["s"])]) ? intval($c["s"]) : 0);
									}
								}
							}
							$conditionals = array();
							if (!$this->_readDataOnly && $xmlSheet && $xmlSheet->conditionalFormatting) {
								...
								foreach ($conditionals as $ref => $cfRules) {
									...
									foreach ($cfRules as $cfRule) {
										...
										if ((string)$cfRule["text"] != '') $objConditional->setText((string)$cfRule["text"]);
										if (count($cfRule->formula) > 1) foreach ($cfRule->formula as $formula) $objConditional->addCondition((string)$formula);
										else $objConditional->addCondition((string)$cfRule->formula);
										$objConditional->setStyle(clone $dxfs[intval($cfRule["dxfId"])]);
										$conditionalStyles[] = $objConditional;
									}
									$aReferences = PHPExcel_Cell::extractAllCellReferencesInRange($ref);
									foreach ($aReferences as $reference) $docSheet->getStyle($reference)->setConditionalStyles($conditionalStyles);
								}
							}
							...
							if ($xmlSheet && $xmlSheet->autoFilter && !$this->_readDataOnly) {
								...
								foreach ($xmlSheet->autoFilter->filterColumn as $filterColumn) {
									...
									if ($filterColumn->customFilters) {
										...
										if ((isset($customFilters["and"])) && ($customFilters["and"] == 1)) $column->setJoin(PHPExcel_Worksheet_AutoFilter_Column::AUTOFILTER_COLUMN_JOIN_AND);
									...
									if ($filterColumn->dynamicFilter) {
										...
										foreach ($filterColumn->dynamicFilter as $filterRule) {
											...
											if (isset($filterRule["val"])) $column->setAttribute('val',(string) $filterRule["val"]);
											if (isset($filterRule["maxVal"])) $column->setAttribute('maxVal',(string) $filterRule["maxVal"]);
										}
									}
							...
							if ($xmlSheet && $xmlSheet->mergeCells && $xmlSheet->mergeCells->mergeCell && !$this->_readDataOnly) {
								foreach ($xmlSheet->mergeCells->mergeCell as $mergeCell) {
									...
									if (strpos($mergeRef,':') !== FALSE) $docSheet->mergeCells((string) $mergeCell["ref"]);
								}
							}
							...
							if ($xmlSheet && $xmlSheet->pageSetup && !$this->_readDataOnly) {
								...
								if (isset($xmlSheet->pageSetup["orientation"])) $docPageSetup->setOrientation((string) $xmlSheet->pageSetup["orientation"]);
								if (isset($xmlSheet->pageSetup["paperSize"])) $docPageSetup->setPaperSize(intval($xmlSheet->pageSetup["paperSize"]));
								if (isset($xmlSheet->pageSetup["scale"])) $docPageSetup->setScale(intval($xmlSheet->pageSetup["scale"]), false);
							...
							if ($xmlSheet && $xmlSheet->rowBreaks && $xmlSheet->rowBreaks->brk && !$this->_readDataOnly) {
								foreach ($xmlSheet->rowBreaks->brk as $brk) {
									if ($brk["man"]) $docSheet->setBreak("A$brk[id]", PHPExcel_Worksheet::BREAK_ROW);
								}
							}
							if ($xmlSheet && $xmlSheet->colBreaks && $xmlSheet->colBreaks->brk && !$this->_readDataOnly) {
								foreach ($xmlSheet->colBreaks->brk as $brk) {
									if ($brk["man"]) $docSheet->setBreak(PHPExcel_Cell::stringFromColumnIndex((string) $brk["id"]) . "1", PHPExcel_Worksheet::BREAK_COLUMN);
								}
							}
							...
							if (!$this->_readDataOnly) {
								if ($zip->locateName(dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")) {
									foreach ($relsWorksheet->Relationship as $ele) {
										if ($ele["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink") $hyperlinks[(string)$ele["Id"]] = (string)$ele["Target"];
									}
								}
								if ($xmlSheet && $xmlSheet->hyperlinks) {
									foreach ($xmlSheet->hyperlinks->hyperlink as $hyperlink) {
										...
										foreach (PHPExcel_Cell::extractAllCellReferencesInRange($hyperlink['ref']) as $cellReference) {
											...
											if (isset($linkRel['id'])) $cell->getHyperlink()->setUrl( $hyperlinks[ (string)$linkRel['id'] ] );
											if (isset($hyperlink['location'])) $cell->getHyperlink()->setUrl( 'sheet://' . (string)$hyperlink['location'] );
							...
							if (!$this->_readDataOnly) {
								if ($zip->locateName(dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")) {
									foreach ($relsWorksheet->Relationship as $ele) {
										if ($ele["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/comments") $comments[(string)$ele["Id"]] = (string)$ele["Target"];
										if ($ele["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/vmlDrawing") $vmlComments[(string)$ele["Id"]] = (string)$ele["Target"];
									}
								}
								foreach ($comments as $relName => $relPath) {
									...
									$authors = array();
									foreach ($commentsFile->authors->author as $author) $authors[] = (string)$author;
									...
									if ($zip->locateName(dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")) {
										...
										if ($vmlRelationship != '') {
											...
											foreach ($shapes as $shape) {
												...
												if (isset($imageData['title'])) $hfImages[ (string)$shape['id'] ]->setName( (string)$imageData['title'] );
										...
										if ($xmlDrawing->oneCellAnchor) {
											foreach ($xmlDrawing->oneCellAnchor as $oneCellAnchor) {
												if ($oneCellAnchor->pic->blipFill) {
													...
													if ($xfrm) $objDrawing->setRotation(PHPExcel_Shared_Drawing::angleToDegrees(self::array_item($xfrm->attributes(), "rot")));
													if ($outerShdw) {
							...
							if ($xmlWorkbook->definedNames) {
								foreach ($xmlWorkbook->definedNames->definedName as $definedName) {
									...
									if (($spos = strpos($extractedRange,'!')) !== false) $extractedRange = substr($extractedRange,0,$spos).str_replace('$', '', substr($extractedRange,$spos));
									else $extractedRange = str_replace('$', '', $extractedRange);
									if (stripos((string)$definedName, '#REF!') !== FALSE || $extractedRange == '') continue;
									if ((string)$definedName['localSheetId'] != '' && (string)$definedName['localSheetId'] == $sheetId) {
										switch ((string)$definedName['name']) {
											case '_xlnm._FilterDatabase':
												if ((string)$definedName['hidden'] !== '1') $docSheet->getAutoFilter()->setRange($extractedRange);
												break;
											case '_xlnm.Print_Titles':
												$extractedRange = explode(',', $extractedRange);
												foreach ($extractedRange as $range) {
													$matches = array();
													if (preg_match('/^([A-Z]+)\:([A-Z]+)$/', $range, $matches)) $docSheet->getPageSetup()->setColumnsToRepeatAtLeft(array($matches[1], $matches[2]));
													elseif (preg_match('/^(\d+)\:(\d+)$/', $range, $matches)) $docSheet->getPageSetup()->setRowsToRepeatAtTop(array($matches[1], $matches[2]));
												}
												break;
						...
						if ($xmlWorkbook->definedNames) {
							foreach ($xmlWorkbook->definedNames->definedName as $definedName) {
								...
								if ((string)$definedName['localSheetId'] != '') {
									...
									if ($locatedSheet !== NULL) $excel->addNamedRange( new PHPExcel_NamedRange((string)$definedName['name'], $locatedSheet, $extractedRange, false) );
								}
							}
						}
					}
					if (!$this->_readDataOnly) {
						...
						if (isset($mapSheetId[$activeTab]) && $mapSheetId[$activeTab] !== null) {
							...
							if ($excel->getSheetCount() == 0) $excel->createSheet();
	...
	private static function _readStyle($docStyle, $style) {
		...
		if (isset($style->font)) {
			...
			if (isset($style->font->b)) $docStyle->getFont()->setBold(!isset($style->font->b["val"]) || $style->font->b["val"] == 'true' || $style->font->b["val"] == '1');
			if (isset($style->font->i)) $docStyle->getFont()->setItalic(!isset($style->font->i["val"]) || $style->font->i["val"] == 'true' || $style->font->i["val"] == '1');
			if (isset($style->font->strike)) $docStyle->getFont()->setStrikethrough(!isset($style->font->strike["val"]) || $style->font->strike["val"] == 'true' || $style->font->strike["val"] == '1');
			$docStyle->getFont()->getColor()->setARGB(self::_readColor($style->font->color));
			if (isset($style->font->u) && !isset($style->font->u["val"])) $docStyle->getFont()->setUnderline(PHPExcel_Style_Font::UNDERLINE_SINGLE);
			else if (isset($style->font->u) && isset($style->font->u["val"])) $docStyle->getFont()->setUnderline((string)$style->font->u["val"]);
			if (isset($style->font->vertAlign) && isset($style->font->vertAlign["val"])) {
				$vertAlign = strtolower((string)$style->font->vertAlign["val"]);
				if ($vertAlign == 'superscript') $docStyle->getFont()->setSuperScript(true);
				if ($vertAlign == 'subscript') $docStyle->getFont()->setSubScript(true);
			}
		}
		if (isset($style->fill)) {
			if ($style->fill->gradientFill) {
				$gradientFill = $style->fill->gradientFill[0];
				if(!empty($gradientFill["type"])) $docStyle->getFill()->setFillType((string) $gradientFill["type"]);
				...
				if ($style->fill->patternFill->fgColor) $docStyle->getFill()->getStartColor()->setARGB(self::_readColor($style->fill->patternFill->fgColor,true));
				else $docStyle->getFill()->getStartColor()->setARGB('FF000000');
				if ($style->fill->patternFill->bgColor) $docStyle->getFill()->getEndColor()->setARGB(self::_readColor($style->fill->patternFill->bgColor,true));
			}
		}
		if (isset($style->border)) {
			$diagonalUp   = false;
			$diagonalDown = false;
			if ($style->border["diagonalUp"] == 'true' || $style->border["diagonalUp"] == 1) $diagonalUp = true;
			if ($style->border["diagonalDown"] == 'true' || $style->border["diagonalDown"] == 1) $diagonalDown = true;
		...
		if (isset($style->alignment)) {
			...
			$textRotation = 0;
			if ((int)$style->alignment["textRotation"] <= 90) $textRotation = (int)$style->alignment["textRotation"];
			else if ((int)$style->alignment["textRotation"] > 90) $textRotation = 90 - (int)$style->alignment["textRotation"];
		...
		if (isset($style->protection)) {
			if (isset($style->protection['locked'])) {
				if ((string)$style->protection['locked'] == 'true') $docStyle->getProtection()->setLocked(PHPExcel_Style_Protection::PROTECTION_PROTECTED);
				else $docStyle->getProtection()->setLocked(PHPExcel_Style_Protection::PROTECTION_UNPROTECTED);
			}
			if (isset($style->protection['hidden'])) {
				if ((string)$style->protection['hidden'] == 'true') $docStyle->getProtection()->setHidden(PHPExcel_Style_Protection::PROTECTION_PROTECTED);
				else $docStyle->getProtection()->setHidden(PHPExcel_Style_Protection::PROTECTION_UNPROTECTED);
			}
		}
	}

	private static function _readBorder($docBorder, $eleBorder) {
		if (isset($eleBorder["style"])) $docBorder->setBorderStyle((string) $eleBorder["style"]);
		if (isset($eleBorder->color)) $docBorder->getColor()->setARGB(self::_readColor($eleBorder->color));
	}
		...
		if (isset($is->t)) {
			...
			foreach ($is->r as $run) {
				if (!isset($run->rPr)) {
					...
					if (isset($run->rPr->sz["val"])) $objText->getFont()->setSize((string) $run->rPr->sz["val"]);
					if (isset($run->rPr->color)) $objText->getFont()->setColor( new PHPExcel_Style_Color( self::_readColor($run->rPr->color) ) );
					...
					if (isset($run->rPr->vertAlign) && isset($run->rPr->vertAlign["val"])) {
						...
						if ($vertAlign == 'superscript') $objText->getFont()->setSuperScript(true);
						if ($vertAlign == 'subscript') $objText->getFont()->setSubScript(true);
					}
					if (isset($run->rPr->u) && !isset($run->rPr->u["val"])) $objText->getFont()->setUnderline(PHPExcel_Style_Font::UNDERLINE_SINGLE);
					else if (isset($run->rPr->u) && isset($run->rPr->u["val"])) $objText->getFont()->setUnderline((string)$run->rPr->u["val"]);
	...
	private static function toCSSArray($style) {
		...
		foreach ($temp as $item) {
			$item = explode(':', $item);
			if (strpos($item[1], 'px') !== false) $item[1] = str_replace('px', '', $item[1]);
