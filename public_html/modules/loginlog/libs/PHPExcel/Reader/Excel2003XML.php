...
class PHPExcel_Reader_Excel2003XML implements PHPExcel_Reader_IReader
{
	...
	public function canRead($pFilename) {
		...
		if(preg_match('/<?xml.*encoding=[\'"](.*?)[\'"].*?>/um',$data,$matches)) $this->_charSet = strtoupper($matches[1]);
		return $valid;
	}

	public function listWorksheetNames($pFilename) {
		if (!file_exists($pFilename)) throw new Exception("Could not open " . $pFilename . " for reading! File does not exist.");
		if (!$this->canRead($pFilename)) throw new Exception($pFilename . " is an Invalid Spreadsheet file.");
		...
		foreach($xml_ss->Worksheet as $worksheet) {
			...
			if (isset($worksheet->Table->Row)) {
				...
				foreach($worksheet->Table->Row as $rowData) {
					...
					++$rowIndex;
					if ($rowHasData) $tmpInfo['totalRows'] = max($tmpInfo['totalRows'], $rowIndex);
				}
			}
	...
	public function loadIntoExisting($pFilename, PHPExcel $objPHPExcel) {
		...
		foreach($xml->Styles[0] as $style) {
			...
			if ($styleID == 'Default') $this->_styles['Default'] = array();
			else $this->_styles[$styleID] = $this->_styles['Default'];
		...
		foreach($xml_ss->Worksheet as $worksheet) {
			$worksheet_ss = $worksheet->attributes($namespaces['ss']);
			if ((isset($this->_loadSheetsOnly)) && (isset($worksheet_ss['Name'])) && (!in_array($worksheet_ss['Name'], $this->_loadSheetsOnly))) continue;
			...
			if (isset($worksheet->Table->Column)) {
				foreach($worksheet->Table->Column as $columnData) {
					...
					if (isset($columnData_ss['Index'])) $columnID = PHPExcel_Cell::stringFromColumnIndex($columnData_ss['Index']-1);
			...
			if (isset($worksheet->Table->Row)) {
				foreach($worksheet->Table->Row as $rowData) {
					...
					if (isset($row_ss['Index'])) $rowID = (integer) $row_ss['Index'];
					$columnID = 'A';
					foreach($rowData->Cell as $cell) {
						$cell_ss = $cell->attributes($namespaces['ss']);
						if (isset($cell_ss['Index'])) $columnID = PHPExcel_Cell::stringFromColumnIndex($cell_ss['Index']-1);
						$cellRange = $columnID.$rowID;
						if ($this->getReadFilter() !== NULL) {
							if (!$this->getReadFilter()->readCell($columnID, $rowID, $worksheetName)) continue;
						}
						if ((isset($cell_ss['MergeAcross'])) || (isset($cell_ss['MergeDown']))) {
							$columnTo = $columnID;
							if (isset($cell_ss['MergeAcross'])) $columnTo = PHPExcel_Cell::stringFromColumnIndex(PHPExcel_Cell::columnIndexFromString($columnID) + $cell_ss['MergeAcross'] -1);
							$rowTo = $rowID;
							if (isset($cell_ss['MergeDown'])) $rowTo = $rowTo + $cell_ss['MergeDown'];
						...
						if (isset($cell_ss['Formula'])) {
							...
							if (isset($cell_ss['ArrayRange'])) $cellDataCSEFormula = $cell_ss['ArrayRange'];
						...
						if (isset($cell->Data)) {
							...
							if ($hasCalculatedValue) {
								...
								if (substr($cellDataFormula,0,3) == 'of:') {
									...
									foreach($temp as &$value) {
										if ($key = !$key) $value = str_replace(array('[.','.',']'),'',$value);
									}
								} else {
									$temp = explode('"',$cellDataFormula);
									$key = false;
							...
							if ($hasCalculatedValue) $objPHPExcel->getActiveSheet()->getCell($columnID.$rowID)->setCalculatedValue($cellValue);
							$cellIsSet = $rowHasData = true;
						}
						if (isset($cell->Comment)) {
							$commentAttributes = $cell->Comment->attributes($namespaces['ss']);
							$author = 'unknown';
							if (isset($commentAttributes->Author)) $author = (string)$commentAttributes->Author;
						...
						if (($cellIsSet) && (isset($cell_ss['StyleID']))) {
							...
							if ((isset($this->_styles[$style])) && (!empty($this->_styles[$style]))) {
								if (!$objPHPExcel->getActiveSheet()->cellExists($columnID.$rowID)) $objPHPExcel->getActiveSheet()->getCell($columnID.$rowID)->setValue(NULL);
					...
					if ($rowHasData) {
						if (isset($row_ss['StyleID'])) $rowStyle = $row_ss['StyleID'];
	...
	private static function _convertStringEncoding($string,$charset) {
		if ($charset != 'UTF-8') return PHPExcel_Shared_String::ConvertEncoding($string,'UTF-8',$charset);
		return $string;
	}
