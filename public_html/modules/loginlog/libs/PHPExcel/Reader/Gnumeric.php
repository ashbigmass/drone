...
class PHPExcel_Reader_Gnumeric implements PHPExcel_Reader_IReader
{
	...
	public function canRead($pFilename) {
		...
		$data = fread($fh, 2);
		fclose($fh);
		if ($data != chr(0x1F).chr(0x8B)) return false;
		return true;
	}
	...
	public function listWorksheetNames($pFilename) {
		...
		foreach($gnmXML->Sheets->Sheet as $sheet) $worksheetNames[] = (string) $sheet->Name;
	...
	public function loadIntoExisting($pFilename, PHPExcel $objPHPExcel) {
		...
		if (isset($namespacesMeta['office'])) {
			...
			foreach($officeDocMetaXML as $officePropertyData) {
				...
				if (isset($namespacesMeta['meta'])) $officePropertyMeta = $officePropertyData->children($namespacesMeta['meta']);
		...
		foreach($gnmXML->Sheets->Sheet as $sheet) {
			...
			if ((isset($this->_loadSheetsOnly)) && (!in_array($worksheetName, $this->_loadSheetsOnly))) continue;
			$maxRow = $maxCol = 0;
			...
			foreach($sheet->Cells->Cell as $cell) {
				...
				if ($this->getReadFilter() !== NULL) {
					if (!$this->getReadFilter()->readCell($column, $row, $worksheetName)) continue;
				}
				...
				if (($styleAttributes['startRow'] <= $maxRow) &&
					...
					if ((!$this->_readDataOnly) ||
						...
						if (!$this->_readDataOnly) {
							...
							if (isset($styleRegion->Style->HyperLink)) $hyperlink = $styleRegion->Style->HyperLink->attributes();
						}
			...
			if (isset($sheet->MergedRegions)) {
				foreach($sheet->MergedRegions->Merge as $mergeCells) {
					if (strpos($mergeCells,':') !== FALSE) $objPHPExcel->getActiveSheet()->mergeCells($mergeCells);
				}
			}
			$worksheetID++;
		}
		if (isset($gnmXML->Names)) {
			foreach($gnmXML->Names->Name as $namedRange) {
				...
				if (stripos($range, '#REF!') !== false) continue;
				$range = explode('!',$range);
	...
	private static function _parseBorderAttributes($borderAttributes) {
		...
		switch ($borderAttributes["Style"]) {
			case '0' : $styleArray['style'] = PHPExcel_Style_Border::BORDER_NONE; break;
			case '1' : $styleArray['style'] = PHPExcel_Style_Border::BORDER_THIN; break;
			case '2' : $styleArray['style'] = PHPExcel_Style_Border::BORDER_MEDIUM; break;
			case '4' : $styleArray['style'] = PHPExcel_Style_Border::BORDER_DASHED; break;
			case '5' : $styleArray['style'] = PHPExcel_Style_Border::BORDER_THICK; break;
			case '6' : $styleArray['style'] = PHPExcel_Style_Border::BORDER_DOUBLE; break;
			case '7' : $styleArray['style'] = PHPExcel_Style_Border::BORDER_DOTTED; break;
			case '9' : $styleArray['style'] = PHPExcel_Style_Border::BORDER_DASHDOT; break;
			case '10' : $styleArray['style'] = PHPExcel_Style_Border::BORDER_MEDIUMDASHDOT; break;
			case '11' : $styleArray['style'] = PHPExcel_Style_Border::BORDER_DASHDOTDOT; break;
			case '12' : $styleArray['style'] = PHPExcel_Style_Border::BORDER_MEDIUMDASHDOTDOT; break;
			case '13' : $styleArray['style'] = PHPExcel_Style_Border::BORDER_MEDIUMDASHDOTDOT; break;
			case '3' : $styleArray['style'] = PHPExcel_Style_Border::BORDER_SLANTDASHDOT; break;
			case '8' : $styleArray['style'] = PHPExcel_Style_Border::BORDER_MEDIUMDASHED; break;
		}
		return $styleArray;
	}
