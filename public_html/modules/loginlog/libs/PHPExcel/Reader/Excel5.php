...
class PHPExcel_Reader_Excel5 implements PHPExcel_Reader_IReader
{
	...
	public function listWorksheetNames($pFilename) {
		...
		foreach ($this->_sheets as $sheet) {
			if ($sheet['sheetType'] != 0x00) continue;
	...
	public function load($pFilename) {
		...
		foreach ($this->_sheets as $sheet) {
			if ($sheet['sheetType'] != 0x00) continue;
			if (isset($this->_loadSheetsOnly) && !in_array($sheet['name'], $this->_loadSheetsOnly)) continue;
			...
			foreach ($this->_objs as $n => $obj) {
				if (isset($allSpContainers[$n + 1]) && is_object($allSpContainers[$n + 1])) {
					...
					if ($spContainer->getNestingLevel() > 1) continue;
			...
			if ($definedName['isBuiltInName']) {
				...
				case pack('C', 0x06):
					...
					foreach ($ranges as $range) {
						...
						if (count($explodes) == 2) $extractedRanges[] = str_replace('$', '', $explodes[1]);
					}
					if ($docSheet = $this->_phpExcel->getSheetByName($sheetName)) $docSheet->getPageSetup()->setPrintArea(implode(',', $extractedRanges));
					break;
				case pack('C', 0x07):
					$ranges = explode(',', $definedName['formula']);
					foreach ($ranges as $range) {
						...
						if (count($explodes) == 2) {
							if ($docSheet = $this->_phpExcel->getSheetByName($explodes[0])) {
								...
								if (count($coordinateStrings) == 2) {
									...
									if ($firstColumn == 'A' and $lastColumn == 'IV') $docSheet->getPageSetup()->setRowsToRepeatAtTop(array($firstRow, $lastRow));
									elseif ($firstRow == 1 and $lastRow == 65536) $docSheet->getPageSetup()->setColumnsToRepeatAtLeft(array($firstColumn, $lastColumn));
								}
							}
						}
					}
	...
	private function _readSummaryInformation() {
		if (!isset($this->_summaryInformation)) return;
	...
	private function _readDocumentSummaryInformation() {
		if (!isset($this->_documentSummaryInformation)) return;
	...
	private function _readBof() {
		...
		switch ($substreamType) {
			case self::XLS_WorkbookGlobals:
				...
				if (($version != self::XLS_BIFF8) && ($version != self::XLS_BIFF7)) throw new Exception('Cannot read this Excel file. Version is too old.');
	...
	private function _readDateMode() {
		...
		if (ord($recordData{0}) == 1) PHPExcel_Shared_Date::setExcelCalendar(PHPExcel_Shared_Date::CALENDAR_MAC_1904);
	}
	...
	private function _readFont() {
		...
		if (!$this->_readDataOnly) {
			...
			if ($this->_version == self::XLS_BIFF8) $string = self::_readUnicodeStringShort(substr($recordData, 14));
			else $string = $this->_readByteStringShort(substr($recordData, 14));
	...
	private function _readFormat() {
		...
		if (!$this->_readDataOnly) {
			...
			if ($this->_version == self::XLS_BIFF8) $string = self::_readUnicodeStringLong(substr($recordData, 2));
			else $string = $this->_readByteStringShort(substr($recordData, 2));
	...
	private function _readXf() {
		...
		if (!$this->_readDataOnly) {
			if (self::_GetInt2d($recordData, 0) < 4) $fontIndex = self::_GetInt2d($recordData, 0);
			else $fontIndex = self::_GetInt2d($recordData, 0) - 1;
			$objStyle->setFont($this->_objFonts[$fontIndex]);
			$numberFormatIndex = self::_GetInt2d($recordData, 2);
			if (isset($this->_formats[$numberFormatIndex])) $numberformat = array('code' => $this->_formats[$numberFormatIndex]);
			elseif (($code = PHPExcel_Style_NumberFormat::builtInFormatCode($numberFormatIndex)) !== '') $numberformat = array('code' => $code);
			else $numberformat = array('code' => 'General');
			...
			if ($this->_version == self::XLS_BIFF8) {
					...
					$rotation = 0;
					if ($angle <= 90) $rotation = $angle;
					else if ($angle <= 180) $rotation = 90 - $angle;
					else if ($angle == 255) $rotation = -165;
					...
					if ($diagonalUp == false && $diagonalDown == false) $objStyle->getBorders()->setDiagonalDirection(PHPExcel_Style_Borders::DIAGONAL_NONE);
					elseif ($diagonalUp == true && $diagonalDown == false) $objStyle->getBorders()->setDiagonalDirection(PHPExcel_Style_Borders::DIAGONAL_UP);
					elseif ($diagonalUp == false && $diagonalDown == true) $objStyle->getBorders()->setDiagonalDirection(PHPExcel_Style_Borders::DIAGONAL_DOWN);
					elseif ($diagonalUp == true && $diagonalDown == true) $objStyle->getBorders()->setDiagonalDirection(PHPExcel_Style_Borders::DIAGONAL_BOTH);
	...
	private function _readSst() {
		...
		for ($i = 0; $i < $nm; ++$i) {
			...
			if ($pos + $len <= $limitpos) {
				...
				while ($charsLeft > 0) {
					...
					if ($isCompressed && ($option == 0)) {
						...
						$len = min($charsLeft, $limitpos - $pos);
						for ($j = 0; $j < $len; ++$j) $retstr .= $recordData{$pos + $j} . chr(0);
						$charsLeft -= $len;
						$isCompressed = false;
					} else {
						$newstr = '';
						for ($j = 0; $j < strlen($retstr); ++$j) $newstr .= $retstr[$j] . chr(0);
						$retstr = $newstr;
						$len = min($charsLeft * 2, $limitpos - $pos);
						$retstr .= substr($recordData, $pos, $len);
						$charsLeft -= $len / 2;
			...
			if ($hasAsian) $pos += $extendedRunLength;
	...
	private function _readHeader() {
		...
		if (!$this->_readDataOnly) {
			if ($recordData) {
				if ($this->_version == self::XLS_BIFF8) $string = self::_readUnicodeStringLong($recordData);
				else $string = $this->_readByteStringShort($recordData);
	...
	private function _readLeftMargin() {
		...
		$recordData = substr($this->_data, $this->_pos + 4, $length);
		$this->_pos += 4 + $length;
		if (!$this->_readDataOnly) $this->_phpSheet->getPageMargins()->setLeft(self::_extractNumber($recordData));
	}
	...
	private function _readRightMargin() {
		...
		$recordData = substr($this->_data, $this->_pos + 4, $length);
		$this->_pos += 4 + $length;
		if (!$this->_readDataOnly) $this->_phpSheet->getPageMargins()->setRight(self::_extractNumber($recordData));
	}
	...
	private function _readTopMargin() {
		...
		$recordData = substr($this->_data, $this->_pos + 4, $length);
		$this->_pos += 4 + $length;
		if (!$this->_readDataOnly) $this->_phpSheet->getPageMargins()->setTop(self::_extractNumber($recordData));
	}
	...
	private function _readBottomMargin() {
		...
		$recordData = substr($this->_data, $this->_pos + 4, $length);
		$this->_pos += 4 + $length;
		if (!$this->_readDataOnly) $this->_phpSheet->getPageMargins()->setBottom(self::_extractNumber($recordData));
	}
	...
	private function _readDefColWidth() {
		...
		if ($width != 8) $this->_phpSheet->getDefaultColumnDimension()->setWidth($width);
	}
	...
	private function _readRow() {
		...
		if (!$this->_readDataOnly) {
			...
			if (!$useDefaultHeight) $this->_phpSheet->getRowDimension($r + 1)->setRowHeight($height / 20);
			...
			if ($hasExplicitFormat) $this->_phpSheet->getRowDimension($r + 1)->setXfIndex($this->_mapCellXfIndex[$xfIndex]);
		}
	}
	...
	private function _readRk() {
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			if (!$this->_readDataOnly) $cell->setXfIndex($this->_mapCellXfIndex[$xfIndex]);
					...
					if (PHPExcel_Shared_String::CountCharacters($text) > 0) {
						if ($i == 0) {
							...
							if (isset($fmtRuns[$i - 1])) {
								...
								else $fontIndex = $fmtRuns[$i - 1]['fontIndex'] - 1;
	...
	private function _readFormula() {
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			if ($isPartOfSharedFormula) $this->_sharedFormulaParts[$columnString . ($row + 1)] = $this->_baseCell;
			$xfIndex = self::_GetInt2d($recordData, 4);
			if ( (ord($recordData{6}) == 0) && (ord($recordData{12}) == 255) && (ord($recordData{13}) == 255) ) {
				$dataType = PHPExcel_Cell_DataType::TYPE_STRING;
				$code = self::_GetInt2d($this->_data, $this->_pos);
				if ($code == self::XLS_Type_SHAREDFMLA) $this->_readSharedFmla();
			...
			if (!$isPartOfSharedFormula) {
				try {
					if ($this->_version != self::XLS_BIFF8) throw new Exception('Not BIFF8. Can only read BIFF8 formulas');
					$formula = $this->_getFormulaFromStructure($formulaStructure);
	...
	private function _readBlank() {
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			$xfIndex = self::_GetInt2d($recordData, 4);
			if (!$this->_readDataOnly) $this->_phpSheet->getCell($columnString . ($row + 1))->setXfIndex($this->_mapCellXfIndex[$xfIndex]);
		}
	}
	...
	private function _readObj() {
		...
		$recordData = substr($this->_data, $this->_pos + 4, $length);
		$this->_pos += 4 + $length;
		if ($this->_readDataOnly || $this->_version != self::XLS_BIFF8) return;
	...
	private function _readWindow2() {
		...
		if ($isActive) $this->_phpExcel->setActiveSheetIndex($this->_phpExcel->getIndex($this->_phpSheet));
	}
	...
	private function _readSelection() {
		...
		if (!$this->_readDataOnly) {
			...
			if (preg_match('/^([A-Z]+1\:[A-Z]+)16384$/', $selectedCells)) $selectedCells = preg_replace('/^([A-Z]+1\:[A-Z]+)16384$/', '${1}1048576', $selectedCells);
			if (preg_match('/^([A-Z]+1\:[A-Z]+)65536$/', $selectedCells)) $selectedCells = preg_replace('/^([A-Z]+1\:[A-Z]+)65536$/', '${1}1048576', $selectedCells);
			if (preg_match('/^(A[0-9]+\:)IV([0-9]+)$/', $selectedCells)) $selectedCells = preg_replace('/^(A[0-9]+\:)IV([0-9]+)$/', '${1}XFD${2}', $selectedCells);
	...
	private function _readMergedCells() {
		...
		if ($this->_version == self::XLS_BIFF8 && !$this->_readDataOnly) {
			...
			foreach ($cellRangeAddressList['cellRangeAddresses'] as $cellRangeAddress) {
				if ((strpos($cellRangeAddress,':') !== FALSE) && ($this->_includeCellRangeFiltered($cellRangeAddress))) $this->_phpSheet->mergeCells($cellRangeAddress);
			}
		}
	}
	...
	private function _readHyperLink() {
		...
		if (!$this->_readDataOnly) {
			...
			$hyperlinkType = null;
			if ($isUNC) $hyperlinkType = 'UNC';
			else if (!$isFileLinkOrUrl) $hyperlinkType = 'workbook';
			else if (ord($recordData{$offset}) == 0x03) $hyperlinkType = 'local';
			else if (ord($recordData{$offset}) == 0xE0) $hyperlinkType = 'URL';
	...
	private function _readDataValidation() {
		...
		try {
			...
			if ($type == PHPExcel_Cell_DataValidation::TYPE_LIST) $formula1 = str_replace(chr(0), ',', $formula1);
		} catch (Exception $e) {
			return;
		}
		$offset += $sz1;
	...
	private function _readRangeProtection() {
		...
		if (!$this->_readDataOnly) {
			...
			$offset += 4;
			if ($cellRanges) $this->_phpSheet->protectCells(implode(' ', $cellRanges), strtoupper(dechex($wPassword)), true);
		}
	}
	...
	private function _getFormulaFromStructure($formulaStructure, $baseCell = 'A1') {
		...
		if (strlen($formulaStructure) > 2 + $sz) $additionalData = substr($formulaStructure, 2 + $sz);
		else $additionalData = '';
		...
		foreach ($tokens as $token) {
			...
			case 'tFuncV':
				if ($token['data']['function'] != '') {
					$ops = array();
					for ($i = 0; $i < $token['data']['args']; ++$i) $ops[] = array_pop($formulaStrings);
					$ops = array_reverse($ops);
					$formulaStrings[] = "$space1$space0{$token['data']['function']}(" . implode(',', $ops) . ")";
					unset($space0, $space1);
				} else {
					$ops = array();
					for ($i = 0; $i < $token['data']['args'] - 1; ++$i) $ops[] = array_pop($formulaStrings);
					$ops = array_reverse($ops);
	...
	private function _readBIFF8CellAddress($cellAddressStructure) {
		$row = self::_GetInt2d($cellAddressStructure, 0) + 1;
			...
			if (!(0x4000 & self::_GetInt2d($cellAddressStructure, 2))) $column = '$' . $column;
			if (!(0x8000 & self::_GetInt2d($cellAddressStructure, 2))) $row = '$' . $row;
		return $column . $row;
	}
	...
	private function _readBIFF5CellRangeAddressFixed($subData) {
		...
		$fc = ord($subData{4});
		$lc = ord($subData{5});
		if ($fr > $lr || $fc > $lc) throw new Exception('Not a cell range address');
		$fc = PHPExcel_Cell::stringFromColumnIndex($fc);
		$lc = PHPExcel_Cell::stringFromColumnIndex($lc);
		if ($fr == $lr and $fc == $lc) return "$fc$fr";
		return "$fc$fr:$lc$lr";
	}
	...
	private function _readBIFF8CellRangeAddress($subData) {
		...
		if (!(0x4000 & self::_GetInt2d($subData, 4))) $fc = '$' . $fc;
		if (!(0x8000 & self::_GetInt2d($subData, 4))) $fr = '$' . $fr;
		$lc = PHPExcel_Cell::stringFromColumnIndex(0x00FF & self::_GetInt2d($subData, 6));
		if (!(0x4000 & self::_GetInt2d($subData, 6))) $lc = '$' . $lc;
		if (!(0x8000 & self::_GetInt2d($subData, 6))) $lr = '$' . $lr;
		return "$fc$fr:$lc$lr";
	}
		...
		if (isset($this->_ref[$index])) {
			...
			switch ($type) {
				case 'internal':
					...
					if ($firstSheetName == $lastSheetName) $sheetRange = $firstSheetName;
					else $sheetRange = "$firstSheetName:$lastSheetName";
					$sheetRange = str_replace("'", "''", $sheetRange);
					if (preg_match("/[ !\"@#£$%&{()}<>=+'|^,;-]/", $sheetRange)) $sheetRange = "'$sheetRange'";
					return $sheetRange;
					break;
				default:
	...
	private static function _extractNumber($data) {
		...
		if ($mantissalow1 != 0) $value += 1 / pow (2 , (21 - $exp));
	...
	private static function _encodeUTF16($string, $compressed = '') {
		if ($compressed) $string = self::_uncompressByteString($string);
	...
	private static function _uncompressByteString($string) {
		...
		for ($i = 0; $i < $strLen; ++$i) $uncompressedString .= $string[$i] . "\0";
	...
	private static function _readColor($color,$palette,$version) {
		if ($color <= 0x07 || $color >= 0x40) {
			...
			if ($version == self::XLS_BIFF8) return self::_mapColor($color);
			else return self::_mapColorBIFF5($color);
		}
		return $color;
	}
