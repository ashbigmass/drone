...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
	...
	public function getCellCollection($pSorted = true) {
		if ($pSorted) return $this->sortCellCollection();
		if ($this->_cellCollection !== NULL) return $this->_cellCollection->getCellList();
		return array();
	}

	public function sortCellCollection() {
		if ($this->_cellCollection !== NULL) return $this->_cellCollection->getSortedCellList();
		return array();
	}
	...
	public function addChart(PHPExcel_Chart $pChart = null, $iChartIndex = null) {
		...
		if (is_null($iChartIndex)) $this->_chartCollection[] = $pChart;
		else array_splice($this->_chartCollection, $iChartIndex, 0, array($pChart));
		return $pChart;
	}
	...
	public function getChartByIndex($index = null) {
		...
		if (is_null($index)) $index = --$chartCount;
		if (!isset($this->_chartCollection[$index])) return false;
		return $this->_chartCollection[$index];
	}

	public function getChartNames() {
		$chartNames = array();
		foreach($this->_chartCollection as $chart) $chartNames[] = $chart->getName();
		return $chartNames;
	}
	...
	public function getChartByName($chartName = '') {
		...
		foreach($this->_chartCollection as $index => $chart) {
			if ($chart->getName() == $chartName) return $this->_chartCollection[$index];
		}
		return false;
	}
	...
	public function calculateColumnWidths($calculateMergeCells = false) {
		...
		foreach ($this->getColumnDimensions() as $colDimension) {
			if ($colDimension->getAutoSize()) $autoSizes[$colDimension->getColumnIndex()] = -1;
		}
		if (!empty($autoSizes)) {
			$isMergeCell = array();
			foreach ($this->getMergeCells() as $cells) {
				foreach (PHPExcel_Cell::extractAllCellReferencesInRange($cells) as $cellReference) $isMergeCell[$cellReference] = true;
			}
	...
	public function rebindParent(PHPExcel $parent) {
		...
		foreach ($namedRanges as $namedRange) $parent->addNamedRange($namedRange);
	...
	public function setTitle($pValue = 'Worksheet', $updateFormulaCellReferences = true) {
		if ($this->getTitle() == $pValue) return $this;
		self::_checkSheetTitle($pValue);
		$oldTitle = $this->getTitle();
		if ($this->getParent()) {
			if ($this->getParent()->sheetNameExists($pValue)) {
				if (PHPExcel_Shared_String::CountCharacters($pValue) > 29) $pValue = PHPExcel_Shared_String::Substring($pValue,0,29);
				$i = 1;
				while ($this->getParent()->sheetNameExists($pValue . ' ' . $i)) {
					++$i;
					if ($i == 10) {
						if (PHPExcel_Shared_String::CountCharacters($pValue) > 28) $pValue = PHPExcel_Shared_String::Substring($pValue,0,28);
					} elseif ($i == 100) {
						if (PHPExcel_Shared_String::CountCharacters($pValue) > 27) $pValue = PHPExcel_Shared_String::Substring($pValue,0,27);
					}
				}
				$altTitle = $pValue . ' ' . $i;
	...
	public function getCell($pCoordinate = 'A1') {
		if ($this->_cellCollection->isDataSet($pCoordinate)) return $this->_cellCollection->getCacheData($pCoordinate);
	...
	public function getConditionalStyles($pCoordinate = 'A1') {
		if (!isset($this->_conditionalStylesCollection[$pCoordinate])) $this->_conditionalStylesCollection[$pCoordinate] = array();
		return $this->_conditionalStylesCollection[$pCoordinate];
	}

	public function conditionalStylesExists($pCoordinate = 'A1') {
		if (isset($this->_conditionalStylesCollection[$pCoordinate])) return true;
		return false;
	}
	...
	public function duplicateConditionalStyle(array $pCellStyle = null, $pRange = '') {
		foreach($pCellStyle as $cellStyle) {
			if (!is_a($cellStyle,'PHPExcel_Style_Conditional')) throw new Exception('Style is not a conditional style');
		}
		$pRange = strtoupper($pRange);
		$rangeA	= '';
		$rangeB	= '';
	...
	public function setBreak($pCell = 'A1', $pBreak = PHPExcel_Worksheet::BREAK_NONE) {
		...
		if ($pCell != '') $this->_breaks[$pCell] = $pBreak;
		else throw new Exception('No cell coordinate specified.');
		return $this;
	}
	...
	public function mergeCells($pRange = 'A1:A1') {
		...
		if (strpos($pRange,':') !== false) {
			...
			if (!$this->cellExists($upperLeft)) $this->getCell($upperLeft)->setValueExplicit(null, PHPExcel_Cell_DataType::TYPE_NULL);
			$count = count($aReferences);
			for ($i = 1; $i < $count; $i++) $this->getCell($aReferences[$i])->setValueExplicit(null, PHPExcel_Cell_DataType::TYPE_NULL);
		} else {
	...
	public function unmergeCells($pRange = 'A1:A1') {
		$pRange = strtoupper($pRange);
		if (strpos($pRange,':') !== false) {
			if (isset($this->_mergeCells[$pRange])) unset($this->_mergeCells[$pRange]);
			else throw new Exception('Cell range ' . $pRange . ' not known as merged.');
		} else {
	...
	public function protectCells($pRange = 'A1', $pPassword = '', $pAlreadyHashed = false) {
		$pRange = strtoupper($pRange);
		if (!$pAlreadyHashed) $pPassword = PHPExcel_Shared_PasswordHasher::hashPassword($pPassword);
	...
	public function unprotectCells($pRange = 'A1') {
		$pRange = strtoupper($pRange);
		if (isset($this->_protectedCells[$pRange])) unset($this->_protectedCells[$pRange]);
		else throw new Exception('Cell range ' . $pRange . ' not known as protected.');
		return $this;
	}
	...
	public function setAutoFilter($pValue) {
		if (is_string($pValue)) $this->_autoFilter->setRange($pValue);
		elseif(is_object($pValue) && ($pValue instanceof PHPExcel_Worksheet_AutoFilter)) $this->_autoFilter = $pValue;
		return $this;
	}
	...
	public function freezePane($pCell = '') {
		$pCell = strtoupper($pCell);
		if (strpos($pCell,':') === false && strpos($pCell,',') === false) $this->_freezePane = $pCell;
		else throw new Exception('Freeze pane can not be set on a range of cells.');
		return $this;
	}
	...
	public function insertNewColumnBeforeByIndex($pBefore = 0, $pNumCols = 1) {
		if ($pBefore >= 0) return $this->insertNewColumnBefore(PHPExcel_Cell::stringFromColumnIndex($pBefore), $pNumCols);
		else throw new Exception("Columns can only be inserted before at least column A (0).");
	}
	...
	public function removeColumnByIndex($pColumn = 0, $pNumCols = 1) {
		if ($pColumn >= 0) return $this->removeColumn(PHPExcel_Cell::stringFromColumnIndex($pColumn), $pNumCols);
		else throw new Exception("Columns to be deleted should at least start from column 0");
	}
	...
	public function fromArray($source = null, $nullValue = null, $startCell = 'A1', $strictNullComparison = false) {
		if (is_array($source)) {
			if (!is_array(end($source))) $source = array($source);
			list ($startColumn, $startRow) = PHPExcel_Cell::coordinateFromString($startCell);
			foreach ($source as $rowData) {
				$currentColumn = $startColumn;
				foreach($rowData as $cellValue) {
					if ($strictNullComparison) {
						if ($cellValue !== $nullValue) $this->getCell($currentColumn . $startRow)->setValue($cellValue);
					} else {
						if ($cellValue != $nullValue) $this->getCell($currentColumn . $startRow)->setValue($cellValue);
					}
					++$currentColumn;
				...
				if ($this->_cellCollection->isDataSet($col.$row)) {
					...
					if ($cell->getValue() !== null) {
						if ($cell->getValue() instanceof PHPExcel_RichText) {
							...
							if ($calculateFormulas) $returnValue[$rRef][$cRef] = $cell->getCalculatedValue();
	...
	public function garbageCollect() {
		...
		if ($highestColumn < 0) $this->_cachedHighestColumn = 'A';
		else $this->_cachedHighestColumn = PHPExcel_Cell::stringFromColumnIndex(--$highestColumn);
	...
	public static function extractSheetTitle($pRange, $returnRange = false) {
		if (($sep = strpos($pRange, '!')) === false) return '';
		if ($returnRange) return array( trim(substr($pRange, 0, $sep),"'"), substr($pRange, $sep + 1));
		return substr($pRange, $sep + 1);
	}

	public function getHyperlink($pCellCoordinate = 'A1') {
		if (isset($this->_hyperlinkCollection[$pCellCoordinate])) return $this->_hyperlinkCollection[$pCellCoordinate];
	...
	public function setHyperlink($pCellCoordinate = 'A1', PHPExcel_Cell_Hyperlink $pHyperlink = null) {
		if ($pHyperlink === null) unset($this->_hyperlinkCollection[$pCellCoordinate]);
		else $this->_hyperlinkCollection[$pCellCoordinate] = $pHyperlink;
		return $this;
	}
	...
	public function getDataValidation($pCellCoordinate = 'A1') {
		if (isset($this->_dataValidationCollection[$pCellCoordinate])) return $this->_dataValidationCollection[$pCellCoordinate];
	...
	public function setDataValidation($pCellCoordinate = 'A1', PHPExcel_Cell_DataValidation $pDataValidation = null) {
		if ($pDataValidation === null) unset($this->_dataValidationCollection[$pCellCoordinate]);
