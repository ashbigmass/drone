...
class PHPExcel_ReferenceHelper
{
	...
	public static function getInstance() {
		if (!isset(self::$_instance) || (self::$_instance === NULL)) self::$_instance = new PHPExcel_ReferenceHelper();
	...
	public function insertNewBefore($pBefore = 'A1', $pNumCols = 0, $pNumRows = 0, PHPExcel_Worksheet $pSheet = null) {
				...
				if ($pSheet->cellExists($coordinate)) {
					...
					for ($j = PHPExcel_Cell::columnIndexFromString($beforeColumn) - 1; $j <= PHPExcel_Cell::columnIndexFromString($beforeColumn) - 2 + $pNumCols; ++$j) {
						...
						if ($conditionalStyles) {
							$cloned = array();
							foreach ($conditionalStyles as $conditionalStyle) $cloned[] = clone $conditionalStyle;
		...
		if (!empty($aColumnDimensions)) {
			foreach ($aColumnDimensions as $objColumnDimension) {
				...
				if ($objColumnDimension->getColumnIndex() != $newReference) $objColumnDimension->setColumnIndex($newReference);
			}
		...
		if (!empty($aRowDimensions)) {
			foreach ($aRowDimensions as $objRowDimension) {
				...
				list(, $newReference) = PHPExcel_Cell::coordinateFromString($newReference);
				if ($objRowDimension->getRowIndex() != $newReference) $objRowDimension->setRowIndex($newReference);
			}
	...
	private function _updateSingleCellReference($pCellReference = 'A1', $pBefore = 'A1', $pNumCols = 0, $pNumRows = 0) {
		if (strpos($pCellReference, ':') === false && strpos($pCellReference, ',') === false) {
			...
			if ($updateRow) $newRow	= $newRow + $pNumRows;
