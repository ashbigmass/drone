...
class PHPExcel_Style_Fill implements PHPExcel_IComparable
{
	...
	public function __construct($isSupervisor = false, $isConditional = false) {
		...
		if ($isConditional) $this->_fillType = NULL;
	...
	public function applyFromArray($pStyles = null) {
		if (is_array($pStyles)) {
			if ($this->_isSupervisor) {
				...
				if (array_key_exists('type', $pStyles)) $this->setFillType($pStyles['type']);
				if (array_key_exists('rotation', $pStyles)) $this->setRotation($pStyles['rotation']);
				if (array_key_exists('startcolor', $pStyles)) $this->getStartColor()->applyFromArray($pStyles['startcolor']);
				if (array_key_exists('endcolor', $pStyles)) $this->getEndColor()->applyFromArray($pStyles['endcolor']);
				if (array_key_exists('color', $pStyles)) $this->getStartColor()->applyFromArray($pStyles['color']);
			}
		} else {
	...
	public function getFillType() {
		if ($this->_isSupervisor) return $this->getSharedComponent()->getFillType();
	...
	public function getRotation() {
		if ($this->_isSupervisor) return $this->getSharedComponent()->getRotation();
