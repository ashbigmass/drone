...
class PHPExcel_Style_Borders implements PHPExcel_IComparable
{
	...
	public function applyFromArray($pStyles = null) {
		if (is_array($pStyles)) {
			if ($this->_isSupervisor) {
				...
				if (array_key_exists('left', $pStyles)) $this->getLeft()->applyFromArray($pStyles['left']);
				if (array_key_exists('right', $pStyles)) $this->getRight()->applyFromArray($pStyles['right']);
				if (array_key_exists('top', $pStyles)) $this->getTop()->applyFromArray($pStyles['top']);
				if (array_key_exists('bottom', $pStyles)) $this->getBottom()->applyFromArray($pStyles['bottom']);
				if (array_key_exists('diagonal', $pStyles)) $this->getDiagonal()->applyFromArray($pStyles['diagonal']);
				if (array_key_exists('diagonaldirection', $pStyles)) $this->setDiagonalDirection($pStyles['diagonaldirection']);
	...
	public function getAllBorders() {
		if (!$this->_isSupervisor) throw new Exception('Can only get pseudo-border for supervisor.');
	...
	public function getDiagonalDirection() {
		if ($this->_isSupervisor) return $this->getSharedComponent()->getDiagonalDirection();
		return $this->_diagonalDirection;
	}

	public function setDiagonalDirection($pValue = PHPExcel_Style_Borders::DIAGONAL_NONE) {
		if ($pValue == '') $pValue = PHPExcel_Style_Borders::DIAGONAL_NONE;
		if ($this->_isSupervisor) {
