...
class PHPExcel_Style_Font implements PHPExcel_IComparable
{
	...
	public function __construct($isSupervisor = false, $isConditional = false) {
		...
		if ($isSupervisor) $this->_color->bindParent($this, '_color');
	}
	...
	public function applyFromArray($pStyles = null) {
		if (is_array($pStyles)) {
			if ($this->_isSupervisor) {
				...
				if (array_key_exists('name', $pStyles)) $this->setName($pStyles['name']);
				if (array_key_exists('bold', $pStyles)) $this->setBold($pStyles['bold']);
				if (array_key_exists('italic', $pStyles)) $this->setItalic($pStyles['italic']);
				if (array_key_exists('superScript', $pStyles)) $this->setSuperScript($pStyles['superScript']);
				if (array_key_exists('subScript', $pStyles)) $this->setSubScript($pStyles['subScript']);
				if (array_key_exists('underline', $pStyles)) $this->setUnderline($pStyles['underline']);
				if (array_key_exists('strike', $pStyles)) $this->setStrikethrough($pStyles['strike']);
				if (array_key_exists('color', $pStyles)) $this->getColor()->applyFromArray($pStyles['color']);
				if (array_key_exists('size', $pStyles)) $this->setSize($pStyles['size']);
			}
		} else {
	...
	public function getName() {
		if ($this->_isSupervisor) return $this->getSharedComponent()->getName();
		return $this->_name;
	}

	public function setName($pValue = 'Calibri') {
  		if ($pValue == '') $pValue = 'Calibri';
		if ($this->_isSupervisor) {
	...
	public function getSize() {
		if ($this->_isSupervisor) return $this->getSharedComponent()->getSize();
		return $this->_size;
	}
	...
	public function getBold() {
		if ($this->_isSupervisor) return $this->getSharedComponent()->getBold();
		return $this->_bold;
	}
	...
	public function getItalic() {
		if ($this->_isSupervisor) return $this->getSharedComponent()->getItalic();
		return $this->_italic;
	}
	...
	public function getSuperScript() {
		if ($this->_isSupervisor) return $this->getSharedComponent()->getSuperScript();
	...
	public function getSubScript() {
		if ($this->_isSupervisor) return $this->getSharedComponent()->getSubScript();
	...
	public function getUnderline() {
		if ($this->_isSupervisor) return $this->getSharedComponent()->getUnderline();
		return $this->_underline;
	}

	public function setUnderline($pValue = PHPExcel_Style_Font::UNDERLINE_NONE) {
		if ($pValue == '') $pValue = PHPExcel_Style_Font::UNDERLINE_NONE;
		if ($this->_isSupervisor) {
	...
	public function getStrikethrough() {
		if ($this->_isSupervisor) return $this->getSharedComponent()->getStrikethrough();
