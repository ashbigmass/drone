...
class PHPExcel_Style_NumberFormat implements PHPExcel_IComparable
{
	...
	public function __construct($isSupervisor = false, $isConditional = false) {
		...
		if ($isConditional) $this->_formatCode = NULL;
	}
	...
	public function applyFromArray($pStyles = null) {
		if (is_array($pStyles)) {
			if ($this->_isSupervisor) {
				...
				if (array_key_exists('code', $pStyles)) $this->setFormatCode($pStyles['code']);
			}
		} else {
	...
	public function getFormatCode() {
		if ($this->_isSupervisor) return $this->getSharedComponent()->getFormatCode();
		if ($this->_builtInFormatCode !== false) return self::builtInFormatCode($this->_builtInFormatCode);
		return $this->_formatCode;
	}

	public function setFormatCode($pValue = PHPExcel_Style_NumberFormat::FORMAT_GENERAL) {
		if ($pValue == '') $pValue = PHPExcel_Style_NumberFormat::FORMAT_GENERAL;
		if ($this->_isSupervisor) {
	...
	public function getBuiltInFormatCode() {
		if ($this->_isSupervisor) return $this->getSharedComponent()->getBuiltInFormatCode();
	...
	public static function builtInFormatCode($pIndex) {
		...
		if (isset(self::$_builtInFormats[$pIndex])) return self::$_builtInFormats[$pIndex];
		return '';
	}

	public static function builtInFormatCodeIndex($formatCode) {
		self::fillBuiltInFormatCodes();
		if (isset(self::$_flippedBuiltInFormats[$formatCode])) return self::$_flippedBuiltInFormats[$formatCode];
		return false;
	}
	...
	public static function toFormattedString($value = '', $format = '', $callBack = null) {
		...
		if (preg_match('/^(\[\$[A-Z]*-[0-9A-F]*\])*[hmsdy]/i', $format)) {
			...
			if ($format === self::FORMAT_PERCENTAGE) {
				...
				if (preg_match('/^[#0]+/', $format, $m)) $format = str_replace($m[0], strlen($m[0]), $format);
			...
			if ($format === self::FORMAT_CURRENCY_EUR_SIMPLE) {
				...
				if (preg_match('/\[\$(.*)\]/u', $format, $m)) {
					...
					if ($currencyCode == '') $currencyCode = PHPExcel_Shared_String::getCurrencyCode();
