...
class PHPExcel_Calculation_Financial
{
	...
	private static function _coupFirstPeriodDate($settlement, $maturity, $frequency, $next) {
		...
		while ($settlement < PHPExcel_Shared_Date::PHPToExcel($result)) $result->modify('-'.$months.' months');
		if ($next) $result->modify('+'.$months.' months');
		if ($eom) $result->modify('-1 day');
		return PHPExcel_Shared_Date::PHPToExcel($result);
	}

	private static function _validFrequency($frequency) {
		if (($frequency == 1) || ($frequency == 2) || ($frequency == 4)) return true;
	...
	public static function ACCRINT($issue, $firstinterest, $settlement, $rate, $par=1000, $frequency=1, $basis=0) {
		...
		if ((is_numeric($rate)) && (is_numeric($par))) {
			$rate	= (float) $rate;
			$par	= (float) $par;
			if (($rate <= 0) || ($par <= 0)) return PHPExcel_Calculation_Functions::NaN();
			$daysBetweenIssueAndSettlement = PHPExcel_Calculation_DateTime::YEARFRAC($issue, $settlement, $basis);
			if (!is_numeric($daysBetweenIssueAndSettlement)) return $daysBetweenIssueAndSettlement;
	...
	public static function AMORDEGRC($cost, $purchased, $firstPeriod, $salvage, $period, $rate, $basis=0) {
		...
		$fUsePer = 1.0 / $rate;
		if ($fUsePer < 3.0) $amortiseCoeff = 1.0;
		elseif ($fUsePer < 5.0) $amortiseCoeff = 1.5;
		elseif ($fUsePer <= 6.0) $amortiseCoeff = 2.0;
		else $amortiseCoeff = 2.5;
		$rate *= $amortiseCoeff;
	...
	public static function AMORLINC($cost, $purchased, $firstPeriod, $salvage, $period, $rate, $basis=0) {
		...
		if (($basis == 1) && ($yearFrac < 1) && (PHPExcel_Calculation_DateTime::_isLeapYear($purchasedYear))) $yearFrac *= 365 / 366;
		$f0Rate = $yearFrac * $rate * $cost;
		$nNumOfFullPeriods = intval(($cost - $salvage - $f0Rate) / $fOneRate);
		if ($period == 0) return $f0Rate;
		elseif ($period <= $nNumOfFullPeriods) return $fOneRate;
		elseif ($period == ($nNumOfFullPeriods + 1)) return ($fCostDelta - $fOneRate * $nNumOfFullPeriods - $f0Rate);
		else return 0.0;
	}
	...
	public static function COUPDAYBS($settlement, $maturity, $frequency, $basis=0) {
		...
		$basis		= (is_null($basis))	? 0 :	(int) PHPExcel_Calculation_Functions::flattenSingleValue($basis);
		if (is_string($settlement = PHPExcel_Calculation_DateTime::_getDateValue($settlement))) return PHPExcel_Calculation_Functions::VALUE();
		if (is_string($maturity = PHPExcel_Calculation_DateTime::_getDateValue($maturity))) return PHPExcel_Calculation_Functions::VALUE();
		if (($settlement > $maturity) || (!self::_validFrequency($frequency)) || (($basis < 0) || ($basis > 4))) return PHPExcel_Calculation_Functions::NaN();
	...
	public static function CUMIPMT($rate, $nper, $pv, $start, $end, $type = 0) {
		...
		if ($type != 0 && $type != 1) return PHPExcel_Calculation_Functions::NaN();
		if ($start < 1 || $start > $end) return PHPExcel_Calculation_Functions::VALUE();
		$interest = 0;
		for ($per = $start; $per <= $end; ++$per) $interest += self::IPMT($rate, $per, $nper, $pv, 0, $type);
		return $interest;
	}
	...
	public static function CUMPRINC($rate, $nper, $pv, $start, $end, $type = 0) {
		...
		if ($start < 1 || $start > $end) return PHPExcel_Calculation_Functions::VALUE();
		$principal = 0;
		for ($per = $start; $per <= $end; ++$per) $principal += self::PPMT($rate, $per, $nper, $pv, 0, $type);
		return $principal;
	}
	...
	public static function DB($cost, $salvage, $life, $period, $month=12) {
		...
		if ((is_numeric($cost)) && (is_numeric($salvage)) && (is_numeric($life)) && (is_numeric($period)) && (is_numeric($month))) {
			...
			elseif (($cost < 0) || (($salvage / $cost) < 0) || ($life <= 0) || ($period < 1) || ($month < 1)) return PHPExcel_Calculation_Functions::NaN();
			$fixedDepreciationRate = 1 - pow(($salvage / $cost), (1 / $life));
			$fixedDepreciationRate = round($fixedDepreciationRate, 3);
			$previousDepreciation = 0;
			for ($per = 1; $per <= $period; ++$per) {
				if ($per == 1) $depreciation = $cost * $fixedDepreciationRate * $month / 12;
				elseif ($per == ($life + 1)) $depreciation = ($cost - $previousDepreciation) * $fixedDepreciationRate * (12 - $month) / 12;
				else $depreciation = ($cost - $previousDepreciation) * $fixedDepreciationRate;
				$previousDepreciation += $depreciation;
			}
			if (PHPExcel_Calculation_Functions::getCompatibilityMode() == PHPExcel_Calculation_Functions::COMPATIBILITY_GNUMERIC) $depreciation = round($depreciation,2);
			return $depreciation;
		}
		return PHPExcel_Calculation_Functions::VALUE();
	}
	...
	public static function DDB($cost, $salvage, $life, $period, $factor=2.0) {
		...
		if ((is_numeric($cost)) && (is_numeric($salvage)) && (is_numeric($life)) && (is_numeric($period)) && (is_numeric($factor))) {
			...
			$period		= (int) $period;
			$factor		= (float) $factor;
			if (($cost <= 0) || (($salvage / $cost) < 0) || ($life <= 0) || ($period < 1) || ($factor <= 0.0) || ($period > $life)) return PHPExcel_Calculation_Functions::NaN();
			$fixedDepreciationRate = 1 - pow(($salvage / $cost), (1 / $life));
			$fixedDepreciationRate = round($fixedDepreciationRate, 3);
			$previousDepreciation = 0;
			for ($per = 1; $per <= $period; ++$per) {
	...
	public static function DISC($settlement, $maturity, $price, $redemption, $basis=0) {
		...
		if ((is_numeric($price)) && (is_numeric($redemption)) && (is_numeric($basis))) {
			...
			$basis		= (int) $basis;
			if (($price <= 0) || ($redemption <= 0)) return PHPExcel_Calculation_Functions::NaN();
			$daysBetweenSettlementAndMaturity = PHPExcel_Calculation_DateTime::YEARFRAC($settlement, $maturity, $basis);
			if (!is_numeric($daysBetweenSettlementAndMaturity)) return $daysBetweenSettlementAndMaturity;
	...
	public static function DOLLARDE($fractional_dollar = Null, $fraction = 0) {
		...
		if (is_null($fractional_dollar) || $fraction < 0) return PHPExcel_Calculation_Functions::NaN();
		if ($fraction == 0) return PHPExcel_Calculation_Functions::DIV0();
	...
	public static function DOLLARFR($decimal_dollar = Null, $fraction = 0) {
		...
		$fraction		= (int)PHPExcel_Calculation_Functions::flattenSingleValue($fraction);
		if (is_null($decimal_dollar) || $fraction < 0) return PHPExcel_Calculation_Functions::NaN();
		if ($fraction == 0) return PHPExcel_Calculation_Functions::DIV0();
	...
	public static function EFFECT($nominal_rate = 0, $npery = 0) {
		...
		if ($nominal_rate <= 0 || $npery < 1) return PHPExcel_Calculation_Functions::NaN();
	...
	public static function FV($rate = 0, $nper = 0, $pmt = 0, $pv = 0, $type = 0) {
		...
		if ($type != 0 && $type != 1) return PHPExcel_Calculation_Functions::NaN();
		if (!is_null($rate) && $rate != 0) return -$pv * pow(1 + $rate, $nper) - $pmt * (1 + $rate * $type) * (pow(1 + $rate, $nper) - 1) / $rate;
		else return -$pv - $pmt * $nper;
	}
	...
	public static function FVSCHEDULE($principal, $schedule) {
		...
		foreach($schedule as $n) $principal *= 1 + $n;
		return $principal;
	}
	...
	public static function INTRATE($settlement, $maturity, $investment, $redemption, $basis=0) {
		...
		if ((is_numeric($investment)) && (is_numeric($redemption)) && (is_numeric($basis))) {
			...
			$redemption	= (float) $redemption;
			$basis		= (int) $basis;
			if (($investment <= 0) || ($redemption <= 0)) return PHPExcel_Calculation_Functions::NaN();
			$daysBetweenSettlementAndMaturity = PHPExcel_Calculation_DateTime::YEARFRAC($settlement, $maturity, $basis);
			if (!is_numeric($daysBetweenSettlementAndMaturity)) return $daysBetweenSettlementAndMaturity;
	...
	public static function IPMT($rate, $per, $nper, $pv, $fv = 0, $type = 0) {
		...
		$type	= (int) PHPExcel_Calculation_Functions::flattenSingleValue($type);
		if ($type != 0 && $type != 1) return PHPExcel_Calculation_Functions::NaN();
		if ($per <= 0 || $per > $nper) return PHPExcel_Calculation_Functions::VALUE();
	...
	public static function IRR($values, $guess = 0.1) {
		...
		for ($i = 0; $i < FINANCIAL_MAX_ITERATIONS; ++$i) {
			if (($f1 * $f2) < 0.0) break;
			if (abs($f1) < abs($f2)) $f1 = self::NPV($x1 += 1.6 * ($x1 - $x2), $values);
			else $f2 = self::NPV($x2 += 1.6 * ($x2 - $x1), $values);
		}
	...
	public static function ISPMT() {
		...
		for($i=0; $i <= $period; ++$i) {
			...
			if($i == $numberPeriods) $returnValue = 0;
		}
		return($returnValue);
	}
	...
	public static function MIRR($values, $finance_rate, $reinvestment_rate) {
		...
		foreach($values as $i => $v) {
			if ($v >= 0) $npv_pos += $v / pow($rr, $i);
			else $npv_neg += $v / pow($fr, $i);
		}
		if (($npv_neg == 0) || ($npv_pos == 0) || ($reinvestment_rate <= -1)) return PHPExcel_Calculation_Functions::VALUE();
	...
	public static function NOMINAL($effect_rate = 0, $npery = 0) {
		...
		$npery			= (int)PHPExcel_Calculation_Functions::flattenSingleValue($npery);
		if ($effect_rate <= 0 || $npery < 1) return PHPExcel_Calculation_Functions::NaN();
	...
	public static function NPER($rate = 0, $pmt = 0, $pv = 0, $fv = 0, $type = 0) {
		...
		if (!is_null($rate) && $rate != 0) {
			if ($pmt == 0 && $pv == 0) return PHPExcel_Calculation_Functions::NaN();
			return log(($pmt * (1 + $rate * $type) / $rate - $fv) / ($pv + $pmt * (1 + $rate * $type) / $rate)) / log(1 + $rate);
		} else {
			if ($pmt == 0) return PHPExcel_Calculation_Functions::NaN();
			return (-$pv -$fv) / $pmt;
		}
	}
	...
	public static function NPV() {
		...
		for ($i = 1; $i <= count($aArgs); ++$i) {
			if (is_numeric($aArgs[$i - 1])) $returnValue += $aArgs[$i - 1] / pow(1 + $rate, $i);
		}
		return $returnValue;
	}
	...
	public static function PMT($rate = 0, $nper = 0, $pv = 0, $fv = 0, $type = 0) {
		...
		$type	= PHPExcel_Calculation_Functions::flattenSingleValue($type);
		if ($type != 0 && $type != 1) return PHPExcel_Calculation_Functions::NaN();
		if (!is_null($rate) && $rate != 0) return (-$fv - $pv * pow(1 + $rate, $nper)) / (1 + $rate * $type) / ((pow(1 + $rate, $nper) - 1) / $rate);
		else return (-$pv - $fv) / $nper;
	}
	...
	public static function PRICE($settlement, $maturity, $rate, $yield, $redemption, $frequency, $basis=0) {
		...
		for($k = 0; $k <= $n; ++$k) $result += $rfp / (pow($baseYF, ($k + $de)));
		$result -= $rfp * ($a / $e);
		return $result;
	}
	...
	public static function PRICEDISC($settlement, $maturity, $discount, $redemption, $basis=0) {
		...
		if ((is_numeric($discount)) && (is_numeric($redemption)) && (is_numeric($basis))) {
			if (($discount <= 0) || ($redemption <= 0)) return PHPExcel_Calculation_Functions::NaN();
			$daysBetweenSettlementAndMaturity = PHPExcel_Calculation_DateTime::YEARFRAC($settlement, $maturity, $basis);
			if (!is_numeric($daysBetweenSettlementAndMaturity)) return $daysBetweenSettlementAndMaturity;
	...
	public static function PRICEMAT($settlement, $maturity, $issue, $rate, $yield, $basis=0) {
		...
		if (is_numeric($rate) && is_numeric($yield)) {
			if (($rate <= 0) || ($yield <= 0)) return PHPExcel_Calculation_Functions::NaN();
			$daysPerYear = self::_daysPerYear(PHPExcel_Calculation_DateTime::YEAR($settlement),$basis);
			if (!is_numeric($daysPerYear)) return $daysPerYear;
			$daysBetweenIssueAndSettlement = PHPExcel_Calculation_DateTime::YEARFRAC($issue, $settlement, $basis);
			if (!is_numeric($daysBetweenIssueAndSettlement)) return $daysBetweenIssueAndSettlement;
			$daysBetweenIssueAndSettlement *= $daysPerYear;
			$daysBetweenIssueAndMaturity = PHPExcel_Calculation_DateTime::YEARFRAC($issue, $maturity, $basis);
			if (!is_numeric($daysBetweenIssueAndMaturity)) return $daysBetweenIssueAndMaturity;
	...
	public static function PV($rate = 0, $nper = 0, $pmt = 0, $fv = 0, $type = 0) {
		...
		$type	= PHPExcel_Calculation_Functions::flattenSingleValue($type);
		if ($type != 0 && $type != 1) return PHPExcel_Calculation_Functions::NaN();
		if (!is_null($rate) && $rate != 0) return (-$pmt * (1 + $rate * $type) * ((pow(1 + $rate, $nper) - 1) / $rate) - $fv) / pow(1 + $rate, $nper);
		else return -$fv - $pmt * $nper;
	}
	...
	public static function RECEIVED($settlement, $maturity, $investment, $discount, $basis=0) {
		...
		if ((is_numeric($investment)) && (is_numeric($discount)) && (is_numeric($basis))) {
			if (($investment <= 0) || ($discount <= 0)) return PHPExcel_Calculation_Functions::NaN();
			$daysBetweenSettlementAndMaturity = PHPExcel_Calculation_DateTime::YEARFRAC($settlement, $maturity, $basis);
			if (!is_numeric($daysBetweenSettlementAndMaturity)) return $daysBetweenSettlementAndMaturity;
	...
	public static function SLN($cost, $salvage, $life) {
		...
		if ((is_numeric($cost)) && (is_numeric($salvage)) && (is_numeric($life))) {
			if ($life < 0) return PHPExcel_Calculation_Functions::NaN();
	...
	public static function SYD($cost, $salvage, $life, $period) {
		...
		if ((is_numeric($cost)) && (is_numeric($salvage)) && (is_numeric($life)) && (is_numeric($period))) {
			if (($life < 1) || ($period > $life)) return PHPExcel_Calculation_Functions::NaN();
	...
	public static function TBILLEQ($settlement, $maturity, $discount) {
		...
		if (is_string($testValue)) return $testValue;
		if (is_string($maturity = PHPExcel_Calculation_DateTime::_getDateValue($maturity))) return PHPExcel_Calculation_Functions::VALUE();
	...
	public static function TBILLPRICE($settlement, $maturity, $discount) {
		...
		if (is_numeric($discount)) {
			if ($discount <= 0) return PHPExcel_Calculation_Functions::NaN();
			if (PHPExcel_Calculation_Functions::getCompatibilityMode() == PHPExcel_Calculation_Functions::COMPATIBILITY_OPENOFFICE) {
				++$maturity;
				$daysBetweenSettlementAndMaturity = PHPExcel_Calculation_DateTime::YEARFRAC($settlement, $maturity) * 360;
				if (!is_numeric($daysBetweenSettlementAndMaturity)) return $daysBetweenSettlementAndMaturity;
			...
			if ($daysBetweenSettlementAndMaturity > 360) return PHPExcel_Calculation_Functions::NaN();
			$price = 100 * (1 - (($discount * $daysBetweenSettlementAndMaturity) / 360));
			if ($price <= 0) return PHPExcel_Calculation_Functions::NaN();
			return $price;
		}
		return PHPExcel_Calculation_Functions::VALUE();
	}
	...
	public static function XIRR($values, $dates, $guess = 0.1) {
		...
		for ($i = 0; $i < FINANCIAL_MAX_ITERATIONS; ++$i) {
			if (($f1 * $f2) < 0.0) break;
			if (abs($f1) < abs($f2)) $f1 = self::XNPV($x1 += 1.6 * ($x1 - $x2), $values, $dates);
			else $f2 = self::XNPV($x2 += 1.6 * ($x2 - $x1), $values, $dates);
		}
		if (($f1 * $f2) > 0.0) return PHPExcel_Calculation_Functions::VALUE();
	...
	public static function YIELDMAT($settlement, $maturity, $issue, $rate, $price, $basis=0) {
		...
		if (is_numeric($rate) && is_numeric($price)) {
			if (($rate <= 0) || ($price <= 0)) return PHPExcel_Calculation_Functions::NaN();
			$daysPerYear = self::_daysPerYear(PHPExcel_Calculation_DateTime::YEAR($settlement),$basis);
			if (!is_numeric($daysPerYear)) return $daysPerYear;
			$daysBetweenIssueAndSettlement = PHPExcel_Calculation_DateTime::YEARFRAC($issue, $settlement, $basis);
			if (!is_numeric($daysBetweenIssueAndSettlement)) return $daysBetweenIssueAndSettlement;
