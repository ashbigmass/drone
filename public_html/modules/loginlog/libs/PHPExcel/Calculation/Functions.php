...
class PHPExcel_Calculation_Functions
{
	...
	public static function IS_BLANK($value = NULL) {
		if (!is_null($value)) $value	= self::flattenSingleValue($value);
	...
	public static function N($value = NULL) {
		while (is_array($value)) $value = array_shift($value);
	...
	public static function TYPE($value = NULL) {
		...
		if (is_array($value) && (count($value) > 1)) {
			$a = array_keys($value);
			$a = array_pop($a);
			if (self::isCellValue($a)) return 16;
		...
		foreach ($array as $value) {
			if (is_array($value)) {
				foreach ($value as $val) {
					if (is_array($val)) {
						foreach ($val as $v) $arrayValues[] = $v;
					} else {
						$arrayValues[] = $val;
					}
				}
		...
		foreach ($array as $k1 => $value) {
			if (is_array($value)) {
				foreach ($value as $k2 => $val) {
					if (is_array($val)) {
						foreach ($val as $k3 => $v) $arrayValues[$k1.'.'.$k2.'.'.$k3] = $v;
					} else {
	...
	public static function flattenSingleValue($value = '') {
		while (is_array($value)) $value = array_pop($value);
		return $value;
	}
}
...
if (!function_exists('money_format')) {
	function money_format($format, $number) {
		...
		if ($left > 0 && $left > $n) {
			if ($flags['isleft']) $number[0] .= str_repeat($flags['fillchar'], $left - $n);
			else $number[0] = str_repeat($flags['fillchar'], $left - $n) . $number[0];
		}
		$number = implode($locale['mon_decimal_point'], $number);
		if ($locale["{$letter}_cs_precedes"]) $number = $prefix . $currency . $space . $number . $suffix;
		else $number = $prefix . $number . $space . $currency . $suffix;
		if ($width > 0) $number = str_pad($number, $width, $flags['fillchar'], $flags['isleft'] ? STR_PAD_RIGHT : STR_PAD_LEFT);
...
if ((!function_exists('mb_str_replace')) &&
	...
	function mb_str_replace($search, $replace, $subject) {
		if(is_array($subject)) {
			$ret = array();
			foreach($subject as $key => $val) $ret[$key] = mb_str_replace($search, $replace, $val);
			return $ret;
		}
