...
class PHPExcel_Calculation_Engineering
{
	...
	public static function _parseComplex($complexNumber) {
		...
		if (!is_numeric($suffix)) $workString = substr($workString,0,-1);
		else $suffix = '';
		$leadingSign = 0;
		if (strlen($workString) > 0) $leadingSign = (($workString{0} == '+') || ($workString{0} == '-')) ? 1 : 0;
		$power = '';
	...
	private static function _nbrConversionFormat($xVal,$places) {
		if (!is_null($places)) {
			if (strlen($xVal) <= $places) return substr(str_pad($xVal,$places,'0',STR_PAD_LEFT),-10);
	...
	public static function BESSELI($x, $ord) {
		...
		if ((is_numeric($x)) && (is_numeric($ord))) {
			$ord	= floor($ord);
			if ($ord < 0) return PHPExcel_Calculation_Functions::NaN();
			if (abs($x) <= 30) {
	...
	public static function BESSELK($x, $ord) {
		...
		$ord	= (is_null($ord))	? 0.0 :	PHPExcel_Calculation_Functions::flattenSingleValue($ord);
		if ((is_numeric($x)) && (is_numeric($ord))) {
			if (($ord < 0) || ($x == 0.0)) return PHPExcel_Calculation_Functions::NaN();
			switch(floor($ord)) {
	...
	public static function BINTODEC($x) {
		...
		if (PHPExcel_Calculation_Functions::getCompatibilityMode() == PHPExcel_Calculation_Functions::COMPATIBILITY_GNUMERIC) $x = floor($x);
		$x = (string) $x;
		if (strlen($x) > preg_match_all('/[01]/',$x,$out)) return PHPExcel_Calculation_Functions::NaN();
		if (strlen($x) > 10) {
	...
	public static function BINTOOCT($x, $places=NULL) {
		...
		$x = (string) $x;
		if (strlen($x) > preg_match_all('/[01]/',$x,$out)) return PHPExcel_Calculation_Functions::NaN();
		if (strlen($x) > 10) return PHPExcel_Calculation_Functions::NaN();
		elseif (strlen($x) == 10) return str_repeat('7',7).substr(strtoupper(decoct(bindec(substr($x,-9)))),-3);
	...
	public static function DECTOBIN($x, $places=NULL) {
		...
		$x = (string) $x;
		if (strlen($x) > preg_match_all('/[-0123456789.]/',$x,$out)) return PHPExcel_Calculation_Functions::VALUE();
		$x = (string) floor($x);
		$r = decbin($x);
		if (strlen($r) == 32) $r = substr($r,-10);
		elseif (strlen($r) > 11) return PHPExcel_Calculation_Functions::NaN();
	...
	public static function HEXTOBIN($x, $places=null) {
		$x	= PHPExcel_Calculation_Functions::flattenSingleValue($x);
		$places	= PHPExcel_Calculation_Functions::flattenSingleValue($places);
		if (is_bool($x)) return PHPExcel_Calculation_Functions::VALUE();
		$x = (string) $x;
		if (strlen($x) > preg_match_all('/[0123456789ABCDEF]/',strtoupper($x),$out)) return PHPExcel_Calculation_Functions::NaN();
	...
	public static function OCTTOBIN($x, $places=null) {
		...
		if (is_bool($x)) return PHPExcel_Calculation_Functions::VALUE();
		$x = (string) $x;
		if (preg_match_all('/[01234567]/',$x,$out) != strlen($x)) return PHPExcel_Calculation_Functions::NaN();
		$r = decbin(octdec($x));
		return self::_nbrConversionFormat($r,$places);
	}
	...
	public static function COMPLEX($realNumber=0.0, $imaginary=0.0, $suffix='i') {
		...
		if (((is_numeric($realNumber)) && (is_numeric($imaginary))) &&
			...
			if ($suffix == '') $suffix = 'i';
			if ($realNumber == 0.0) {
				if ($imaginary == 0.0) return (string) '0';
				elseif ($imaginary == 1.0) return (string) $suffix;
				elseif ($imaginary == -1.0) return (string) '-'.$suffix;
	...
	public static function IMARGUMENT($complexNumber) {
		...
		if ($parsedComplex['real'] == 0.0) {
			if ($parsedComplex['imaginary'] == 0.0) return 0.0;
			elseif($parsedComplex['imaginary'] < 0.0) return M_PI / -2;
			else return M_PI / 2;
	...
	public static function IMSQRT($complexNumber) {
		...
		if ($parsedComplex['suffix'] == '') return self::COMPLEX($d1 * $r,$d2 * $r);
		else return self::COMPLEX($d1 * $r,$d2 * $r,$parsedComplex['suffix']);
	}

	public static function IMLN($complexNumber) {
		$complexNumber	= PHPExcel_Calculation_Functions::flattenSingleValue($complexNumber);
		$parsedComplex = self::_parseComplex($complexNumber);
		if (($parsedComplex['real'] == 0.0) && ($parsedComplex['imaginary'] == 0.0)) return PHPExcel_Calculation_Functions::NaN();
		$logR = log(sqrt(($parsedComplex['real'] * $parsedComplex['real']) + ($parsedComplex['imaginary'] * $parsedComplex['imaginary'])));
		$t = self::IMARGUMENT($complexNumber);
		if ($parsedComplex['suffix'] == '') return self::COMPLEX($logR,$t);
		else return self::COMPLEX($logR,$t,$parsedComplex['suffix']);
	}
	...
	public static function IMLOG10($complexNumber) {
		...
		$parsedComplex = self::_parseComplex($complexNumber);
		if (($parsedComplex['real'] == 0.0) && ($parsedComplex['imaginary'] == 0.0)) return PHPExcel_Calculation_Functions::NaN();
		elseif (($parsedComplex['real'] > 0.0) && ($parsedComplex['imaginary'] == 0.0)) return log10($parsedComplex['real']);
	...
	public static function IMLOG2($complexNumber) {
		...
		$parsedComplex = self::_parseComplex($complexNumber);
		if (($parsedComplex['real'] == 0.0) && ($parsedComplex['imaginary'] == 0.0)) return PHPExcel_Calculation_Functions::NaN();
		elseif (($parsedComplex['real'] > 0.0) && ($parsedComplex['imaginary'] == 0.0)) return log($parsedComplex['real'],2);
	...
	public static function IMEXP($complexNumber) {
		$complexNumber	= PHPExcel_Calculation_Functions::flattenSingleValue($complexNumber);
		$parsedComplex = self::_parseComplex($complexNumber);
		if (($parsedComplex['real'] == 0.0) && ($parsedComplex['imaginary'] == 0.0)) return '1';
		$e = exp($parsedComplex['real']);
		$eX = $e * cos($parsedComplex['imaginary']);
		$eY = $e * sin($parsedComplex['imaginary']);
		if ($parsedComplex['suffix'] == '') return self::COMPLEX($eX,$eY);
		else return self::COMPLEX($eX,$eY,$parsedComplex['suffix']);
	}

	public static function IMPOWER($complexNumber,$realNumber) {
		$complexNumber	= PHPExcel_Calculation_Functions::flattenSingleValue($complexNumber);
		$realNumber		= PHPExcel_Calculation_Functions::flattenSingleValue($realNumber);
		if (!is_numeric($realNumber)) return PHPExcel_Calculation_Functions::VALUE();
		$parsedComplex = self::_parseComplex($complexNumber);
		$r = sqrt(($parsedComplex['real'] * $parsedComplex['real']) + ($parsedComplex['imaginary'] * $parsedComplex['imaginary']));
		$rPower = pow($r,$realNumber);
		$theta = self::IMARGUMENT($complexNumber) * $realNumber;
		if ($theta == 0) return 1;
		elseif ($parsedComplex['imaginary'] == 0.0) return self::COMPLEX($rPower * cos($theta),$rPower * sin($theta),$parsedComplex['suffix']);
		else return self::COMPLEX($rPower * cos($theta),$rPower * sin($theta),$parsedComplex['suffix']);
	}
	...
	public static function IMDIV($complexDividend,$complexDivisor) {
		...
		$r = $d1/$d3;
		$i = $d2/$d3;
		if ($i > 0.0) return self::_cleanComplex($r.'+'.$i.$parsedComplexDivisor['suffix']);
		elseif ($i < 0.0) return self::_cleanComplex($r.$i.$parsedComplexDivisor['suffix']);
		else return $r;
	}
	...
	public static function IMSUM() {
		...
		foreach ($aArgs as $arg) {
			...
			if ($activeSuffix == '') $activeSuffix = $parsedComplex['suffix'];
			elseif (($parsedComplex['suffix'] != '') && ($activeSuffix != $parsedComplex['suffix'])) return PHPExcel_Calculation_Functions::VALUE();
	...
	public static function _erfVal($x) {
		if (abs($x) > 2.2) return 1 - self::_erfcVal($x);
		$sum = $term = $x;
		$xsqr = ($x * $x);
		$j = 1;
		do {
	...
	public static function ERF($lower, $upper = NULL) {
		...
		if (is_numeric($lower)) {
			if (is_null($upper)) return self::_erfVal($lower);
			if (is_numeric($upper)) return self::_erfVal($upper) - self::_erfVal($lower);
		}
		return PHPExcel_Calculation_Functions::VALUE();
	}
	...
	private static function _erfcVal($x) {
		if (abs($x) < 2.2) return 1 - self::_erfVal($x);
	...
	public static function ERFC($x) {
		$x = PHPExcel_Calculation_Functions::flattenSingleValue($x);
		if (is_numeric($x)) return self::_erfcVal($x);
		return PHPExcel_Calculation_Functions::VALUE();
	}

	public static function getConversionGroups() {
		$conversionGroups = array();
		foreach(self::$_conversionUnits as $conversionUnit) $conversionGroups[] = $conversionUnit['Group'];
	...
	public static function CONVERTUOM($value, $fromUOM, $toUOM) {
		...
		if (isset(self::$_conversionUnits[$fromUOM])) {
			...
			if (isset(self::$_conversionMultipliers[$fromMultiplier])) $fromMultiplier = self::$_conversionMultipliers[$fromMultiplier]['multiplier'];
			else return PHPExcel_Calculation_Functions::NA();
			if ((isset(self::$_conversionUnits[$fromUOM])) && (self::$_conversionUnits[$fromUOM]['AllowPrefix'])) $unitGroup1 = self::$_conversionUnits[$fromUOM]['Group'];
			else return PHPExcel_Calculation_Functions::NA();
		}
		...
		if ($unitGroup1 != $unitGroup2) return PHPExcel_Calculation_Functions::NA();
		if (($fromUOM == $toUOM) && ($fromMultiplier == $toMultiplier)) {
			...
			if (($fromUOM == 'F') || ($fromUOM == 'fah')) {
				if (($toUOM == 'F') || ($toUOM == 'fah')) {
					...
					$value = (($value - 32) / 1.8);
					if (($toUOM == 'K') || ($toUOM == 'kel')) $value += 273.15;
					return $value;
				}
			...
			if (($toUOM == 'F') || ($toUOM == 'fah')) {
				if (($fromUOM == 'K') || ($fromUOM == 'kel')) $value -= 273.15;
				return ($value * 1.8) + 32;
			}
			if (($toUOM == 'C') || ($toUOM == 'cel')) return $value - 273.15;
			return $value + 273.15;
		}
