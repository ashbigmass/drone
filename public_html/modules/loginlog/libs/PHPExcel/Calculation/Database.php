...
class PHPExcel_Calculation_Database
{
	...
	private static function __filter($database,$criteria) {
		...
		if ($testConditionsCount > 1) $testConditionSet = 'AND('.implode(',',$testConditions).')';
		elseif($testConditionsCount == 1) $testConditionSet = $testConditions[0];
		foreach($database as $dataRow => $dataValues) {
			...
			if (!$result) unset($database[$dataRow]);
		}
		return $database;
	}
	...
	public static function DAVERAGE($database,$field,$criteria) {
		...
		$colData = array();
		foreach($database as $row) $colData[] = $row[$field];
	...
	public static function DGET($database,$field,$criteria) {
		...
		$colData = array();
		foreach($database as $row) $colData[] = $row[$field];
		if (count($colData) > 1) return PHPExcel_Calculation_Functions::NaN();
		return $colData[0];
	}
