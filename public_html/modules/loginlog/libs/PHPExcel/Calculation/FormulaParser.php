...
class PHPExcel_Calculation_FormulaParser {
	...
	public function __construct($pFormula = '') {
		if (is_null($pFormula)) throw new Exception("Invalid parameter passed: formula");
	...
	public function getToken($pId = 0) {
		if (isset($this->_tokens[$pId])) return $this->_tokens[$pId];
		else throw new Exception("Token with id $pId does not exist.");
	}
	...
	private function _parseToTokens() {
		...
		for ($i = 0; $i < $tokenCount; ++$i) {
			$token = $tokens1[$i];
			if (isset($tokens1[$i - 1])) $previousToken = $tokens1[$i - 1];
			else $previousToken = null;
			if (isset($tokens1[$i + 1])) $nextToken = $tokens1[$i + 1];
			else $nextToken = null;
			...
			if (is_null($previousToken)) continue;
			if (! (
		...
		for ($i = 0; $i < $tokenCount; ++$i) {
			$token = $tokens2[$i];
			if (isset($tokens2[$i - 1])) $previousToken = $tokens2[$i - 1];
			else $previousToken = null;
			if (isset($tokens2[$i + 1])) $nextToken = $tokens2[$i + 1];
			else $nextToken = null;
			if (is_null($token)) continue;
