...
class PHPExcel_Shared_OLE_PPS_Root extends PHPExcel_Shared_OLE_PPS
{
	...
	public function save($filename) {
		...
		if (is_resource($filename)) {
			...
			if ($this->_FILEH_ == false) throw new Exception("Can't create temporary file.");
		} else {
			$this->_FILEH_ = fopen($filename, "wb");
		}
		if ($this->_FILEH_ == false) throw new Exception("Can't open $filename. It may be in use or protected.");
		$aList = array();
		...
		if (!is_resource($filename)) fclose($this->_FILEH_);
		return true;
	}
	...
	public function _saveHeader($iSBDcnt, $iBBcnt, $iPPScnt) {
		...
		if ($iBdCnt > $i1stBdL) {
			while (1) {
				...
				if ($iBdCnt <= ($iBdExL*$iBlCnt+ $i1stBdL)) break;
			}
		}
		fwrite($FILE,
		...
		for ($i = 0; $i < $i1stBdL && $i < $iBdCnt; ++$i) fwrite($FILE, pack("V", $iAll+$i));
		if ($i < $i1stBdL) {
			$jB = $i1stBdL - $i;
			for ($j = 0; $j < $jB; ++$j) fwrite($FILE, (pack("V", -1)));
		}
	}
	...
	public function _makeSmallData(&$raList) {
		...
		if ($iSmBlk % $iSbCnt) {
			...
			for ($i = 0; $i < $iB; ++$i) fwrite($FILE, pack("V", -1));
		}
		return $sRes;
	}
	...
	public function _savePps(&$raList) {
		$iC = count($raList);
		for ($i = 0; $i < $iC; ++$i) fwrite($this->_FILEH_, $raList[$i]->_getPpsWk());
		$iCnt = count($raList);
		$iBCnt = $this->_BIG_BLOCK_SIZE / PHPExcel_Shared_OLE::OLE_PPS_SIZE;
		if ($iCnt % $iBCnt) fwrite($this->_FILEH_, str_repeat("\x00",($iBCnt - ($iCnt % $iBCnt)) * PHPExcel_Shared_OLE::OLE_PPS_SIZE));
	}
	...
	public function _saveBbd($iSbdSize, $iBsize, $iPpsCnt) {
		...
		if ($iBdCnt >$i1stBdL) {
			while (1) {
				...
				if ($iBdCnt <= ($iBdExL*$iBbCnt+ $i1stBdL)) break;
			}
		}
		if ($iSbdSize > 0) {
			for ($i = 0; $i < ($iSbdSize - 1); ++$i) fwrite($FILE, pack("V", $i+1));
			fwrite($FILE, pack("V", -2));
		}
		for ($i = 0; $i < ($iBsize - 1); ++$i) fwrite($FILE, pack("V", $i+$iSbdSize+1));
		fwrite($FILE, pack("V", -2));
		for ($i = 0; $i < ($iPpsCnt - 1); ++$i) fwrite($FILE, pack("V", $i+$iSbdSize+$iBsize+1));
		fwrite($FILE, pack("V", -2));
		for ($i = 0; $i < $iBdCnt; ++$i) fwrite($FILE, pack("V", 0xFFFFFFFD));
		for ($i = 0; $i < $iBdExL; ++$i) fwrite($FILE, pack("V", 0xFFFFFFFC));
		if (($iAllW + $iBdCnt) % $iBbCnt) {
			$iBlock = ($iBbCnt - (($iAllW + $iBdCnt) % $iBbCnt));
			for ($i = 0; $i < $iBlock; ++$i) fwrite($FILE, pack("V", -1));
		}
		if ($iBdCnt > $i1stBdL) {
			$iN=0;
			$iNb=0;
