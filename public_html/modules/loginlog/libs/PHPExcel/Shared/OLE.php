...
class PHPExcel_Shared_OLE
{
	...
	public function read($file) {
		$fh = fopen($file, "r");
		if (!$fh) throw new Exception("Can't open file $file");
		$this->_file_handle = $fh;
		$signature = fread($fh, 8);
		if ("\xD0\xCF\x11\xE0\xA1\xB1\x1A\xE1" != $signature) throw new Exception("File doesn't seem to be an OLE container.");
		fseek($fh, 28);
		if (fread($fh, 2) != "\xFE\xFF") throw new Exception("Only Little-Endian encoding is supported.");
		...
		$this->bbat = array();
		$mbatBlocks = array();
		for ($i = 0; $i < 109; ++$i) $mbatBlocks[] = self::_readInt4($fh);
		$pos = $this->_getBlockOffset($mbatFirstBlockId);
		for ($i = 0; $i < $mbbatBlockCount; ++$i) {
			fseek($fh, $pos);
			for ($j = 0; $j < $this->bigBlockSize / 4 - 1; ++$j) $mbatBlocks[] = self::_readInt4($fh);
			$pos = $this->_getBlockOffset(self::_readInt4($fh));
		}
		for ($i = 0; $i < $bbatBlockCount; ++$i) {
			$pos = $this->_getBlockOffset($mbatBlocks[$i]);
			fseek($fh, $pos);
			for ($j = 0 ; $j < $this->bigBlockSize / 4; ++$j) $this->bbat[] = self::_readInt4($fh);
		}
		$this->sbat = array();
		$shortBlockCount = $sbbatBlockCount * $this->bigBlockSize / 4;
		$sbatFh = $this->getStream($sbatFirstBlockId);
		for ($blockId = 0; $blockId < $shortBlockCount; ++$blockId) $this->sbat[$blockId] = self::_readInt4($sbatFh);
		fclose($sbatFh);
	...
	public function isFile($index) {
		if (isset($this->_list[$index])) return ($this->_list[$index]->Type == self::OLE_PPS_TYPE_FILE);
		return false;
	}

	public function isRoot($index) {
		if (isset($this->_list[$index])) return ($this->_list[$index]->Type == self::OLE_PPS_TYPE_ROOT);
		return false;
	}
	...
	public function getData($index, $position, $length) {
		if (!isset($this->_list[$index]) || ($position >= $this->_list[$index]->Size) || ($position < 0)) return '';
	...
	public function getDataLength($index) {
		if (isset($this->_list[$index])) return $this->_list[$index]->Size;
		return 0;
	}

	public static function Asc2Ucs($ascii) {
		$rawname = '';
		for ($i = 0; $i < strlen($ascii); ++$i) $rawname .= $ascii{$i} . "\x00";
		return $rawname;
	}

	public static function LocalDate2OLE($date = null) {
		if (!isset($date)) return "\x00\x00\x00\x00\x00\x00\x00\x00";
		$factor = pow(2, 32);
		$days = 134774;
	...
	public static function OLE2LocalDate($string) {
		if (strlen($string) != 8) return new PEAR_Error("Expecting 8 byte string");
		$factor = pow(2,32);
