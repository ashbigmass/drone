...
class PHPExcel_Shared_Font
{
	...
	public static function setAutoSizeMethod($pValue = self::AUTOSIZE_METHOD_APPROX) {
		if (!in_array($pValue,self::$_autoSizeMethods)) return FALSE;
	...
	public static function calculateColumnWidth(PHPExcel_Style_Font $font, $cellText = '', $rotation = 0, PHPExcel_Style_Font $defaultFont = null) {
		if ($cellText instanceof PHPExcel_RichText) $cellText = $cellText->getPlainText();
		if (strpos($cellText, "\n") !== false) {
			$lineTexts = explode("\n", $cellText);
			$lineWitdhs = array();
			foreach ($lineTexts as $lineText) $lineWidths[] = self::calculateColumnWidth($font, $lineText, $rotation = 0, $defaultFont);
			return max($lineWidths);
		}
		try {
			if (self::$autoSizeMethod == self::AUTOSIZE_METHOD_APPROX) throw new Exception('AutoSize method is set to approx');
	...
	public static function getTextWidthPixelsExact($text, PHPExcel_Style_Font $font, $rotation = 0) {
		if (!function_exists('imagettfbbox')) throw new Exception('GD library needs to be enabled');
	...
	public static function getTextWidthPixelsApprox($columnText, PHPExcel_Style_Font $font = null, $rotation = 0) {
		...
		if ($rotation !== 0) {
			if ($rotation == -165) $columnWidth = 4;
			else $columnWidth = $columnWidth * cos(deg2rad($rotation)) + $fontSize * abs(sin(deg2rad($rotation))) / 5;
		}
	...
	public static function getTrueTypeFontFileFromFont($font) {
		if (!file_exists(self::$trueTypeFontPath) || !is_dir(self::$trueTypeFontPath)) throw new Exception('Valid directory to TrueType Font files not specified');
		$name		= $font->getName();
		$bold		= $font->getBold();
		...
		if (!file_exists($fontFile)) throw New Exception('TrueType Font file not found');
		return $fontFile;
	}
	...
	public static function getDefaultColumnWidthByFont(PHPExcel_Style_Font $font, $pPixels = false) {
		if (isset(self::$defaultColumnWidths[$font->getName()][$font->getSize()])) {
			...
			if ($pPixels) $columnWidth = (int) round($columnWidth);
		}
		return $columnWidth;
	}
