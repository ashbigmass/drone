...
class PHPExcel_Shared_JAMA_Matrix
{
	...
	public function __construct() {
		if (func_num_args() > 0) {
			...
			switch($match) {
				...
				case 'array,integer':
						$this->m = $args[1];
						if ($this->m != 0) $this->n = count($args[0]) / $this->m;
						else $this->n = 0;
						if (($this->m * $this->n) == count($args[0])) {
							for($i = 0; $i < $this->m; ++$i) {
								for($j = 0; $j < $this->n; ++$j) $this->A[$i][$j] = $args[0][$i + $j * $this->m];
							}
						} else {
		...
		if (func_num_args() > 0) {
			...
			switch($match) {
				case 'integer,integer':
						...
						for($i = $i0; $i < $this->m; ++$i) for($j = $j0; $j < $this->n; ++$j) $R->set($i, $j, $this->A[$i][$j]);
						return $R;
						break;
				case 'integer,integer,integer,integer':
						...
						for($i = $i0; $i <= $iF; ++$i) {
							for($j = $j0; $j <= $jF; ++$j) $R->set($i - $i0, $j - $j0, $this->A[$i][$j]);
						}
						return $R;
						break;
				case 'array,array':
						...
						$R = new PHPExcel_Shared_JAMA_Matrix($m, $n);
						for($i = 0; $i < $m; ++$i) {
							for($j = 0; $j < $n; ++$j) $R->set($i - $i0, $j - $j0, $this->A[$RL[$i]][$CL[$j]]);
						}
						return $R;
						break;
				case 'array,array':
						...
						$R = new PHPExcel_Shared_JAMA_Matrix($m, $n);
						for($i = 0; $i < $m; ++$i) {
							for($j = 0; $j < $n; ++$j) $R->set($i, $j, $this->A[$RL[$i]][$CL[$j]]);
						}
						return $R;
						break;
				case 'integer,integer,array':
						...
						$R = new PHPExcel_Shared_JAMA_Matrix($m, $n);
						for($i = $i0; $i < $iF; ++$i) {
							for($j = 0; $j < $n; ++$j) $R->set($i - $i0, $j, $this->A[$RL[$i]][$j]);
						}
						return $R;
						break;
				case 'array,integer,integer':
						...
						for($i = 0; $i < $m; ++$i) {
							for($j = $j0; $j <= $jF; ++$j) $R->set($i, $j - $j0, $this->A[$RL[$i]][$j]);
						}
						return $R;
						break;
				default:
	...
	public function checkMatrixDimensions($B = null) {
		if ($B instanceof PHPExcel_Shared_JAMA_Matrix) {
			if (($this->m == $B->getRowDimension()) && ($this->n == $B->getColumnDimension())) return true;
			else throw new Exception(self::MatrixDimensionException);
		} else {
	...
	public function diagonal($m = null, $n = null, $c = 1) {
		$R = new PHPExcel_Shared_JAMA_Matrix($m, $n);
		for($i = 0; $i < $m; ++$i) $R->set($i, $i, $c);
		return $R;
	}

	public function getMatrixByRow($i0 = null, $iF = null) {
		if (is_int($i0)) {
			if (is_int($iF)) return $this->getMatrix($i0, 0, $iF + 1, $this->n);
			else return $this->getMatrix($i0, 0, $i0 + 1, $this->n);
		} else {
			throw new Exception(self::ArgumentTypeException);
		}
	}
	...
	public function getMatrixByCol($j0 = null, $jF = null) {
		if (is_int($j0)) {
			if (is_int($jF)) return $this->getMatrix(0, $j0, $this->m, $jF + 1);
			else return $this->getMatrix(0, $j0, $this->m, $j0 + 1);
		} else {
			throw new Exception(self::ArgumentTypeException);
		}
	}
	...
	public function transpose() {
		...
		for($i = 0; $i < $this->m; ++$i) {
			for($j = 0; $j < $this->n; ++$j) $R->set($j, $i, $this->A[$i][$j]);
		}
		return $R;
	}
	...
	public function plus() {
		if (func_num_args() > 0) {
			...
			for($i = 0; $i < $this->m; ++$i) {
				for($j = 0; $j < $this->n; ++$j) $M->set($i, $j, $M->get($i, $j) + $this->A[$i][$j]);
			}
			return $M;
		} else {
			throw new Exception(self::PolymorphicArgumentException);
		...
		if (func_num_args() > 0) {
			...
			for($i = 0; $i < $this->m; ++$i) {
				for($j = 0; $j < $this->n; ++$j) {
					...
					if ($validValues) $this->A[$i][$j] += $value;
					else $this->A[$i][$j] = PHPExcel_Calculation_Functions::NaN();
				}
			}
			return $this;
		} else {
		...
		if (func_num_args() > 0) {
			...
			for($i = 0; $i < $this->m; ++$i) {
				for($j = 0; $j < $this->n; ++$j) {
					...
					if ($validValues) {
						if ($value == 0) $M->set($i, $j, '#DIV/0!');
						else $M->set($i, $j, $this->A[$i][$j] / $value);
					} else {
	...
	public function arrayRightDivideEquals() {
		if (func_num_args() > 0) {
			...
			$this->checkMatrixDimensions($M);
			for($i = 0; $i < $this->m; ++$i) {
				for($j = 0; $j < $this->n; ++$j) $this->A[$i][$j] = $this->A[$i][$j] / $M->get($i, $j);
			}
			return $M;
		} else {
			throw new Exception(self::PolymorphicArgumentException);
	...
	public function arrayLeftDivideEquals() {
		if (func_num_args() > 0) {
			...
			$this->checkMatrixDimensions($M);
			for($i = 0; $i < $this->m; ++$i) {
				for($j = 0; $j < $this->n; ++$j) $this->A[$i][$j] = $M->get($i, $j) / $this->A[$i][$j];
			}
			return $M;
		} else {
			throw new Exception(self::PolymorphicArgumentException);
			...
			switch($match) {
				case 'object':
						...
						if ($this->n == $B->m) {
							...
							for($j = 0; $j < $B->n; ++$j) {
								for ($k = 0; $k < $this->n; ++$k) $Bcolj[$k] = $B->A[$k][$j];
								for($i = 0; $i < $this->m; ++$i) {
									$Arowi = $this->A[$i];
									$s = 0;
									for($k = 0; $k < $this->n; ++$k) $s += $Arowi[$k] * $Bcolj[$k];
									$C->A[$i][$j] = $s;
								}
							}
							return $C;
				...
				case 'array':
						...
						if ($this->n == $B->m) {
							...
							for($i = 0; $i < $C->m; ++$i) {
								for($j = 0; $j < $C->n; ++$j) {
									$s = "0";
									for($k = 0; $k < $C->n; ++$k) $s += $this->A[$i][$k] * $B->A[$k][$j];
									$C->A[$i][$j] = $s;
								}
							}
							return $C;
				...
				case 'integer':
						...
						for($i = 0; $i < $C->m; ++$i) {
							for($j = 0; $j < $C->n; ++$j) $C->A[$i][$j] *= $args[0];
						}
						return $C;
						break;
				case 'double':
						...
						for($i = 0; $i < $C->m; ++$i) {
							for($j = 0; $j < $C->n; ++$j) $C->A[$i][$j] = $args[0] * $this->A[$i][$j];
						}
						return $C;
						break;
				case 'float':
		...
		if (func_num_args() > 0) {
			...
			for($i = 0; $i < $this->m; ++$i) {
				for($j = 0; $j < $this->n; ++$j) {
					...
					if ($validValues) $this->A[$i][$j] = pow($this->A[$i][$j],$value);
					else $this->A[$i][$j] = PHPExcel_Calculation_Functions::NaN();
				}
			}
			return $this;
	...
	public function concat() {
		if (func_num_args() > 0) {
			...
			$this->checkMatrixDimensions($M);
			for($i = 0; $i < $this->m; ++$i) {
				for($j = 0; $j < $this->n; ++$j) $this->A[$i][$j] = trim($this->A[$i][$j],'"').trim($M->get($i, $j),'"');
			}
			return $this;
		} else {
			throw new Exception(self::PolymorphicArgumentException);
