...
class MagicSquareExample
{
	function magic($n) {
		if (($n % 2) == 1) {
			...
			$b = ($n+1);
			for ($j = 0; $j < $n; ++$j)
				for ($i = 0; $i < $n; ++$i) $M[$i][$j] = $n*(($i+$j+$a) % $n) + (($i+2*$j+$b) % $n) + 1;
		} else if (($n % 4) == 0) {
			for ($j = 0; $j < $n; ++$j) {
				for ($i = 0; $i < $n; ++$i) {
					if ((($i+1)/2)%2 == (($j+1)/2)%2) $M[$i][$j] = $n*$n-$n*$i-$j;
					else $M[$i][$j] = $n*$i+$j+1;
				}
			}
		} else {
...
<?php
		...
		for ($n = 3; $n <= 6; ++$n) {
			...
			$c = $M->cond();
			if ($c < 1/$eps) echo "<td align='right'>".sprintf("%.3f",$c)."</td>";
			else echo "<td align='right'>Inf</td>";
