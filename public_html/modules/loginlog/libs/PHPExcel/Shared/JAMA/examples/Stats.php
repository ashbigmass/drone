...
class Base {
	...
	function setData($arr, $opt=STATS_DATA_SIMPLE) {
		if (!is_array($arr)) return PEAR::raiseError('invalid data, an array of numeric data was expected');
		$this->_data = null;
	...
	function getData($expanded=false) {
		if ($this->_data == null) return PEAR::raiseError('data has not been set');
		if ($this->_dataOption == STATS_DATA_CUMMULATIVE && $expanded) return $this->_dataExpanded;
	...
	function studentize() {
		$mean = $this->mean();
		if (PEAR::isError($mean)) return $mean;
		$std = $this->stDev();
		if (PEAR::isError($std)) return $std;
		if ($std == 0) return PEAR::raiseError('cannot studentize data, standard deviation is zero.');
		$arr  = array();
	...
	function calc($mode, $returnErrorObject=true) {
		if ($this->_data == null) return PEAR::raiseError('data has not been set');
		if ($mode == STATS_BASIC) return $this->calcBasic($returnErrorObject);
		elseif ($mode == STATS_FULL) return $this->calcFull($returnErrorObject);
		else return PEAR::raiseError('incorrect mode, expected STATS_BASIC or STATS_FULL');
	}
	...
	function min() {
		...
		if (!array_key_exists('min', $this->_calculatedValues)) {
			if ($this->_dataOption == STATS_DATA_CUMMULATIVE) $min = min(array_keys($this->_data));
			else $min = min($this->_data);
	...
	function max() {
		...
		if (!array_key_exists('max', $this->_calculatedValues)) {
			if ($this->_dataOption == STATS_DATA_CUMMULATIVE) $max = max(array_keys($this->_data));
			else $max = max($this->_data);
	...
	function sum() {
		if (!array_key_exists('sum', $this->_calculatedValues)) {
			$sum = $this->sumN(1);
			if (PEAR::isError($sum)) return $sum;
			else $this->_calculatedValues['sum'] = $sum;
		}
	...
	function sum2() {
		if (!array_key_exists('sum2', $this->_calculatedValues)) {
			$sum2 = $this->sumN(2);
			if (PEAR::isError($sum2)) return $sum2;
			else $this->_calculatedValues['sum2'] = $sum2;
		}
	...
	function sumN($n) {
		...
		$sumN = 0;
		if ($this->_dataOption == STATS_DATA_CUMMULATIVE) {
			foreach($this->_data as $val=>$freq) $sumN += $freq * pow((double)$val, (double)$n);
		} else {
			foreach($this->_data as $val) $sumN += pow((double)$val, (double)$n);
		}
		return $sumN;
	}
	...
	function product() {
		if (!array_key_exists('product', $this->_calculatedValues)) {
			...
			if (PEAR::isError($product)) return $product;
			else $this->_calculatedValues['product'] = $product;
		}
	...
	function count() {
		...
		if (!array_key_exists('count', $this->_calculatedValues)) {
			if ($this->_dataOption == STATS_DATA_CUMMULATIVE) $count = count($this->_dataExpanded);
			else $count = count($this->_data);
	...
	function mean() {
		if (!array_key_exists('mean', $this->_calculatedValues)) {
			...
			if (PEAR::isError($sum)) return $sum;
			$count = $this->count();
			if (PEAR::isError($count)) return $count;
	...
	function range() {
		if (!array_key_exists('range', $this->_calculatedValues)) {
			$min = $this->min();
			if (PEAR::isError($min)) return $min;
			$max = $this->max();
			if (PEAR::isError($max)) return $max;
	...
	function variance() {
		if (!array_key_exists('variance', $this->_calculatedValues)) {
			...
			if (PEAR::isError($variance)) return $variance;
	...
	function stDevWithMean($mean) {
		...
		if (PEAR::isError($varianceWM)) return $varianceWM;
	...
	function absDev() {
		if (!array_key_exists('absDev', $this->_calculatedValues)) {
			...
			if (PEAR::isError($absdev)) return $absdev;
	...
	function skewness() {
		if (!array_key_exists('skewness', $this->_calculatedValues)) {
			...
			if (PEAR::isError($count)) return $count;
			$stDev = $this->stDev();
			if (PEAR::isError($stDev)) return $stDev;
			$sumdiff3 = $this->__sumdiff(3);
			if (PEAR::isError($sumdiff3)) return $sumdiff3;
	...
	function kurtosis() {
		if (!array_key_exists('kurtosis', $this->_calculatedValues)) {
			...
			if (PEAR::isError($sumdiff4)) return $sumdiff4;
	...
	function median() {
		...
		if (!array_key_exists('median', $this->_calculatedValues)) {
			if ($this->_dataOption == STATS_DATA_CUMMULATIVE) $arr =& $this->_dataExpanded;
			else $arr =& $this->_data;
			$n = $this->count();
			if (PEAR::isError($n)) return $n;
			$h = intval($n / 2);
			if ($n % 2 == 0) $median = ($arr[$h] + $arr[$h - 1]) / 2;
			else $median = $arr[$h + 1];
	...
	function mode() {
		...
		if (!array_key_exists('mode', $this->_calculatedValues)) {
			if ($this->_dataOption == STATS_DATA_CUMMULATIVE) $arr = $this->_data;
			else $arr = $this->frequency();
			arsort($arr);
			$mcount = 1;
	...
	function geometricMean() {
		if (!array_key_exists('geometricMean', $this->_calculatedValues)) {
			...
			if (PEAR::isError($count)) return $count;
			$prod = $this->product();
			if (PEAR::isError($prod)) return $prod;
			if ($prod == 0.0) return 0.0;
			if ($prod < 0) return PEAR::raiseError('The product of the data set is negative, geometric mean undefined.');
	...
	function harmonicMean() {
		...
		if (!array_key_exists('harmonicMean', $this->_calculatedValues)) {
			...
			if ($this->_dataOption == STATS_DATA_CUMMULATIVE) {
				foreach($this->_data as $val=>$freq) {
					if ($val == 0) return PEAR::raiseError('cannot calculate a '.'harmonic mean with data values of zero.');
					$invsum += $freq / $val;
				}
			} else {
				foreach($this->_data as $val) {
	...
	function sampleCentralMoment($n) {
		if (!is_int($n) || $n < 1) return PEAR::isError('moment must be a positive integer >= 1.');
		if ($n == 1) return 0;
		$count = $this->count();
		if (PEAR::isError($count)) return $count;
		if ($count == 0) return PEAR::raiseError("Cannot calculate {$n}th sample moment, ".'there are zero data entries');
	...
	function sampleRawMoment($n) {
		...
		$count = $this->count();
		if (PEAR::isError($count)) return $count;
		if ($count == 0) return PEAR::raiseError("Cannot calculate {$n}th raw moment, ".'there are zero data entries.');
		$sum = $this->sumN($n);
		if (PEAR::isError($sum)) return $sum;
		return ($sum / $count);
	}
	...
	function coeffOfVariation() {
		if (!array_key_exists('coeffOfVariation', $this->_calculatedValues)) {
			$mean = $this->mean();
			if (PEAR::isError($mean)) return $mean;
			if ($mean == 0.0) return PEAR::raiseError('cannot calculate the coefficient '.'of variation, mean of sample is zero');
			$stDev = $this->stDev();
			if (PEAR::isError($stDev)) return $stDev;
	...
	function frequency() {
		...
		if (!array_key_exists('frequency', $this->_calculatedValues)) {
			if ($this->_dataOption == STATS_DATA_CUMMULATIVE) {
				...
				$freq = array();
				foreach ($this->_data as $val) $freq["$val"]++;
				ksort($freq);
			}
	...
	function quartiles() {
		if (!array_key_exists('quartiles', $this->_calculatedValues)) {
			...
			if (PEAR::isError($q1)) return $q1;
			$q2 = $this->percentile(50);
			if (PEAR::isError($q2)) return $q2;
			$q3 = $this->percentile(75);
			if (PEAR::isError($q3)) return $q3;
	...
	function interquartileMean() {
		if (!array_key_exists('interquartileMean', $this->_calculatedValues)) {
			...
			if (PEAR::isError($quart)) return $quart;
			$q3 = $quart['75'];
			$q1 = $quart['25'];
			$sum = 0;
			$n = 0;
			...
			if ($n == 0) return PEAR::raiseError('error calculating interquartile mean, '.'empty interquartile range of values.');
	...
	function percentile($p) {
		$count = $this->count();
		if (PEAR::isError($count)) return $count;
		if ($this->_dataOption == STATS_DATA_CUMMULATIVE) $data =& $this->_dataExpanded;
	...
	function __sumdiff($power, $mean=null) {
		...
		$sdiff = 0;
		if ($this->_dataOption == STATS_DATA_CUMMULATIVE) {
			foreach ($this->_data as $val=>$freq) $sdiff += $freq * pow((double)($val - $mean), (double)$power);
		} else {
			foreach ($this->_data as $val) $sdiff += pow((double)($val - $mean), (double)$power);
		}
		return $sdiff;
	}
	...
	function __calcVariance($mean = null) {
		...
		if (PEAR::isError($sumdiff2)) return $sumdiff2;
		$count = $this->count();
		if (PEAR::isError($count)) return $count;
		if ($count == 1) return PEAR::raiseError('cannot calculate variance of a singe data point');
	...
	function __calcAbsoluteDeviation($mean = null) {
		...
		if (PEAR::isError($sumabsdev)) return $sumabsdev;
		return $sumabsdev / $count;
	}

	function __sumabsdev($mean=null) {
		if ($this->_data == null) return PEAR::raiseError('data has not been set');
		if (is_null($mean)) $mean = $this->mean();
		$sdev = 0;
		if ($this->_dataOption == STATS_DATA_CUMMULATIVE) {
			foreach ($this->_data as $val=>$freq) $sdev += $freq * abs($val - $mean);
		} else {
			foreach ($this->_data as $val) $sdev += abs($val - $mean);
		}
		return $sdev;
	}

	function __format($v, $useErrorObject=true) {
		if (PEAR::isError($v) && $useErrorObject == false) return $v->getMessage();
		else return $v;
	}
	...
	function _validate() {
		...
		if ($flag) {
			ksort($this->_data);
			$this->_dataExpanded = array();
			foreach ($this->_data as $val=>$freq) $this->_dataExpanded = array_pad($this->_dataExpanded, count($this->_dataExpanded) + $freq, $val);
