...
class PHPExcel_Shared_JAMA_LUDecomposition
{
	...
	public function __construct($A) {
		if ($A instanceof PHPExcel_Shared_JAMA_Matrix) {
			...
			for ($i = 0; $i < $this->m; ++$i) $this->piv[$i] = $i;
			$this->pivsign = 1;
			$LUrowi = $LUcolj = array();
			for ($j = 0; $j < $this->n; ++$j) {
				for ($i = 0; $i < $this->m; ++$i) $LUcolj[$i] = &$this->LU[$i][$j];
				for ($i = 0; $i < $this->m; ++$i) {
					$LUrowi = $this->LU[$i];
					$kmax = min($i,$j);
					$s = 0.0;
				...
				$p = $j;
				for ($i = $j+1; $i < $this->m; ++$i) if (abs($LUcolj[$i]) > abs($LUcolj[$p])) $p = $i;
				if ($p != $j) {
					for ($k = 0; $k < $this->n; ++$k) {
						$t = $this->LU[$p][$k];
				...
				if (($j < $this->m) && ($this->LU[$j][$j] != 0.0)) {
					for ($i = $j+1; $i < $this->m; ++$i) $this->LU[$i][$j] /= $this->LU[$j][$j];
				}
			}
		} else {
	...
	public function getL() {
		for ($i = 0; $i < $this->m; ++$i) {
			for ($j = 0; $j < $this->n; ++$j) {
				if ($i > $j) $L[$i][$j] = $this->LU[$i][$j];
				elseif ($i == $j) $L[$i][$j] = 1.0;
				else $L[$i][$j] = 0.0;
			}
		}
	...
	public function det() {
		if ($this->m == $this->n) {
			$d = $this->pivsign;
			for ($j = 0; $j < $this->n; ++$j) $d *= $this->LU[$j][$j];
			return $d;
		} else {
			...
			if ($this->isNonsingular()) {
				...
				for ($k = 0; $k < $this->n; ++$k) {
					for ($i = $k+1; $i < $this->n; ++$i) {
						for ($j = 0; $j < $nx; ++$j) $X->A[$i][$j] -= $X->A[$k][$j] * $this->LU[$i][$k];
					}
				}
				for ($k = $this->n-1; $k >= 0; --$k) {
					for ($j = 0; $j < $nx; ++$j) $X->A[$k][$j] /= $this->LU[$k][$k];
					for ($i = 0; $i < $k; ++$i) {
						for ($j = 0; $j < $nx; ++$j) $X->A[$i][$j] -= $X->A[$k][$j] * $this->LU[$i][$k];
					}
				}
