...
class PHPExcel_Shared_OLERead
{
	...
	public function read($sFileName) {
		if(!is_readable($sFileName)) throw new Exception("Could not open " . $sFileName . " for reading! File does not exist, or it is not readable.");
		$this->data = file_get_contents($sFileName);
		if (substr($this->data, 0, 8) != self::IDENTIFIER_OLE) throw new Exception('The filename ' . $sFileName . ' is not recognised as an OLE file');
		...
		if ($this->numExtensionBlocks != 0) $bbdBlocks = (self::BIG_BLOCK_SIZE - self::BIG_BLOCK_DEPOT_BLOCKS_POS)/4;
		...
		for ($j = 0; $j < $this->numExtensionBlocks; ++$j) {
			...
			if ($bbdBlocks < $this->numBigBlockDepotBlocks) $this->extensionBlock = self::_GetInt4d($this->data, $pos);
		}
		$pos = $index = 0;
	...
	public function getStream($stream) {
		...
		if ($this->props[$stream]['size'] < self::SMALL_BLOCK_THRESHOLD) {
			...
			if ($this->props[$stream]['size'] % self::BIG_BLOCK_SIZE != 0) ++$numBlocks;
	...
	private function _readPropertySets() {
		...
		while ($offset < $entryLen) {
			...
			if (($name == 'Workbook') || ($name == 'Book') || ($name == 'WORKBOOK') || ($name == 'BOOK')) $this->wrkbook = count($this->props) - 1;
			if ($name == 'Root Entry' || $name == 'ROOT ENTRY' || $name == 'R') $this->rootentry = count($this->props) - 1;
			if ($name == chr(5) . 'SummaryInformation') $this->summaryInformation = count($this->props) - 1;
			if ($name == chr(5) . 'DocumentSummaryInformation') $this->documentSummaryInformation = count($this->props) - 1;
