...
class PHPExcel_Shared_Excel5
{
	public static function sizeCol($sheet, $col = 'A') {
		...
		if (isset($columnDimensions[$col]) and !$columnDimensions[$col]->getVisible()) $effectivePixelWidth = 0;
		else $effectivePixelWidth = $pixelWidth;
	...
	public static function sizeRow($sheet, $row = 1) {
		...
		if ( isset($rowDimensions[$row]) and !$rowDimensions[$row]->getVisible() ) $effectivePixelRowHeight = 0;
		else $effectivePixelRowHeight = $pixelRowHeight;
	...
	public static function getDistanceX(PHPExcel_Worksheet $sheet, $startColumn = 'A', $startOffsetX = 0, $endColumn = 'A', $endOffsetX = 0) {
		...
		for ($i = $startColumnIndex; $i <= $endColumnIndex; ++$i) $distanceX += self::sizeCol($sheet, PHPExcel_Cell::stringFromColumnIndex($i));
	...
	public static function getDistanceY(PHPExcel_Worksheet $sheet, $startRow = 1, $startOffsetY = 0, $endRow = 1, $endOffsetY = 0) {
		$distanceY = 0;
		for ($row = $startRow; $row <= $endRow; ++$row) $distanceY += self::sizeRow($sheet, $row);
	...
	public static function oneAnchor2twoAnchor($sheet, $coordinates, $offsetX, $offsetY, $width, $height) {
		...
		$col_end	= $col_start;
		$row_end	= $row_start;
		if ($x1 >= self::sizeCol($sheet, PHPExcel_Cell::stringFromColumnIndex($col_start))) $x1 = 0;
		if ($y1 >= self::sizeRow($sheet, $row_start + 1)) $y1 = 0;
		$width	  = $width  + $x1 -1;
		$height	 = $height + $y1 -1;
		...
		if (self::sizeCol($sheet, PHPExcel_Cell::stringFromColumnIndex($col_start)) == 0) return;
		if (self::sizeCol($sheet, PHPExcel_Cell::stringFromColumnIndex($col_end))   == 0) return;
