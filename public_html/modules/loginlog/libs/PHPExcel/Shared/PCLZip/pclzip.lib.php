...
class PclZip
{
	...
	function create($p_filelist) {
		...
		if (is_array($p_filelist)) {
			if (isset($p_filelist[0]) && is_array($p_filelist[0])) $v_att_list = $p_filelist;
		...
		if (sizeof($v_string_list) != 0) {
			foreach ($v_string_list as $v_string) {
				if ($v_string != '') $v_att_list[][PCLZIP_ATT_FILE_NAME] = $v_string;
			}
		}
	...
	function add($p_filelist) {
		...
		if (sizeof($v_string_list) != 0) {
			foreach ($v_string_list as $v_string) $v_att_list[][PCLZIP_ATT_FILE_NAME] = $v_string;
		}
		$v_supported_attributes = array ( PCLZIP_ATT_FILE_NAME => 'mandatory'
			 ,PCLZIP_ATT_FILE_NEW_SHORT_NAME => 'optional'
			 ,PCLZIP_ATT_FILE_NEW_FULL_NAME => 'optional'
	...
	function extractByIndex($p_index) {
		...
		if ($v_size > 1) {
			...
			if ((is_integer($v_arg_list[0])) && ($v_arg_list[0] > 77000)) {
				...
				if (!isset($v_options[PCLZIP_OPT_EXTRACT_AS_STRING])) $v_options[PCLZIP_OPT_EXTRACT_AS_STRING] = FALSE;
			} else {
				$v_path = $v_arg_list[0];
				if ($v_size == 2) {
					$v_remove_path = $v_arg_list[1];
	...
	function errorCode() {
		if (PCLZIP_ERROR_EXTERNAL == 1) return(PclErrorCode());
		else return($this->error_code);
	}
	...
	function errorName($p_with_code=false) {
		...
		if (isset($v_name[$this->error_code])) $v_value = $v_name[$this->error_code];
		else $v_value = 'NoName';
	...
	function errorInfo($p_full=false) {
		if (PCLZIP_ERROR_EXTERNAL == 1) {
			...
			if ($p_full) return($this->errorName(true)." : ".$this->error_string);
			else return($this->error_string." [code ".$this->error_code."]");
		}
	}
				...
				if ($v_folder_handler = @opendir($v_descr['filename'])) {
					while (($v_item_handler = @readdir($v_folder_handler)) !== false) {
						...
						if (($v_descr['stored_filename'] != $v_descr['filename']) && (!isset($p_options[PCLZIP_OPT_REMOVE_ALL_PATH]))) {
							if ($v_descr['stored_filename'] != '') $v_dirlist_descr[$v_dirlist_nb]['new_full_name'] = $v_descr['stored_filename'].'/'.$v_item_handler;
	...
	function privAddFile($p_filedescr, &$p_header, &$p_options) {
		...
		else if ($p_filedescr['type'] == 'virtual_file') $p_header['mtime'] = time();
	...
	function privCalculateStoredFilename(&$p_filedescr, &$p_options) {
		$v_result=1;
		$p_filename = $p_filedescr['filename'];
		if (isset($p_options[PCLZIP_OPT_ADD_PATH])) $p_add_dir = $p_options[PCLZIP_OPT_ADD_PATH];
		else $p_add_dir = '';
		if (isset($p_options[PCLZIP_OPT_REMOVE_PATH])) $p_remove_dir = $p_options[PCLZIP_OPT_REMOVE_PATH];
		else $p_remove_dir = '';
		if (isset($p_options[PCLZIP_OPT_REMOVE_ALL_PATH])) $p_remove_all_dir = $p_options[PCLZIP_OPT_REMOVE_ALL_PATH];
		else $p_remove_all_dir = 0;
		if (isset($p_filedescr['new_full_name'])) {
			...
			if ($p_remove_all_dir) {
				...
				if ($v_compare > 0) {
					if ($v_compare == 2) $v_stored_filename = "";
	...
	function privExtractByRule(&$p_file_list, $p_path, $p_remove_path, $p_remove_all_path, &$p_options) {
		...
		if (($p_remove_path != "") && (substr($p_remove_path, -1) != '/')) $p_remove_path .= '/';
		...
		for ($i=0, $v_nb_extracted=0; $i<$v_central_dir['entries']; $i++) {
			...
			if ((isset($p_options[PCLZIP_OPT_BY_NAME])) && ($p_options[PCLZIP_OPT_BY_NAME] != 0)) {
					...
					if (preg_match($p_options[PCLZIP_OPT_BY_PREG], $v_header['stored_filename'])) $v_extract = true;
			} else if (	 (isset($p_options[PCLZIP_OPT_BY_INDEX])) && ($p_options[PCLZIP_OPT_BY_INDEX] != 0)) {
					for ($j=$j_start; ($j<sizeof($p_options[PCLZIP_OPT_BY_INDEX])) && (!$v_extract); $j++) {
							if (($i>=$p_options[PCLZIP_OPT_BY_INDEX][$j]['start']) && ($i<=$p_options[PCLZIP_OPT_BY_INDEX][$j]['end'])) $v_extract = true;
