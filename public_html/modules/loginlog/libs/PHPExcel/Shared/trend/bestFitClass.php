...
class PHPExcel_Best_Fit
{
	...
	public function getSlope($dp=0) {
		if ($dp != 0) return round($this->_slope,$dp);
		return $this->_slope;
	}

	public function getSlopeSE($dp=0) {
		if ($dp != 0) return round($this->_slopeSE,$dp);
		return $this->_slopeSE;
	}

	public function getIntersect($dp=0) {
		if ($dp != 0) return round($this->_intersect,$dp);
		return $this->_intersect;
	}

	public function getIntersectSE($dp=0) {
		if ($dp != 0) return round($this->_intersectSE,$dp);
		return $this->_intersectSE;
	}

	public function getGoodnessOfFit($dp=0) {
		if ($dp != 0) return round($this->_goodnessOfFit,$dp);
		return $this->_goodnessOfFit;
	}

	public function getGoodnessOfFitPercent($dp=0) {
		if ($dp != 0) return round($this->_goodnessOfFit * 100,$dp);
		return $this->_goodnessOfFit * 100;
	}

	public function getStdevOfResiduals($dp=0) {
		if ($dp != 0) return round($this->_stdevOfResiduals,$dp);
		return $this->_stdevOfResiduals;
	}

	public function getSSRegression($dp=0) {
		if ($dp != 0) return round($this->_SSRegression,$dp);
		return $this->_SSRegression;
	}

	public function getSSResiduals($dp=0) {
		if ($dp != 0) return round($this->_SSResiduals,$dp);
		return $this->_SSResiduals;
	}

	public function getDFResiduals($dp=0) {
		if ($dp != 0) return round($this->_DFResiduals,$dp);
		return $this->_DFResiduals;
	}

	public function getF($dp=0) {
		if ($dp != 0) return round($this->_F,$dp);
		return $this->_F;
	}

	public function getCovariance($dp=0) {
		if ($dp != 0) return round($this->_covariance,$dp);
		return $this->_covariance;
	}

	public function getCorrelation($dp=0) {
		if ($dp != 0) return round($this->_correlation,$dp);
	...
	protected function _calculateGoodnessOfFit($sumX,$sumY,$sumX2,$sumY2,$sumXY,$meanX,$meanY, $const) {
		...
		foreach($this->_xValues as $xKey => $xValue) {
			...
			if ($const) $SStot += ($this->_yValues[$xKey] - $meanY) * ($this->_yValues[$xKey] - $meanY);
			else $SStot += $this->_yValues[$xKey] * $this->_yValues[$xKey];
			$SScov += ($this->_xValues[$xKey] - $meanX) * ($this->_yValues[$xKey] - $meanY);
			if ($const) $SSsex += ($this->_xValues[$xKey] - $meanX) * ($this->_xValues[$xKey] - $meanX);
			else $SSsex += $this->_xValues[$xKey] * $this->_xValues[$xKey];
		}
		$this->_SSResiduals = $SSres;
		$this->_DFResiduals = $this->_valueCount - 1 - $const;
		if ($this->_DFResiduals == 0.0) $this->_stdevOfResiduals = 0.0;
		else $this->_stdevOfResiduals = sqrt($SSres / $this->_DFResiduals);
		if (($SStot == 0.0) || ($SSres == $SStot)) $this->_goodnessOfFit = 1;
		else $this->_goodnessOfFit = 1 - ($SSres / $SStot);
		...
		if ($this->_SSResiduals != 0.0) {
			if ($this->_DFResiduals == 0.0) $this->_F = 0.0;
			else $this->_F = $this->_SSRegression / ($this->_SSResiduals / $this->_DFResiduals);
		} else {
			if ($this->_DFResiduals == 0.0) $this->_F = 0.0;
	...
	protected function _leastSquareFit($yValues, $xValues, $const) {
		...
		if ($const) $this->_intersect = $meanY - ($this->_slope * $meanX);
		else $this->_intersect = 0;
