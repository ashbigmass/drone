...
class PHPExcel_Polynomial_Best_Fit extends PHPExcel_Best_Fit
{
	...
	public function getValueOfYForX($xValue) {
		...
		foreach($slope as $key => $value) {
			if ($value != 0.0) $retVal += $value * pow($xValue, $key + 1);
		}
		return $retVal;
	}
	...
	public function getSlope($dp=0) {
		if ($dp != 0) {
			$coefficients = array();
			foreach($this->_slope as $coefficient) $coefficients[] = round($coefficient,$dp);
			return $coefficients;
		}
		return $this->_slope;
	}
	...
	private function _polynomial_regression($order, $yValues, $xValues, $const) {
		...
		for ($i = 0; $i < $this->_valueCount; ++$i) {
			for ($j = 0; $j <= $order; ++$j) $A[$i][$j] = pow($xValues[$i], $j);
		}
		for ($i=0; $i < $this->_valueCount; ++$i) $B[$i] = array($yValues[$i]);
		...
		foreach($this->_xValues as $xKey => $xValue) $this->_yBestFitValues[$xKey] = $this->getValueOfYForX($xValue);
	}
	...
	function __construct($order, $yValues, $xValues=array(), $const=True) {
		if (parent::__construct($yValues, $xValues) !== False) {
			if ($order < $this->_valueCount) {
				...
				if (($this->getGoodnessOfFit() < 0.0) || ($this->getGoodnessOfFit() > 1.0)) $this->_error = True;
			} else {
				$this->_error = True;
			}
		}
