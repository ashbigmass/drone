...
class PHPExcel_Exponential_Best_Fit extends PHPExcel_Best_Fit
{
	...
	public function getSlope($dp=0) {
		if ($dp != 0) return round(exp($this->_slope),$dp);
		return exp($this->_slope);
	}

	public function getIntersect($dp=0) {
		if ($dp != 0) return round(exp($this->_intersect),$dp);
		return exp($this->_intersect);
	}

	private function _exponential_regression($yValues, $xValues, $const) {
		foreach($yValues as &$value) {
			if ($value < 0.0) $value = 0 - log(abs($value));
			elseif ($value > 0.0) $value = log($value);
		}
		unset($value);
	...
	function __construct($yValues, $xValues=array(), $const=True) {
		if (parent::__construct($yValues, $xValues) !== False) $this->_exponential_regression($yValues, $xValues, $const);
	}
}
