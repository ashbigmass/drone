...
class PHPExcel_Worksheet_PageSetup
{
	...
	public function setScale($pValue = 100, $pUpdate = true) {
		if (($pValue >= 0) || is_null($pValue)) {
			$this->_scale = $pValue;
			if ($pUpdate) $this->_fitToPage = false;
		} else {
	...
	public function setFitToHeight($pValue = 1, $pUpdate = TRUE) {
		...
		if ($pUpdate) $this->_fitToPage = TRUE;
		return $this;
	}
	...
	public function isColumnsToRepeatAtLeftSet() {
		if (is_array($this->_columnsToRepeatAtLeft)) {
			if ($this->_columnsToRepeatAtLeft[0] != '' && $this->_columnsToRepeatAtLeft[1] != '') return true;
		}
		return false;
	}
	...
	public function setColumnsToRepeatAtLeft($pValue = null) {
		if (is_array($pValue)) $this->_columnsToRepeatAtLeft = $pValue;
		return $this;
	}
	...
	public function isRowsToRepeatAtTopSet() {
		if (is_array($this->_rowsToRepeatAtTop)) {
			if ($this->_rowsToRepeatAtTop[0] != 0 && $this->_rowsToRepeatAtTop[1] != 0) return true;
		}
		return false;
	}
	...
	public function setRowsToRepeatAtTop($pValue = null) {
		if (is_array($pValue)) $this->_rowsToRepeatAtTop = $pValue;
		return $this;
	}
	...
	public function getPrintArea($index = 0) {
		if ($index == 0) return $this->_printArea;
		$printAreas = explode(',',$this->_printArea);
		if (isset($printAreas[$index-1])) return $printAreas[$index-1];
		throw new Exception("Requested Print Area does not exist");
	}

	public function isPrintAreaSet($index = 0) {
		if ($index == 0) return !is_null($this->_printArea);
		$printAreas = explode(',',$this->_printArea);
	...
	public function setPrintArea($value, $index = 0, $method = self::SETPRINTRANGE_OVERWRITE) {
		if (strpos($value,'!') !== false) throw new Exception('Cell coordinate must not specify a worksheet.');
		elseif (strpos($value,':') === false) throw new Exception('Cell coordinate must be a range of cells.');
		elseif (strpos($value,'$') !== false) throw new Exception('Cell coordinate must not be absolute.');
		...
		if ($method == self::SETPRINTRANGE_OVERWRITE) {
			if ($index == 0) {
				...
				$printAreas = explode(',',$this->_printArea);
				if($index < 0) $index = count($printAreas) - abs($index) + 1;
				if (($index <= 0) || ($index > count($printAreas))) throw new Exception('Invalid index for setting print range.');
			...
			if ($index == 0) {
				...
				if ($index > count($printAreas)) throw new Exception('Invalid index for setting print range.');
	...
	public function __clone() {
		...
		foreach ($vars as $key => $value) {
			if (is_object($value)) $this->$key = clone $value;
			else $this->$key = $value;
		}
	}
}
