...
class PHPExcel_Worksheet_AutoFilter
{
	...
	public function setRange($pRange = '') {
		...
		if (count($cellAddress) > 1) list($worksheet,$pRange) = $cellAddress;
		if (strpos($pRange,':') !== FALSE) $this->_range = $pRange;
		elseif(empty($pRange)) $this->_range = '';
		else throw new PHPExcel_Exception('Autofilter must be set on a range of cells.');
		if (empty($pRange)) {
			$this->_columns = array();
		} else {
			...
			foreach($this->_columns as $key => $value) {
				...
				if (($rangeStart[0] > $colIndex) || ($rangeEnd[0] < $colIndex)) unset($this->_columns[$key]);
			}
		}
		return $this;
	}
	...
	public function testColumnInRange($column) {
		if (empty($this->_range)) throw new PHPExcel_Exception("No autofilter range is defined.");
		$columnIndex = PHPExcel_Cell::columnIndexFromString($column);
		list($rangeStart,$rangeEnd) = PHPExcel_Cell::rangeBoundaries($this->_range);
		if (($rangeStart[0] > $columnIndex) || ($rangeEnd[0] < $columnIndex)) throw new PHPExcel_Exception("Column is outside of current autofilter range.");
	...
	public function getColumn($pColumn) {
		...
		if (!isset($this->_columns[$pColumn])) $this->_columns[$pColumn] = new PHPExcel_Worksheet_AutoFilter_Column($pColumn, $this);
	...
	public function setColumn($pColumn) {
		if ((is_string($pColumn)) && (!empty($pColumn))) $column = $pColumn;
		elseif(is_object($pColumn) && ($pColumn instanceof PHPExcel_Worksheet_AutoFilter_Column)) $column = $pColumn->getColumnIndex();
		else throw new PHPExcel_Exception("Column is not within the autofilter range.");
	...
	public function clearColumn($pColumn) {
		$this->testColumnInRange($pColumn);
		if (isset($this->_columns[$pColumn])) unset($this->_columns[$pColumn]);
		return $this;
	}
	...
	private static function _filterTestInSimpleDataSet($cellValue,$dataSet) {
		...
		if (($cellValue == '') || ($cellValue === NULL)) return $blanks;
	...
	private static function _filterTestInPeriodDateSet($cellValue,$monthSet) {
		...
		if (is_numeric($cellValue)) {
			...
			if (in_array($dateValue,$monthSet)) return TRUE;
		}
		return FALSE;
	}
	...
	private function _calculateTopTenValue($columnID,$startRow,$endRow,$ruleType,$ruleValue) {
		...
		if ($ruleType == PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_TOP) rsort($dataValues);
		else sort($dataValues);
	...
	public function __clone() {
		...
		foreach ($vars as $key => $value) {
			if (is_object($value)) {
				if ($key == '_workSheet') $this->$key = NULL;
