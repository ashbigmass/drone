...
class PHPExcel_Worksheet_AutoFilter_Column
{
	...
	public function setColumnIndex($pColumn) {
		...
		if ($this->_parent !== NULL) $this->_parent->testColumnInRange($pColumn);
		$this->_columnIndex = $pColumn;
		return $this;
	}
	...
	public function setFilterType($pFilterType = self::AUTOFILTER_FILTERTYPE_FILTER) {
		if (!in_array($pFilterType,self::$_filterTypes)) throw new PHPExcel_Exception('Invalid filter type for column AutoFilter.');
	...
	public function setJoin($pJoin = self::AUTOFILTER_COLUMN_JOIN_OR) {
		...
		if (!in_array($pJoin,self::$_ruleJoins)) throw new PHPExcel_Exception('Invalid rule connection for column AutoFilter.');
		$this->_join = $pJoin;
		return $this;
	}
	...
	public function getRule($pIndex) {
		if (!isset($this->_ruleset[$pIndex])) $this->_ruleset[$pIndex] = new PHPExcel_Worksheet_AutoFilter_Column_Rule($this);
	...
	public function deleteRule($pIndex) {
		if (isset($this->_ruleset[$pIndex])) {
			...
			if (count($this->_ruleset) <= 1) $this->setJoin(self::AUTOFILTER_COLUMN_JOIN_OR);
		}
		return $this;
	}
