...
class PHPExcel_Worksheet_AutoFilter_Column_Rule
{
	...
	public function setRuleType($pRuleType = self::AUTOFILTER_RULETYPE_FILTER) {
		if (!in_array($pRuleType,self::$_ruleTypes)) throw new PHPExcel_Exception('Invalid rule type for column AutoFilter Rule.');
	...
	public function setValue($pValue = '') {
		if (is_array($pValue)) {
			...
			foreach($pValue as $key => $value) {
				if (!in_array($key,self::$_dateTimeGroups)) unset($pValue[$key]);
				else $grouping = max($grouping,array_search($key,self::$_dateTimeGroups));
			}
			if (count($pValue) == 0) throw new PHPExcel_Exception('Invalid rule value for column AutoFilter Rule.');
