...
class PHPExcel_CachedObjectStorage_SQLite extends PHPExcel_CachedObjectStorage_CacheBase implements PHPExcel_CachedObjectStorage_ICache
{
	...
	public function getCacheData($pCoord) {
		...
		if ($cellResultSet === false) throw new Exception(sqlite_error_string($this->_DBHandle->lastError()));
		elseif ($cellResultSet->numRows() == 0) return null;
		$this->_currentObjectID = $pCoord;
	...
	public function isDataSet($pCoord) {
		...
		$cellResultSet = $this->_DBHandle->query($query,SQLITE_ASSOC);
		if ($cellResultSet === false) throw new Exception(sqlite_error_string($this->_DBHandle->lastError()));
		elseif ($cellResultSet->numRows() == 0) return false;
		return true;
	}
	...
	public function deleteCacheData($pCoord) {
		...
		if (!$this->_DBHandle->queryExec($query)) throw new Exception(sqlite_error_string($this->_DBHandle->lastError()));
		$this->_currentCellIsDirty = false;
	}
	...
	public function getCellList() {
		...
		if ($cellIdsResult === false) throw new Exception(sqlite_error_string($this->_DBHandle->lastError()));
		$cellKeys = array();
		foreach($cellIdsResult as $row) $cellKeys[] = $row['id'];
		return $cellKeys;
	}
