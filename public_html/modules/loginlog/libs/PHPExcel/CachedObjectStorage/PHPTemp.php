...
class PHPExcel_CachedObjectStorage_PHPTemp extends PHPExcel_CachedObjectStorage_CacheBase implements PHPExcel_CachedObjectStorage_ICache
{
	...
	public function copyCellCollection(PHPExcel_Worksheet $parent) {
		...
		while (!feof($this->_fileHandle)) fwrite($newFileHandle,fread($this->_fileHandle, 1024));
	...
	public function __construct(PHPExcel_Worksheet $parent, $arguments) {
		...
		if (is_null($this->_fileHandle)) $this->_fileHandle = fopen('php://temp/maxmemory:'.$this->_memoryCacheSize,'a+');
	}

	public function __destruct() {
		if (!is_null($this->_fileHandle)) fclose($this->_fileHandle);
