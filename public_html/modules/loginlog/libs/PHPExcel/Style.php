...
class PHPExcel_Style implements PHPExcel_IComparable
{
	...
	public function getSharedComponent() {
		...
		if ($activeSheet->cellExists($selectedCell)) $xfIndex = $activeSheet->getCell($selectedCell)->getXfIndex();
		else $xfIndex = 0;
	...
	public function applyFromArray($pStyles = null, $pAdvanced = true) {
		if (is_array($pStyles)) {
			if ($this->_isSupervisor) {
				...
				if ($pAdvanced && isset($pStyles['borders'])) {
					if (isset($pStyles['borders']['allborders'])) {
						foreach (array('outline', 'inside') as $component) {
							if (!isset($pStyles['borders'][$component])) $pStyles['borders'][$component] = $pStyles['borders']['allborders'];
						}
						unset($pStyles['borders']['allborders']);
					}
					if (isset($pStyles['borders']['outline'])) {
						foreach (array('top', 'right', 'bottom', 'left') as $component) {
							if (!isset($pStyles['borders'][$component])) $pStyles['borders'][$component] = $pStyles['borders']['outline'];
						}
						unset($pStyles['borders']['outline']);
					}
					if (isset($pStyles['borders']['inside'])) {
						foreach (array('vertical', 'horizontal') as $component) {
							if (!isset($pStyles['borders'][$component])) $pStyles['borders'][$component] = $pStyles['borders']['inside'];
						}
					...
					for ($x = 1; $x <= $xMax; ++$x) {
						...
						for ($y = 1; $y <= $yMax; ++$y) {
							$edges = array();
							if ($x == 1) $edges[] = 'left';
							if ($x == $xMax) $edges[] = 'right';
							if ($y == 1) $edges[] = 'top';
							if ($y == $yMax) $edges[] = 'bottom';
							...
							foreach ($innerEdges as $innerEdge) {
								switch ($innerEdge) {
									case 'top':
									case 'bottom':
										if (isset($pStyles['borders']['horizontal'])) $regionStyles['borders'][$innerEdge] = $pStyles['borders']['horizontal'];
										else unset($regionStyles['borders'][$innerEdge]);
										break;
									case 'left':
									case 'right':
										if (isset($pStyles['borders']['vertical'])) $regionStyles['borders'][$innerEdge] = $pStyles['borders']['vertical'];
										else unset($regionStyles['borders'][$innerEdge]);
										break;
								}
							}
				...
				if (preg_match('/^[A-Z]+1:[A-Z]+1048576$/', $pRange)) $selectionType = 'COLUMN';
				else if (preg_match('/^A[0-9]+:XFD[0-9]+$/', $pRange)) $selectionType = 'ROW';
				else $selectionType = 'CELL';
				...
				if (array_key_exists('fill', $pStyles)) $this->getFill()->applyFromArray($pStyles['fill']);
				if (array_key_exists('font', $pStyles)) $this->getFont()->applyFromArray($pStyles['font']);
				if (array_key_exists('borders', $pStyles)) $this->getBorders()->applyFromArray($pStyles['borders']);
				if (array_key_exists('alignment', $pStyles)) $this->getAlignment()->applyFromArray($pStyles['alignment']);
				if (array_key_exists('numberformat', $pStyles)) $this->getNumberFormat()->applyFromArray($pStyles['numberformat']);
				if (array_key_exists('protection', $pStyles)) $this->getProtection()->applyFromArray($pStyles['protection']);
			}
		} else {
	...
	public function setConditionalStyles($pValue = null) {
		if (is_array($pValue)) $this->getActiveSheet()->setConditionalStyles($this->getSelectedCells(), $pValue);
		return $this;
	}
	...
	public function getHashCode() {
		$hashConditionals = '';
		foreach ($this->_conditionalStyles as $conditional) $hashConditionals .= $conditional->getHashCode();
		return md5(
