...
class PHPExcel_Calculation
{
	...
	private function __construct() {
		...
		foreach (glob($localeFileDirectory.'/*',GLOB_ONLYDIR) as $filename) {
			...
			if ($filename != 'en') self::$_validLocaleLanguages[] = $filename;
		}
		$setPrecision = (PHP_INT_SIZE == 4) ? 12 : 16;
		$this->_savedPrecision = ini_get('precision');
		if ($this->_savedPrecision < $setPrecision) ini_set('precision',$setPrecision);
	}

	public function __destruct() {
		if ($this->_savedPrecision != ini_get('precision')) ini_set('precision',$this->_savedPrecision);
	}
	...
	public static function flushInstance() {
		if (isset(self::$_instance) && (self::$_instance !== NULL)) self::$_instance->clearCalculationCache();
	}
	...
	public function setLocale($locale='en_us') {
		...
		if (strpos($locale,'_') !== false) list($language) = explode('_',$locale);
		if (in_array($language,self::$_validLocaleLanguages)) {
			...
			if ($locale != 'en_us') {
				...
				if (!file_exists($functionNamesFile)) {
					...
					if (!file_exists($functionNamesFile)) return false;
				}
	...
	public static function _translateSeparator($fromSeparator,$toSeparator,$formula,&$inBraces) {
		...
		for ($i = 0; $i < $strlen; ++$i) {
			...
			switch ($chr) {
				...
				case $fromSeparator : if (!$inBraces) $formula = mb_substr($formula,0,$i).$toSeparator.mb_substr($formula,$i+1);
			}
		}
		return $formula;
	}
	...
	public static function _wrapResult($value) {
		if (is_string($value)) {
			if (preg_match('/^'.self::CALCULATION_REGEXP_ERROR.'$/i', $value, $match)) return $value;
			return '"'.$value.'"';
	...
	public static function _unwrapResult($value) {
		if (is_string($value)) {
			if ((isset($value{0})) && ($value{0} == '"') && (substr($value,-1) == '"')) return substr($value,1,-1);
		} else if((is_float($value)) && ((is_nan($value)) || (is_infinite($value)))) {
			return PHPExcel_Calculation_Functions::NaN();
		}
		return $value;
	...
	public function calculateCellValue(PHPExcel_Cell $pCell = null, $resetLog = true) {
		...
		if ($resetLog) self::$returnArrayAsType = $returnArrayAsType;
		try {
		...
		if ((is_array($result)) && (self::$returnArrayAsType != self::RETURN_ARRAY_AS_ARRAY)) {
			...
			if (self::$returnArrayAsType == self::RETURN_ARRAY_AS_ERROR) return PHPExcel_Calculation_Functions::VALUE();
			if (count($testResult) != 1) {
				...
				if (is_array($result[$r])) {
					...
					$c = array_shift($c);
					if (!is_numeric($c)) return PHPExcel_Calculation_Functions::VALUE();
				}
			}
			$result = array_shift($testResult);
		}
		if ($result === NULL) return 0;
		elseif((is_float($result)) && ((is_nan($result)) || (is_infinite($result)))) return PHPExcel_Calculation_Functions::NaN();
		return $result;
	}
	...
	public function _calculateFormulaValue($formula, $cellID=null, PHPExcel_Cell $pCell = null) {
		...
		if ($pCell !== NULL) {
			...
			if ($pCellParent !== NULL) $wsTitle = $pCellParent->getTitle();
		}
		if ($cellID !== NULL) {
		...
		if ((in_array($wsTitle.'!'.$cellID,$this->debugLogStack)) && ($wsTitle != "\x00Wrk")) {
			if ($this->cyclicFormulaCount <= 0) {
				...
				if ($this->_cyclicFormulaCount >= $this->cyclicFormulaCount) return $cellValue;
			}
		}
	...
	private static function _checkMatrixOperands(&$operand1,&$operand2,$resize = 1) {
		...
		if (($matrix1Rows == $matrix2Columns) && ($matrix2Rows == $matrix1Columns)) $resize = 1;
		if ($resize == 2) {
	...
	public static function _getMatrixDimensions(&$matrix) {
		...
		foreach($matrix as $rowKey => $rowValue) {
			...
			if (!is_array($rowValue)) $matrix[$rowKey] = array($rowValue);
			else $matrix[$rowKey] = array_values($rowValue);
		}
		...
		if (($matrix2Columns < $matrix1Columns) || ($matrix2Rows < $matrix1Rows)) {
			if ($matrix2Columns < $matrix1Columns) {
				for ($i = 0; $i < $matrix1Rows; ++$i) {
					for ($j = $matrix2Columns; $j < $matrix1Columns; ++$j) unset($matrix1[$i][$j]);
				}
			}
			if ($matrix2Rows < $matrix1Rows) {
				for ($i = $matrix2Rows; $i < $matrix1Rows; ++$i) unset($matrix1[$i]);
			}
		}
		if (($matrix1Columns < $matrix2Columns) || ($matrix1Rows < $matrix2Rows)) {
			if ($matrix1Columns < $matrix2Columns) {
				for ($i = 0; $i < $matrix2Rows; ++$i) {
					for ($j = $matrix1Columns; $j < $matrix2Columns; ++$j) unset($matrix2[$i][$j]);
				}
			}
			if ($matrix1Rows < $matrix2Rows) {
				for ($i = $matrix1Rows; $i < $matrix2Rows; ++$i) unset($matrix2[$i]);
			}
		}
	}
		...
		if (($matrix2Columns < $matrix1Columns) || ($matrix2Rows < $matrix1Rows)) {
			if ($matrix2Columns < $matrix1Columns) {
				for ($i = 0; $i < $matrix2Rows; ++$i) {
					...
					for ($j = $matrix2Columns; $j < $matrix1Columns; ++$j) $matrix2[$i][$j] = $x;
				}
			}
			if ($matrix2Rows < $matrix1Rows) {
				$x = $matrix2[$matrix2Rows-1];
				for ($i = 0; $i < $matrix1Rows; ++$i) $matrix2[$i] = $x;
			}
		}
		if (($matrix1Columns < $matrix2Columns) || ($matrix1Rows < $matrix2Rows)) {
			if ($matrix1Columns < $matrix2Columns) {
				for ($i = 0; $i < $matrix1Rows; ++$i) {
					...
					for ($j = $matrix1Columns; $j < $matrix2Columns; ++$j) $matrix1[$i][$j] = $x;
				}
			}
			if ($matrix1Rows < $matrix2Rows) {
				$x = $matrix1[$matrix1Rows-1];
				for ($i = 0; $i < $matrix2Rows; ++$i) $matrix1[$i] = $x;
			}
		}
	}
	...
	private function _showValue($value) {
		if ($this->writeDebugLog) {
			...
			if (count($testArray) == 1) $value = array_pop($testArray);
			if (is_array($value)) {
				$returnMatrix = array();
				$pad = $rpad = ', ';
	...
	private function _showTypeDetails($value) {
		if ($this->writeDebugLog) {
			...
			if ($value === NULL) {
				...
				elseif ($value{0} == '#') return 'a '.$value.' error';
				else $typeString = 'a string';
			}
	...
	private static function _convertMatrixReferences($formula) {
		...
		if (strpos($formula,'{') !== false) {
			...
			if ($openCount < $closeCount) {
				if ($openCount > 0) return $this->_raiseFormulaError("Formula Error: Mismatched matrix braces '}'");
				else return $this->_raiseFormulaError("Formula Error: Unexpected '}' encountered");
			} elseif ($openCount > $closeCount) {
				if ($closeCount > 0) return $this->_raiseFormulaError("Formula Error: Mismatched matrix braces '{'");
				else return $this->_raiseFormulaError("Formula Error: Unexpected '{' encountered");
			}
		}
		return $formula;
	...
	private function _parseFormula($formula, PHPExcel_Cell $pCell = null) {
		if (($formula = self::_convertMatrixReferences(trim($formula))) === false) return FALSE;
		$pCellParent = ($pCell !== NULL) ? $pCell->getParent() : NULL;
		$operatorAssociativity	= array('^' => 0,
										'*' => 0, '/' => 0,
										'+' => 0, '-' => 0,
										'&' => 0,
										'|' => 0, ':' => 0,
				...
				if (preg_match('/^'.self::CALCULATION_REGEXP_FUNCTION.'$/i', $val, $matches)) {
					...
					if ($testPrevOp['value'] == ':') {
						if ($matches[2] == '') {
							...
							if ($startMatches[2] > '') $val = $startMatches[2].'!'.$val;
						} else {
					...
					if ($testPrevOp['value'] == ':') {
						...
						$rangeWS1 = '';
						if (strpos('!',$startRowColRef) !== false) list($rangeWS1,$startRowColRef) = explode('!',$startRowColRef);
						if ($rangeWS1 != '') $rangeWS1 .= '!';
						$rangeWS2 = $rangeWS1;
						if (strpos('!',$val) !== false) list($rangeWS2,$val) = explode('!',$val);
	...
	private static function _dataTestReference(&$operandData) {
		...
		if (($operandData['reference'] === NULL) && (is_array($operand))) {
			...
			if (ctype_upper($colKey)) $operandData['reference'] = $colKey.$rowKey;
		}
		return $operand;
	}
		...
		foreach ($tokens as $tokenData) {
			...
			if (isset(self::$_binaryOperators[$token])) {
				...
				switch ($token) {
					...
					case ':'	:
						$sheet1 = $sheet2 = '';
						if (strpos($operand1Data['reference'],'!') !== false) list($sheet1,$operand1Data['reference']) = explode('!',$operand1Data['reference']);
						else $sheet1 = ($pCellParent !== NULL) ? $pCellParent->getTitle() : '';
						if (strpos($operand2Data['reference'],'!') !== false) list($sheet2,$operand2Data['reference']) = explode('!',$operand2Data['reference']);
						else $sheet2 = $sheet1;
						if ($sheet1 == $sheet2) {
					...
					case '^'	:
						...
						break;
					case '&'	:
						if (is_bool($operand1)) $operand1 = ($operand1) ? self::$_localeBoolean['TRUE'] : self::$_localeBoolean['FALSE'];
						if (is_bool($operand2)) $operand2 = ($operand2) ? self::$_localeBoolean['TRUE'] : self::$_localeBoolean['FALSE'];
				...
				if (isset($matches[8])) {
					if ($pCell === NULL) {
						...
						if ($matches[2] > '') {
							...
							if ($pCellParent !== NULL) $cellValue = $this->extractCellRange($cellRef, $pCellParent, false);
							else return $this->_raiseFormulaError('Unable to access Cell Reference');
				...
				if ((isset(self::$_PHPExcelFunctions[$functionName])) || (isset(self::$_controlFunctions[$functionName]))) {
					...
					if ($functionName != 'MKMATRIX') {
						...
						if ($passCellReference) $args[] = $pCell;
						if (strpos($functionCall,'::') !== false) {
							$result = call_user_func_array(explode('::',$functionCall),$args);
						} else {
							foreach($args as &$arg) $arg = PHPExcel_Calculation_Functions::flattenSingleValue($arg);
							unset($arg);
	...
	private function _writeDebug($message) {
		if ($this->writeDebugLog) {
			if ($this->echoDebugLog) echo implode(' -> ',$this->debugLogStack).' -> '.$message,'<br />';
	...
	public function listFunctionNames() {
		$returnValue = array();
		foreach(self::$_PHPExcelFunctions as $functionName => $function) {
			if ($function['functionCall'] != 'PHPExcel_Calculation_Functions::DUMMY') $returnValue[] = $functionName;
		}
		return $returnValue;
	}
}
...
class PHPExcel_Token_Stack
{
	...
	public function push($type,$value,$reference=null) {
		...
		if ($type == 'Function') {
			...
			if ($localeFunction != $value) $this->_stack[($this->_count - 1)]['localeValue'] = $localeFunction;
		}
	}

	public function pop() {
		if ($this->_count > 0) return $this->_stack[--$this->_count];
		return null;
	}

	public function last($n=1) {
		if ($this->_count-$n < 0) return null;
