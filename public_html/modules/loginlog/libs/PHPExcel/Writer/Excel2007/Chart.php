...
class PHPExcel_Writer_Excel2007_Chart extends PHPExcel_Writer_Excel2007_WriterPart
{
	...
	private static function _getChartType($plotArea) {
		...
		if ($groupCount == 1) {
			...
			$chartTypes = array();
			for($i = 0; $i < $groupCount; ++$i) $chartTypes[] = $plotArea->getPlotGroupByIndex($i)->getPlotType();
			$chartType = array_unique($chartTypes);
			if (count($chartTypes) == 0) throw new Exception('Chart is not yet implemented');
		}
		return $chartType;
	}
									...
									)
	{
		...
		foreach($plotSeriesOrder as $plotSeriesIdx => $plotSeriesRef) {
			...
			if ($groupType === PHPExcel_Chart_DataSeries::TYPE_BUBBLECHART) $this->_writeBubbles($plotSeriesValues, $objWriter, $pSheet);
			$objWriter->endElement();
		}
	}

	private function _writePlotSeriesLabel($plotSeriesLabel, $objWriter) {
		if (is_null($plotSeriesLabel)) return;
	...
	{
		if (is_null($plotSeriesValues)) return;
