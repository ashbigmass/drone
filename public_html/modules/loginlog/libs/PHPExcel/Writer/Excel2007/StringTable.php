...
class PHPExcel_Writer_Excel2007_StringTable extends PHPExcel_Writer_Excel2007_WriterPart
{
		...
		if ($pStringTable !== NULL) {
			...
			foreach ($pStringTable as $textElement) {
				...
				if (! $textElement instanceof PHPExcel_RichText) {
					...
					if ($textToWrite !== trim($textToWrite)) $objWriter->writeAttribute('xml:space', 'preserve');
		...
		foreach ($elements as $element) {
			...
			if ($element instanceof PHPExcel_RichText_Run) {
				...
				if ($element->getFont()->getSuperScript() || $element->getFont()->getSubScript()) {
					...
					if ($element->getFont()->getSuperScript()) $objWriter->writeAttribute('val', 'superscript');
					else if ($element->getFont()->getSubScript()) $objWriter->writeAttribute('val', 'subscript');
					$objWriter->endElement();
				}
	...
	public function flipStringTable($stringTable = array()) {
		...
		foreach ($stringTable as $key => $value) {
			if (! $value instanceof PHPExcel_RichText) $returnValue[$value] = $key;
			else if ($value instanceof PHPExcel_RichText) $returnValue[$value->getHashCode()] = $key;
		}
		return $returnValue;
	}
}
