...
class PHPExcel_Writer_Excel2007_Style extends PHPExcel_Writer_Excel2007_WriterPart
{
	public function writeStyles(PHPExcel $pPHPExcel = null) {
		...
		foreach ($pPHPExcel->getCellXfCollection() as $cellXf) $this->_writeCellStyleXf($objWriter, $cellXf, $pPHPExcel);
		$objWriter->endElement();
	...
	private function _writeFont(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_Style_Font $pFont = null) {
		...
		if ($pFont->getSuperScript() === TRUE || $pFont->getSubScript() === TRUE) {
			...
			if ($pFont->getSuperScript() === TRUE) $objWriter->writeAttribute('val', 'superscript');
			else if ($pFont->getSubScript() === TRUE) $objWriter->writeAttribute('val', 'subscript');
			$objWriter->endElement();
		}
	...
	private function _writeCellStyleXf(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_Style $pStyle = null, PHPExcel $pPHPExcel = null) {
		...
		$textRotation = 0;
		if ($pStyle->getAlignment()->getTextRotation() >= 0) $textRotation = $pStyle->getAlignment()->getTextRotation();
		else if ($pStyle->getAlignment()->getTextRotation() < 0) $textRotation = 90 - $pStyle->getAlignment()->getTextRotation();
	...
	public function allConditionalStyles(PHPExcel $pPHPExcel = null) {
		...
		for ($i = 0; $i < $sheetCount; ++$i) {
			foreach ($pPHPExcel->getSheet($i)->getConditionalStylesCollection() as $conditionalStyles) {
				foreach ($conditionalStyles as $conditionalStyle) $aStyles[] = $conditionalStyle;
			}
		}
		return $aStyles;
	}
	...
	public function allFills(PHPExcel $pPHPExcel = null) {
		...
		foreach ($aStyles as $style) {
			if (!array_key_exists($style->getFill()->getHashCode(), $aFills)) $aFills[ $style->getFill()->getHashCode() ] = $style->getFill();
		}
		return $aFills;
	}
	...
	public function allFonts(PHPExcel $pPHPExcel = null) {
		...
		$aStyles 	= $this->allStyles($pPHPExcel);
		foreach ($aStyles as $style) {
			if (!array_key_exists($style->getFont()->getHashCode(), $aFonts)) $aFonts[ $style->getFont()->getHashCode() ] = $style->getFont();
		}
		return $aFonts;
	}
	...
	public function allBorders(PHPExcel $pPHPExcel = null) {
		...
		$aStyles 	= $this->allStyles($pPHPExcel);
		foreach ($aStyles as $style) {
			if (!array_key_exists($style->getBorders()->getHashCode(), $aBorders)) $aBorders[ $style->getBorders()->getHashCode() ] = $style->getBorders();
		}
		return $aBorders;
	}
