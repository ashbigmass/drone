...
class PHPExcel_Writer_Excel2007_Worksheet extends PHPExcel_Writer_Excel2007_WriterPart
{
	...
	private function _writeSheetViews(PHPExcel_Shared_XMLWriter $objWriter = NULL, PHPExcel_Worksheet $pSheet = NULL) {
		$objWriter->startElement('sheetViews');
			...
			if ($pSheet->getShowGridlines()) $objWriter->writeAttribute('showGridLines',	'true');
			else $objWriter->writeAttribute('showGridLines',	'false');
			if ($pSheet->getShowRowColHeaders()) $objWriter->writeAttribute('showRowColHeaders', '1');
			else $objWriter->writeAttribute('showRowColHeaders', '0');
			if ($pSheet->getRightToLeft()) $objWriter->writeAttribute('rightToLeft',	'true');
			...
			if ($pane != '') $objWriter->writeAttribute('pane', $pane);
		...
		if (count($pSheet->getColumnDimensions()) > 0)  {
			$objWriter->startElement('cols');
				...
				foreach ($pSheet->getColumnDimensions() as $colDimension) {
					...
					if ($colDimension->getWidth() < 0) $objWriter->writeAttribute('width',		'9.10');
					else $objWriter->writeAttribute('width',		PHPExcel_Shared_String::FormatNumber($colDimension->getWidth()));
					if ($colDimension->getVisible() == false) $objWriter->writeAttribute('hidden',		'true');
					if ($colDimension->getAutoSize()) $objWriter->writeAttribute('bestFit',		'true');
					if ($colDimension->getWidth() != $pSheet->getDefaultColumnDimension()->getWidth()) $objWriter->writeAttribute('customWidth',	'true');
					if ($colDimension->getCollapsed() == true) $objWriter->writeAttribute('collapsed',		'true');
					if ($colDimension->getOutlineLevel() > 0) $objWriter->writeAttribute('outlineLevel',	$colDimension->getOutlineLevel());
				...
				if ($conditional->getConditionType() != PHPExcel_Style_Conditional::CONDITION_NONE) {
					...
					$objWriter->writeAttribute('sqref',	$cellCoordinate);
						...
						if ($conditional->getConditionType() == PHPExcel_Style_Conditional::CONDITION_CONTAINSTEXT
							...
							foreach ($conditional->getConditions() as $formula) $objWriter->writeElement('formula',	$formula);
						}
						$objWriter->endElement();
					$objWriter->endElement();
				}
	...
	private function _writeDataValidations(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_Worksheet $pSheet = null) {
		...
		if (!empty($dataValidationCollection)) {
			...
			foreach ($dataValidationCollection as $coordinate => $dv) {
				...
				if ($dv->getType() != '') $objWriter->writeAttribute('type', $dv->getType());
				if ($dv->getErrorStyle() != '') $objWriter->writeAttribute('errorStyle', $dv->getErrorStyle());
				if ($dv->getOperator() != '') $objWriter->writeAttribute('operator', $dv->getOperator());
				$objWriter->writeAttribute('allowBlank',		($dv->getAllowBlank()		? '1'  : '0'));
				$objWriter->writeAttribute('showDropDown',		(!$dv->getShowDropDown()	? '1'  : '0'));
				$objWriter->writeAttribute('showInputMessage',	($dv->getShowInputMessage()	? '1'  : '0'));
				$objWriter->writeAttribute('showErrorMessage',	($dv->getShowErrorMessage()	? '1'  : '0'));
				if ($dv->getErrorTitle() !== '') $objWriter->writeAttribute('errorTitle', $dv->getErrorTitle());
				if ($dv->getError() !== '') $objWriter->writeAttribute('error', $dv->getError());
				if ($dv->getPromptTitle() !== '') $objWriter->writeAttribute('promptTitle', $dv->getPromptTitle());
				if ($dv->getPrompt() !== '') $objWriter->writeAttribute('prompt', $dv->getPrompt());
				$objWriter->writeAttribute('sqref', $coordinate);
				if ($dv->getFormula1() !== '') $objWriter->writeElement('formula1', $dv->getFormula1());
				if ($dv->getFormula2() !== '') $objWriter->writeElement('formula2', $dv->getFormula2());
				$objWriter->endElement();
			}
			$objWriter->endElement();
		}
	...
	private function _writeHyperlinks(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_Worksheet $pSheet = null) {
		...
		if (!empty($hyperlinkCollection)) {
			...
			foreach ($hyperlinkCollection as $coordinate => $hyperlink) {
				...
				if ($hyperlink->getTooltip() != '') $objWriter->writeAttribute('tooltip', $hyperlink->getTooltip());
				$objWriter->endElement();
			}
			$objWriter->endElement();
		}
	...
	private function _writePrintOptions(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_Worksheet $pSheet = null) {
		...
		if ($pSheet->getPageSetup()->getHorizontalCentered()) $objWriter->writeAttribute('horizontalCentered', 'true');
		if ($pSheet->getPageSetup()->getVerticalCentered()) $objWriter->writeAttribute('verticalCentered', 'true');
		$objWriter->endElement();
	}
	...
	private function _writeAutoFilter(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_Worksheet $pSheet = null) {
		...
		if (!empty($autoFilterRange)) {
			...
			$range = $range[0];
			if (strpos($range[0],'!') !== false) list($ws,$range[0]) = explode('!',$range[0]);
			$range = implode(':', $range);
					...
					if (count($rules > 0)) {
						$objWriter->startElement('filterColumn');
							...
							$objWriter->startElement( $column->getFilterType());
								if ($column->getJoin() == PHPExcel_Worksheet_AutoFilter_Column::AUTOFILTER_COLUMN_JOIN_AND) $objWriter->writeAttribute('and',	1);
								foreach ($rules as $rule) {
									if (($column->getFilterType() === PHPExcel_Worksheet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_FILTER) &&
										...
										if ($val !== NULL) $objWriter->writeAttribute('val', $val);
										$maxVal = $column->getAttribute('maxVal');
										if ($maxVal !== NULL) $objWriter->writeAttribute('maxVal', $maxVal);
	...
	private function _writePageSetup(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_Worksheet $pSheet = null) {
		...
		if (!is_null($pSheet->getPageSetup()->getScale())) $objWriter->writeAttribute('scale', $pSheet->getPageSetup()->getScale());
		if (!is_null($pSheet->getPageSetup()->getFitToHeight())) $objWriter->writeAttribute('fitToHeight', $pSheet->getPageSetup()->getFitToHeight());
		else $objWriter->writeAttribute('fitToHeight', '0');
		if (!is_null($pSheet->getPageSetup()->getFitToWidth())) $objWriter->writeAttribute('fitToWidth', $pSheet->getPageSetup()->getFitToWidth());
	...
	private function _writeBreaks(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_Worksheet $pSheet = null) {
		...
		foreach ($pSheet->getBreaks() as $cell => $breakType) {
			if ($breakType == PHPExcel_Worksheet::BREAK_ROW) $aRowBreaks[] = $cell;
			else if ($breakType == PHPExcel_Worksheet::BREAK_COLUMN) $aColumnBreaks[] = $cell;
		}
		if (!empty($aRowBreaks)) {
			...
			$objWriter->startElement('sheetData');
				...
				while($currentRow++ < $highestRow) {
					...
					if ($writeCurrentRow) {
						...
						if ($rowDimension->getVisible() == false) $objWriter->writeAttribute('hidden',		'true');
						if ($rowDimension->getCollapsed() == true) $objWriter->writeAttribute('collapsed',		'true');
						if ($rowDimension->getOutlineLevel() > 0) $objWriter->writeAttribute('outlineLevel',	$rowDimension->getOutlineLevel());
						if ($rowDimension->getXfIndex() !== null) {
							$objWriter->writeAttribute('s',	$rowDimension->getXfIndex());
							$objWriter->writeAttribute('customFormat', '1');
						}
						if (isset($cellsByRow[$currentRow])) {
							foreach($cellsByRow[$currentRow] as $cellAddress) $this->_writeCell($objWriter, $pSheet, $cellAddress, $pStringTable, $aFlippedStringTable);
						}
						$objWriter->endElement();
					}
				}
	...
	private function _writeCell(
		PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_Worksheet $pSheet = null,
		$pCellAddress = null, $pStringTable = null, $pFlippedStringTable = null)
	{
		if (is_array($pStringTable) && is_array($pFlippedStringTable)) {
			...
			if ($pCell->getXfIndex() != '') $objWriter->writeAttribute('s', $pCell->getXfIndex());
			...
			if (is_object($cellValue) || $cellValue !== '') {
				...
				switch (strtolower($mappedType)) {
					...
					case 'f':
						...
						if ($this->getParentWriter()->getPreCalculateFormulas()) $calculatedValue = $pCell->getCalculatedValue();
						else $calculatedValue = $cellValue;
						if (is_string($calculatedValue)) $objWriter->writeAttribute('t', 'str');
						break;
					case 'e':
						$objWriter->writeAttribute('t', $mappedType);
				}
				switch (strtolower($mappedType)) {
					...
					case 's':
						if (! $cellValue instanceof PHPExcel_RichText) {
							if (isset($pFlippedStringTable[$cellValue])) $objWriter->writeElement('v', $pFlippedStringTable[$cellValue]);
						} else if ($cellValue instanceof PHPExcel_RichText) {
