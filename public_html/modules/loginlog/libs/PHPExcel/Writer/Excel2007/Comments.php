...
class PHPExcel_Writer_Excel2007_Comments extends PHPExcel_Writer_Excel2007_WriterPart
{
	public function writeComments(PHPExcel_Worksheet $pWorksheet = null) {
		...
		foreach ($comments as $comment) {
			if (!isset($authors[$comment->getAuthor()])) $authors[$comment->getAuthor()] = $authorId++;
		}
		$objWriter->startElement('comments');
		$objWriter->writeAttribute('xmlns', 'http://schemas.openxmlformats.org/spreadsheetml/2006/main');
		$objWriter->startElement('authors');
		foreach ($authors as $author => $index) $objWriter->writeElement('author', $author);
		$objWriter->endElement();
		$objWriter->startElement('commentList');
		foreach ($comments as $key => $value) $this->_writeComment($objWriter, $key, $value, $authors);
		$objWriter->endElement();
		$objWriter->endElement();
	...
	public function writeVMLComments(PHPExcel_Worksheet $pWorksheet = null) {
		...
		$objWriter->endElement();
		$objWriter->endElement();
		foreach ($comments as $key => $value) $this->_writeVMLComment($objWriter, $key, $value);
		$objWriter->endElement();
		return $objWriter->getData();
	}
