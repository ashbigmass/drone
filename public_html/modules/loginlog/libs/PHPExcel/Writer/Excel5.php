...
class PHPExcel_Writer_Excel5 implements PHPExcel_Writer_IWriter
{
	...
	public function save($pFilename = null) {
		...
		for ($i = 0; $i < 15; ++$i) $this->_writerWorkbook->addXfWriter($cellXfCollection[0], true);
		foreach ($this->_phpExcel->getCellXfCollection() as $style) $this->_writerWorkbook->addXfWriter($style, false);
		for ($i = 0; $i < $countSheets; ++$i) {
		...
		for ($i = 0; $i < $countSheets; ++$i) $OLE->append($this->_writerWorksheets[$i]->getData());
		...
		if(isset($OLE_SummaryInformation)) $arrRootData[] = $OLE_SummaryInformation;
		if(isset($OLE_DocumentSummaryInformation)) $arrRootData[] = $OLE_DocumentSummaryInformation;
	...
	private function _buildWorksheetEschers() {
		...
		foreach ($this->_phpExcel->getAllsheets() as $sheet) {
			...
			if (count($sheet->getDrawingCollection()) == 0 && empty($filterRange)) continue;
	...
	private function _writeDocumentSummaryInformation(){
		...
		foreach ($dataSection as $dataProp){
			...
			if($dataProp['type']['data'] == 0x02){
				...
				if($dataProp['data']['data'] == false) $dataSection_Content .= pack('V', 0x0000);
				else $dataSection_Content .= pack('V', 0x0001);
				$dataSection_Content_Offset += 4 + 4;
