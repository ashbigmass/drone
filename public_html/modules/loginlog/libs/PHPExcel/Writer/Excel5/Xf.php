...
class PHPExcel_Writer_Excel5_Xf
{
	...
	function writeXf() {
		...
		if (self::_mapBorderStyle($this->_style->getBorders()->getBottom()->getBorderStyle()) == 0) $this->_bottom_color = 0;
		if (self::_mapBorderStyle($this->_style->getBorders()->getTop()->getBorderStyle()) == 0) $this->_top_color = 0;
		if (self::_mapBorderStyle($this->_style->getBorders()->getRight()->getBorderStyle()) == 0) $this->_right_color = 0;
		if (self::_mapBorderStyle($this->_style->getBorders()->getLeft()->getBorderStyle()) == 0) $this->_left_color = 0;
		if (self::_mapBorderStyle($this->_style->getBorders()->getDiagonal()->getBorderStyle()) == 0) $this->_diag_color = 0;
		$record = 0x00E0;
		$length = 0x0014;
	...
	private static function _mapTextRotation($textRotation) {
		if ($textRotation >= 0) return $textRotation;
		if ($textRotation == -165) return 255;
