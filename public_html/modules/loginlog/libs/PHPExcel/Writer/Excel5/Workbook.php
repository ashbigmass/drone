...
class PHPExcel_Writer_Excel5_Workbook extends PHPExcel_Writer_Excel5_BIFFwriter
{
	...
	public function __construct(PHPExcel $phpExcel = null, &$str_total, &$str_unique, &$str_table, &$colors, $parser ) {
		...
		for ($i = 0; $i < $countSheets; ++$i) {
			...
			if ($phpSheet->isTabColorSet()) $this->_addColor($phpSheet->getTabColor()->getRGB());
		}
	}
	...
	public function writeWorkbook($pWorksheetSizes = null) {
		...
		$this->_writePalette();
		$part3 = '';
		if ($this->_country_code != -1) $part3 .= $this->_writeCountry();
		...
		for ($i = 0; $i < $total_worksheets; ++$i) $this->_writeBoundsheet($this->_phpExcel->getSheet($i), $this->_worksheetOffsets[$i]);
		$this->_data .= $part3;
		return $this->_data;
	}
	...
	private function _writeAllFonts() {
		foreach ($this->_fontWriters as $fontWriter) $this->_append($fontWriter->writeFont());
	}
	...
	private function _writeAllXfs() {
		foreach ($this->_xfWriters as $xfWriter) $this->_append($xfWriter->writeXf());
	}
	...
	private function _writeExterns() {
		...
		for ($i = 0; $i < $countSheets; ++$i) $this->_writeExternsheet($this->_phpExcel->getSheet($i)->getTitle());
	}
		...
		if (count($this->_phpExcel->getNamedRanges()) > 0) {
			...
			foreach ($namedRanges as $namedRange) {
				...
				for ($i = 0; $i < count($range); $i++) {
					...
					if (isset($range[$i][1])) $range[$i][1] = PHPExcel_Cell::absoluteCoordinate($range[$i][1]);
				}
				$range = PHPExcel_Cell::buildRange($range);
				try {
					$error = $this->_parser->parse($range);
					$formulaData = $this->_parser->toReversePolish();
					if (isset($formulaData{0}) and ($formulaData{0} == "\x7A" or $formulaData{0} == "\x5A")) $formulaData = "\x3A" . substr($formulaData, 1);
					if ($namedRange->getLocalOnly()) $scope = $this->_phpExcel->getIndex($namedRange->getScope()) + 1;
					else $scope = 0;
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			...
			if ($sheetSetup->isPrintAreaSet()) {
				...
				for ($j = 0; $j < $countPrintArea; ++$j) {
					...
					if ($j > 0) $formulaData .= pack('C', 0x10);
				}
	...
	private function _writeExternsheetBiff8() {
		...
		for ($i = 0; $i < $total_references; ++$i) $data .= $this->_parser->_references[$i];
		return $this->writeData($header . $data);
	}
	...
	private function _writePalette() {
		...
		foreach ($aref as $color) {
			foreach ($color as $byte) $data .= pack("C",$byte);
		}
	...
	private function _writeSharedStringsTable() {
			...
			while ($finished === false) {
				if (strlen($recordData) + strlen($string) <= $continue_limit) {
					...
					if ($space_remaining < $min_space_needed) {
						...
						if ( $encoding == 1 && (strlen($string) - $space_remaining) % 2 == 1 ) --$effective_space_remaining;
		...
		if (strlen($recordData) > 0) $recordDatas[] = $recordData;
		$chunk = '';
