...
class PHPExcel_Writer_Excel2007 implements PHPExcel_Writer_IWriter
{
	...
	public function __construct(PHPExcel $pPHPExcel = null) {
		...
		foreach ($writerPartsArray as $writer => $class) $this->_writerParts[$writer] = new $class($this);
		...
		foreach ($hashTablesArray as $tableName) $this->$tableName 	= new PHPExcel_HashTable();
	}

	public function getWriterPart($pPartName = '') {
		if ($pPartName != '' && isset($this->_writerParts[strtolower($pPartName)])) return $this->_writerParts[strtolower($pPartName)];
		else return null;
	}
	...
	public function save($pFilename = null) {
		if ($this->_spreadSheet !== NULL) {
			...
			if (strtolower($pFilename) == 'php://output' || strtolower($pFilename) == 'php://stdout') {
				...
				if ($pFilename == '') $pFilename = $originalFilename;
			}
			...
			if ($objZip->open($pFilename, $zipOverWrite) !== true) {
				if ($objZip->open($pFilename, $zipCreate) !== true) throw new Exception("Could not open " . $pFilename . " for writing.");
			}
			...
			for ($i = 0; $i < $this->_spreadSheet->getSheetCount(); ++$i) {
				...
				if ($this->_includeCharts) $chartCount = $this->_spreadSheet->getSheet($i)->getChartCount();
			...
			if ($objZip->close() === false) throw new Exception("Could not close zip file $pFilename.");
			if ($originalFilename != $pFilename) {
				if (copy($pFilename, $originalFilename) === false) throw new Exception("Could not copy temporary zip file $pFilename to $originalFilename.");
				@unlink($pFilename);
			}
		} else {
	...
	public function getPHPExcel() {
		if ($this->_spreadSheet !== null) return $this->_spreadSheet;
		else throw new Exception("No PHPExcel assigned.");
	}
	...
	public function setUseDiskCaching($pValue = false, $pDirectory = null) {
		...
		if ($pDirectory !== NULL) {
			if (is_dir($pDirectory)) $this->_diskCachingDirectory = $pDirectory;
			else throw new Exception("Directory does not exist: $pDirectory");
		}
		return $this;
	}
