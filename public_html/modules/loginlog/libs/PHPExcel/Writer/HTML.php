...
class PHPExcel_Writer_HTML implements PHPExcel_Writer_IWriter
{
	...
	public function save($pFilename = null) {
		...
		if ((!$this->_isPdf) && ($this->_generateSheetNavigationBlock)) fwrite($fileHandle, $this->generateNavigation());
	...
	public function generateHTMLHeader($pIncludeStyles = false) {
		if (is_null($this->_phpExcel)) throw new Exception('Internal PHPExcel object not set to an instance of an object.');
	...
	public function generateSheetData() {
		if (is_null($this->_phpExcel)) throw new Exception('Internal PHPExcel object not set to an instance of an object.');
		if (!$this->_spansAreCalculated) $this->_calculateSpans();
		$sheets = array();
		if (is_null($this->_sheetIndex)) $sheets = $this->_phpExcel->getAllSheets();
		else $sheets[] = $this->_phpExcel->getSheet($this->_sheetIndex);
		$html = '';
		$sheetId = 0;
		foreach ($sheets as $sheet) {
			...
			$row = $rowMin-1;
			while($row++ < $rowMax) {
				if ($row == $theadStart) $html .= '		<thead>' . PHP_EOL;
				if ($row == $tbodyStart) $html .= '		<tbody>' . PHP_EOL;
				if ( !isset($this->_isSpannedRow[$sheet->getParent()->getIndex($sheet)][$row]) ) {
					$rowData = array();
					$column = $dimension[0][0] - 1;
					while($column++ < $dimension[1][0]) {
						if ($sheet->cellExistsByColumnAndRow($column, $row)) $rowData[$column] = $sheet->getCellByColumnAndRow($column, $row);
						else $rowData[$column] = '';
					}
					$html .= $this->_generateRow($sheet, $rowData, $row - 1);
				}
				if ($row == $theadEnd) $html .= '		</thead>' . PHP_EOL;
				if ($row == $tbodyEnd) $html .= '		</tbody>' . PHP_EOL;
			}
			$html .= $this->_generateTableFooter();
			if ($this->_isPdf) {
				if (is_null($this->_sheetIndex) && $sheetId + 1 < $this->_phpExcel->getSheetCount()) $html .= '<div style="page-break-before:always" />';
			}
			++$sheetId;
		}
		return $html;
		...
		foreach ($pSheet->getDrawingCollection() as $drawing) {
			if ($drawing instanceof PHPExcel_Worksheet_Drawing) {
				if ($drawing->getCoordinates() == $coordinates) {
					...
					if (substr($filename, 0, 1) == '.' && substr($filename, 0, 2) != './') $filename = substr($filename, 1);
	...
	public function generateStyles($generateSurroundingHTML = true) {
		...
		foreach ($css as $styleName => $styleDefinition) {
			if ($styleName != 'html') $html .= '	  ' . $styleName . ' { ' . $this->_assembleCSS($styleDefinition) . ' }' . PHP_EOL;
		}
		if ($generateSurroundingHTML) $html .= '	</style>' . PHP_EOL;
		return $html;
	}

	public function buildCSS($generateSurroundingHTML = true) {
		if (is_null($this->_phpExcel)) throw new Exception('Internal PHPExcel object not set to an instance of an object.');
		if (!is_null($this->_cssStyles)) return $this->_cssStyles;
		if (!$this->_spansAreCalculated) $this->_calculateSpans();
		$css = array();
		if ($generateSurroundingHTML) {
		...
		if (!$this->_isPdf) $css['table']['page-break-after'] = 'always';
		...
		foreach ($this->_phpExcel->getCellXfCollection() as $index => $style) $css['td.style' . $index] = $this->_createCSSStyle( $style );
		$sheets = array();
		if (is_null($this->_sheetIndex)) $sheets = $this->_phpExcel->getAllSheets();
		else $sheets[] = $this->_phpExcel->getSheet($this->_sheetIndex);
		foreach ($sheets as $sheet) {
		...
		if (is_null($this->_cssStyles)) $this->_cssStyles = $css;
		return $css;
	}
	...
	private function _createCSSStyleFont(PHPExcel_Style_Font $pStyle) {
		$css = array();
		if ($pStyle->getBold()) $css['font-weight'] = 'bold';
		...
		if ($pStyle->getItalic()) $css['font-style'] = 'italic';
		...
		if (is_array($pValues)) {
			...
			foreach ($pValues as $cell) {
				...
				if (!$this->_useInlineCss) {
					...
					$cssClass = array();
					if (isset($this->_cssStyles['table.sheet' . $sheetIndex . ' td.column' . $colNum])) $this->_cssStyles['table.sheet' . $sheetIndex . ' td.column' . $colNum];
				}
				$colSpan = 1;
				$rowSpan = 1;
				$cellData = '';
				if ($cell instanceof PHPExcel_Cell) {
					if (is_null($cell->getParent())) $cell->attach($pSheet);
					if ($cell->getValue() instanceof PHPExcel_RichText) {
						...
						foreach ($elements as $element) {
							if ($element instanceof PHPExcel_RichText_Run) {
								...
								else if ($element->getFont()->getSubScript()) $cellData .= '<sub>';
							}
					...
					if (!$this->_useInlineCss) {
						...
						if (isset($this->_cssStyles['td.style' . $cell->getXfIndex()])) $cssClass = array_merge($cssClass, $this->_cssStyles['td.style' . $cell->getXfIndex()]);
				...
				if ($writeCell) {
					$html .= '			<td';
						if (!$this->_useInlineCss) {
							...
							$e = $colNum + $colSpan - 1;
							while($i++ < $e) {
								if (isset($this->_columnWidths[$sheetIndex][$i])) $width += $this->_columnWidths[$sheetIndex][$i];
							}
	...
	private function _assembleCSS($pValue = array()) {
		$pairs = array();
		foreach ($pValue as $property => $value) $pairs[] = $property . ':' . $value;
	...
	public function formatColor($pValue, $pFormat) {
		...
		if ($color !== null) $value = '<span style="color:' . $color . '">' . $value . '</span>';
		return $value;
	}
