...
class PHPExcel_Chart_DataSeries
{
	...
	public function __construct($plotType = null, $plotGrouping = null, $plotOrder = array(), $plotLabel = array(), $plotCategory = array(), $plotValues = array(), $smoothLine = null, $plotStyle = null)
	{
		...
		if ((count($plotLabel) == 0) || (is_null($plotLabel[$keys[0]]))) $plotLabel[$keys[0]] = new PHPExcel_Chart_DataSeriesValues();
		$this->_plotLabel = $plotLabel;
		if ((count($plotCategory) == 0) || (is_null($plotCategory[$keys[0]]))) $plotCategory[$keys[0]] = new PHPExcel_Chart_DataSeriesValues();
	...
	public function getPlotLabelByIndex($index) {
		...
		if (in_array($index,$keys)) return $this->_plotLabel[$index];
		elseif(isset($keys[$index])) return $this->_plotLabel[$keys[$index]];
		return false;
	}
	...
	public function getPlotCategoryByIndex($index) {
		...
		if (in_array($index,$keys)) return $this->_plotCategory[$index];
		elseif(isset($keys[$index])) return $this->_plotCategory[$keys[$index]];
		return false;
	}
	...
	public function getPlotValuesByIndex($index) {
		...
		if (in_array($index,$keys)) return $this->_plotValues[$index];
		elseif(isset($keys[$index])) return $this->_plotValues[$keys[$index]];
		return false;
	}
	...
	public function refresh(PHPExcel_Worksheet $worksheet) {
	    foreach($this->_plotValues as $plotValues) {
			if ($plotValues !== NULL) $plotValues->refresh($worksheet);
		}
