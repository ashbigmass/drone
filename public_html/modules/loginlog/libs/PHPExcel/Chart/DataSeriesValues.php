...
class PHPExcel_Chart_DataSeriesValues
{
	...
	public function setDataType($dataType = self::DATASERIES_TYPE_NUMBER) {
		if (!in_array($dataType, self::$_dataTypeValues)) throw new PHPExcel_Chart_Exception('Invalid datatype for chart data series values');
	...
	public function isMultiLevelSeries() {
		if (count($this->_dataValues) > 0) return is_array($this->_dataValues[0]);
		return null;
	}

	public function multiLevelCount() {
		$levelCount = 0;
		foreach($this->_dataValues as $dataValueSet) $levelCount = max($levelCount,count($dataValueSet));
		return $levelCount;
	}
	...
	public function getDataValue() {
		...
		elseif ($count == 1) return $this->_dataValues[0];
		return $this->_dataValues;
	}
