...
class PHPExcel
{
	...
	public function addSheet(PHPExcel_Worksheet $pSheet, $iSheetIndex = NULL) {
		...
		if($iSheetIndex === NULL) {
			...
			if ($this->_activeSheetIndex >= $iSheetIndex) ++$this->_activeSheetIndex;
		}
		return $pSheet;
	}
	...
	public function getSheetByName($pName = '') {
		...
		for ($i = 0; $i < $worksheetCount; ++$i) {
			if ($this->_workSheetCollection[$i]->getTitle() == $pName) return $this->_workSheetCollection[$i];
		}
		return null;
	}
	...
	public function getIndex(PHPExcel_Worksheet $pSheet) {
		foreach ($this->_workSheetCollection as $key => $value) {
			if ($value->getHashCode() == $pSheet->getHashCode()) return $key;
		}
	}
	...
	public function getSheetNames() {
		...
		for ($i = 0; $i < $worksheetCount; ++$i) $returnValue[] = $this->getSheet($i)->getTitle();
		return $returnValue;
	}
	...
	public function addExternalSheet(PHPExcel_Worksheet $pSheet, $iSheetIndex = null) {
		...
		foreach ($pSheet->getParent()->getCellXfCollection() as $cellXf) $this->addCellXf(clone $cellXf);
	...
	public function getNamedRange($namedRange, PHPExcel_Worksheet $pSheet = null) {
		...
		if ($namedRange != '' && ($namedRange !== NULL)) {
			if (isset($this->_namedRanges[$namedRange])) $returnValue = $this->_namedRanges[$namedRange];
	...
	public function removeNamedRange($namedRange, PHPExcel_Worksheet $pSheet = null) {
		if ($pSheet === NULL) {
			if (isset($this->_namedRanges[$namedRange])) unset($this->_namedRanges[$namedRange]);
		} else {
	...
	public function __clone() {
		foreach($this as $key => $val) {
			if (is_object($val) || (is_array($val))) $this->{$key} = unserialize(serialize($val));
		}
	}
	...
	public function getCellXfByHashCode($pValue = '') {
		foreach ($this->_cellXfCollection as $cellXf) {
			if ($cellXf->getHashCode() == $pValue) return $cellXf;
		}
		return false;
	}

	public function getDefaultStyle() {
		if (isset($this->_cellXfCollection[0])) return $this->_cellXfCollection[0];
		...
		if ($pIndex > count($this->_cellXfCollection) - 1) {
			...
			foreach ($this->_workSheetCollection as $worksheet) {
				foreach ($worksheet->getCellCollection(false) as $cellID) {
					...
					if ($xfIndex > $pIndex ) $cell->setXfIndex($xfIndex - 1);
					else if ($xfIndex == $pIndex) $cell->setXfIndex(0);
				}
			}
		}
	}
	...
	public function getCellStyleXfByHashCode($pValue = '') {
		foreach ($this->_cellXfStyleCollection as $cellStyleXf) {
			if ($cellStyleXf->getHashCode() == $pValue) return $cellStyleXf;
		}
		return false;
	}
	...
	public function garbageCollect() {
		...
		foreach ($this->_cellXfCollection as $index => $cellXf) $countReferencesCellXf[$index] = 0;
		foreach ($this->getWorksheetIterator() as $sheet) {
			...
			foreach ($sheet->getRowDimensions() as $rowDimension) {
				if ($rowDimension->getXfIndex() !== null) ++$countReferencesCellXf[$rowDimension->getXfIndex()];
			}
		...
		foreach ($this->_cellXfCollection as $index => $cellXf) {
			...
			else unset($this->_cellXfCollection[$index]);
			$map[$index] = $countNeededCellXfs - 1;
		}
		$this->_cellXfCollection = array_values($this->_cellXfCollection);
		foreach ($this->_cellXfCollection as $i => $cellXf) $cellXf->setIndex($i);
		if (empty($this->_cellXfCollection)) $this->_cellXfCollection[] = new PHPExcel_Style();
		foreach ($this->getWorksheetIterator() as $sheet) {
			foreach ($sheet->getCellCollection(false) as $cellID) {
				$cell = $sheet->getCell($cellID);
