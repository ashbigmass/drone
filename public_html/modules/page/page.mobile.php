...
class pageMobile extends pageView
{
	...
	function _getWidgetContent() {
		if($this->module_info->mcontent) {
			...
			if($interval>0) {
				if(!file_exists($cache_file) || filesize($cache_file) < 1) $mtime = 0;
	...
	function _getArticleContent() {
		...
		if($this->module_info->mskin) $templatePath = (sprintf($this->module_path.'m.skins/%s', $this->module_info->mskin));
		else $templatePath = ($this->module_path.'m.skins/default');
