...
class imageprocessModel extends imageprocess
{
	...
	function createImageFile($source_file, $resize_width, $resize_height = 0, $ipConfig) {
		...
		if(count($noresizegroup)==1) {
			...
			foreach($noresizegroup as $group) if(array_key_exists($group,$logged_info->group_list)) return;
		}
	...
	function alphaWatermark($source_file,$ipConfig) {
		...
		if(count($nowatergroup)==1) {
			...
			foreach($nowatergroup as $group) if(array_key_exists($group,$logged_info->group_list)) return;
		}
		...
		if($each_watermark[$t_mid]) $water = FileHandler::getRealPath($each_watermark[$t_mid]);
	...
	function GDrotate($file,$ext) {
		...
		if(!empty($exif['Orientation'])) {
			...
			else if($ext == "png") $image = imagecreatefrompng($file);
			else if($ext == "bmp" || $ext == "wbmp") $image = imagecreatefromwbmp($file);
			else if($ext == "gif") $image = imagecreatefromgif($file);
			switch($exif['Orientation']) {
				case 8: $image = imagerotate($image,90,0); break;
				case 3: $image = imagerotate($image,180,0); break;
				case 6: $image = imagerotate($image,-90,0); break;
			}
			if($ext == "jpg" || $ext == "jpeg") imagejpeg($image,$file);
			else if($ext == "png") imagepng($image,$file);
			else if($ext == "bmp" || $ext == "wbmp") imagewbmp($image,$file);
			else if($ext == "gif") imagegif($image,$file);
			@imagedestroy($image);
		}
	}
	...
	function getMidList($args = null) {
		...
		foreach($list as $val) $mid_list[$val->module_srl] = $val;
		return $mid_list;
	}
	...
	function _imagemagick_convert_exec($command_args, $work_path, $convert_path) {
		if (!isset($convert_path)) return FALSE;
		if (stripos(PHP_OS, 'WIN') === 0) $convert_path = 'start "ImageMagick" /D ' . escapeshellarg($work_path) . ' /B ' . escapeshellarg($convert_path);
		...
		if ($h = proc_open($command, $descriptors, $pipes, $work_path)) {
			...
			$error = '';
			while (!feof($pipes[2])) $error .= fgets($pipes[2]);
			fclose($pipes[0]);
			fclose($pipes[1]);
			fclose($pipes[2]);
			return proc_close($h);
