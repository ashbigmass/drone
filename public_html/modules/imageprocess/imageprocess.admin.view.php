...
class imageprocessAdminView extends imageprocess
{
	...
	function checkMagicPath() {
		...
		foreach($serverPath as $key) if(@file_exists($key.'/'.$exe[0]) && @file_exists($key.'/'.$exe[1]) && @file_exists($key.'/'.$exe[2])) return $key.'/';
	}
	...
	function getStampList() {
		...
		$arr=array();
		foreach ($txt as $key) {
			if(strtolower(substr(strrchr($key,'.'),1)) != 'png') continue;
