...
class addonAdminController extends addonController {
	...
	function procAddonAdminSaveActivate() {
		...
		foreach($updateList as $targetAddon) {
			$args = new stdClass();
			if(in_array($targetAddon, $pcOnList)) $args->is_used = 'Y';
			else $args->is_used = 'N';
			if(in_array($targetAddon, $mobileOnList)) $args->is_used_m = 'Y';
			else $args->is_used_m = 'N';
			if(in_array($targetAddon, $fixed)) $args->fixed = 'Y';
			else $args->fixed = 'N';
		...
		else $this->setRedirectUrl(getNotEncodedUrl('', 'module', 'admin', 'act', 'dispAddonAdminIndex'));
	}
