...
class commentItem extends Object
{
	...
	function setAttribute($attribute) {
		...
		$this->adds($attribute);
		if(count($attribute)) {
			foreach($attribute as $key => $val) $this->{$key} = $val;
		}
	}
	...
	function getHomepageUrl() {
		...
		if(!$url) return;
		if(strncasecmp('http://', $url, 7) !== 0) $url = "http://" . $url;
		...
		if($this->hasUploadedFiles()) {
			...
			foreach($file_list as $file) {
				...
				if(preg_match("/\.(jpe?g|png|gif|bmp)$/i", $file->source_filename)) {
					if(file_exists($file->uploaded_filename)) $first_image = $file->uploaded_filename;
				}
			}
