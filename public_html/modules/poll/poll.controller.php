...
class pollController extends poll {
	...
	function procPollInsert() {
		...
		foreach($vars as $key => $val) {
			if(stripos($key, 'tidx')) continue;
			$tmp_arr = explode('_', $key);
			$poll_index = $tmp_arr[1];
			if(!$poll_index) continue;
			if(!trim($val)) continue;
			if($tmp_args[$poll_index] == NULL) $tmp_args[$poll_index] = new stdClass;
			if(!is_array($tmp_args[$poll_index]->item)) $tmp_args[$poll_index]->item = array();
			if($logged_info->is_admin != 'Y') $val = htmlspecialchars($val, ENT_COMPAT | ENT_HTML401, 'UTF-8', false);
			switch($tmp_arr[0]) {
				case 'title': $tmp_args[$poll_index]->title = $val; break;
				case 'checkcount': $tmp_args[$poll_index]->checkcount = $val; break;
				case 'item': $tmp_args[$poll_index]->item[] = $val; break;
			}
		}
