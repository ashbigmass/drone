...
class pointModel extends point
{
	...
	function getPoint($member_srl, $from_db = false) {
		...
		$path = sprintf(_XE_PATH_ . 'files/member_extra_info/point/%s',getNumberingPath($member_srl));
		$cache_filename = sprintf('%s%d.cache.txt', $path, $member_srl);
		if(!$from_db && file_exists($cache_filename)) return $this->pointList[$member_srl] = trim(FileHandler::readFile($cache_filename));
		$args =new stdClass();
		$args->member_srl = $member_srl;
		$output = executeQuery('point.getPoint', $args);
	...
	function getMemberList($args = null, $columnList = array()) {
		...
		if(!$search_keyword) unset($args->is_admin, $args->is_denied, $args->selected_group_srl, $search_target);
		...
		if($args->selected_group_srl) $query_id = 'point.getMemberListWithinGroup';
		else $query_id = 'point.getMemberList';
		$output = executeQuery($query_id, $args, $columnList);
		if($output->total_count) {
			$oModuleModel = getModel('module');
			$config = $oModuleModel->getModuleConfig('point');
			foreach($output->data as $key => $val) $output->data[$key]->level = $this->getLevel($val->point, $config->level_step);
		}
		return $output;
	}
}
