...
class autoinstallModel extends autoinstall
{
	...
	function getCategoryList() {
		...
		foreach($categoryList as $key => $category) {
			if($category->parent_srl) $categoryList[$category->parent_srl]->children[] = & $categoryList[$key];
			else $depth0[] = $key;
		}
		$resultList = array();
	...
	function setDepth(&$item, $depth, &$list, &$resultList) {
		...
		foreach($item->children as $child) $siblingList .= "," . $this->setDepth($list[$child->category_srl], $depth + 1, $list, $resultList);
		if(count($item->children) < 1) $item->nPackages = $this->getPackageCount($item->category_srl);
	...
	function getInstalledPackages($package_list) {
		...
		foreach($output->data as $value) $result[$value->package_srl] = $value;
		return $result;
	}
	...
	function getInstalledPackageList($page) {
		...
		$args->list_count = 10;
		$args->page_count = 5;
		if(Context::getDBType() == 'mssql') $args->sort_index = 'package_srl';
		$output = executeQueryArray("autoinstall.getInstalledPackageList", $args);
		$res = array();
		if($output->data) {
			foreach($output->data as $val) $res[$val->package_srl] = $val;
		}
		$output->data = $res;
		return $output;
	}
	...
	function getTypeFromPath($path) {
		...
		if(!$target_name) $target_name = array_pop($path_array);
	...
	function getConfigFilePath($type) {
		...
		switch($type) {
			...
			case "layout":
			case "widget":
			case 'theme': $config_file = "/conf/info.xml"; break;
			case "component": $config_file = "/info.xml"; break;
			case "m.skin":
			case "skin":
			case "widgetstyle":
			case "style": $config_file = "/skin.xml"; break;
			case "drcomponent": $config_file = "/info.xml"; break;
		}
		return $config_file;
	}
