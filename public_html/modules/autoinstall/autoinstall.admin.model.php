...
class autoinstallAdminModel extends autoinstall
{
	...
	function preProcParam(&$order_target, &$order_type, &$page) {
		...
		if(!isset($order_target_array[$order_target])) $order_target = 'newest';
		$order_type_array = array('asc' => 1, 'desc' => 1);
		if(!isset($order_type_array[$order_type])) $order_type = 'desc';
		$page = (int) $page;
		if($page < 1) $page = 1;
	}
	...
	function getAutoinstallAdminLayoutPackageList() {
		...
		if(!isset($type_array[$type])) $type = 'P';
		if($type == 'P') $category_srl = $this->layout_category_srl;
		else $category_srl = $this->mobile_layout_category_srl;
		$this->preProcParam($order_target, $order_type, $page);
	...
	function getAutoinstallAdminSkinPackageList() {
		...
		$type = Context::get('type');
		if(!isset($type_array[$type])) $type = 'P';
		if($type == 'P') $category_srl = $this->module_skin_category_srl;
		else $category_srl = $this->module_mobile_skin_category_srl;
		$this->preProcParam($order_target, $order_type, $page);
	...
	function getAutoinstallAdminIsAuthed() {
		...
		if($output->toBool()==TRUE) {
			...
			if(!$ftp_info->ftp_root_path) $is_authed = -1;
			else $is_authed = (int) isset($_SESSION['ftp_password']);
		}
	...
	public function getAutoInstallAdminInstallInfo() {
		...
		if(!$packageSrl) return new Object(-1, 'msg_invalid_request');
	...
	public function isWritableDir($path) {
		...
		while($path_list) {
			...
			if(FileHandler::isDir($check_path)) break;
			array_pop($path_list);
		}
