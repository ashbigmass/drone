...
class autoinstallAdminController extends autoinstall
{
	...
	function _updateinfo() {
		...
		if($item) $params["updatedate"] = $item->updatedate;
	...
	function checkInstalled() {
		...
		foreach($packages as $package) {
			...
			if($type == "core") {
				...
				$config_file = NULL;
				switch($type) {
					case "m.layout": $type = "layout";
					case "module":
					case "addon":
					case "layout":
					case "widget": $config_file = "/conf/info.xml"; break;
					case "component": $config_file = "/info.xml"; break;
					case "style":
					case "m.skin": $type = "skin";
					case "skin":
					case "widgetstyle": $config_file = "/skin.xml"; break;
					case "drcomponent":
			...
			if(version_compare($args->version, $args->current_version, ">")) $args->need_update = "Y";
			else $args->need_update = "N";
	...
	function procAutoinstallAdminPackageinstall() {
		...
		if(!$_SESSION['ftp_password']) $ftp_password = Context::get('ftp_password');
		else $ftp_password = $_SESSION['ftp_password'];
		$isSftpSupported = function_exists(ssh2_sftp);
		foreach($packages as $package_srl) {
			$package = $oModel->getPackage($package_srl);
			if($oAdminModel->checkUseDirectModuleInstall($package)->toBool()) $oModuleInstaller = new DirectModuleInstaller($package);
			else if($ftp_info->sftp && $ftp_info->sftp == 'Y' && $isSftpSupported) $oModuleInstaller = new SFTPModuleInstaller($package);
			else if(function_exists(ftp_connect)) $oModuleInstaller = new PHPFTPModuleInstaller($package);
			else $oModuleInstaller = new FTPModuleInstaller($package);
	...
	function updatePackages(&$xmlDoc) {
		...
		foreach($xmlDoc->response->packages->item as $item) {
			$args = new stdClass();
			foreach($targets as $target) $args->{$target} = $item->{$target}->body;
	...
	function procAutoinstallAdminUninstallPackage() {
		...
		if($output->toBool()==FALSE) return $output;
		if(Context::get('return_url')) $this->setRedirectUrl(Context::get('return_url'));
		else $this->setRedirectUrl(getNotEncodedUrl('', 'module', 'admin', 'act', 'dispAutoinstallAdminInstalledPackages'));
	}
