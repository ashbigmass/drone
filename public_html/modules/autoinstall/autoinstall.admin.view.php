...
class autoinstallAdminView extends autoinstall
{
	...
	function rearrange(&$item, &$targets) {
		$ret = new stdClass();
		foreach($targets as $target) $ret->{$target} = $item->{$target}->body;
		return $ret;
	}
	...
	function rearranges($items, $packages = null) {
		...
		foreach($items as $item) $targetpackages[$item->package_srl->body] = 0;
		$oModel = getModel('autoinstall');
		if($package == null) $packages = $oModel->getInstalledPackages(array_keys($targetpackages));
		$depto = array();
		foreach($items as $item) {
			...
			if($packages[$v->package_srl]) {
				...
				if($this->ftp_set && $v->depfrom) {
					...
					foreach($depfrom as $package_srl) $depto[$package_srl][] = $v->package_srl;
				}
				if($v->type == "core") $v->avail_remove = FALSE;
				else if($v->type == "module") $v->avail_remove = $oModel->checkRemovable($packages[$v->package_srl]->path);
	...
	function dispAutoinstallAdminInstalledPackages() {
		...
		if($xmlDoc && $xmlDoc->response->packagelist->item) {
			...
			foreach($package_list as $package_srl => $package) {
				if($item_list[$package_srl]) $res[] = $item_list[$package_srl];
			}
	...
	function dispAutoinstallAdminInstall() {
		...
		if(!$_SESSION['ftp_password']) Context::set('need_password', TRUE);
		$output = $oAdminModel->checkUseDirectModuleInstall($package);
		if($output->toBool()==TRUE) Context::set('show_ftp_note', FALSE);
	...
	function dispAutoinstallAdminIndex() {
		...
		if(!$updateDate) return $this->stop('msg_connection_fail');
		$oModel = getModel('autoinstall');
		...
		$params["page"] = $page;
		if($search_keyword) $params["search_keyword"] = $search_keyword;
