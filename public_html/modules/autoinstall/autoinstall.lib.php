...
class ModuleInstaller
{
	...
	function installModule() {
		...
		if($type == "module") {
			...
			if($oModuleModel->checkNeedInstall($target_name)) $oInstallController->installModule($target_name, $module_path);
			if($oModuleModel->checkNeedUpdate($target_name)) {
				$oModule = getModule($target_name, 'class');
				if(method_exists($oModule, 'moduleUpdate')) $oModule->moduleUpdate();
			}
		}
	}
...
class SFTPModuleInstaller extends ModuleInstaller
{
	...
	function _connect() {
		if(!function_exists('ssh2_connect')) return new Object(-1, 'msg_sftp_not_supported');
		if(!$this->ftp_info->ftp_user || !$this->ftp_info->sftp || $this->ftp_info->sftp != 'Y') return new Object(-1, 'msg_ftp_invalid_auth_info');
		if($this->ftp_info->ftp_host) $ftp_host = $this->ftp_info->ftp_host;
		else $ftp_host = "127.0.0.1";
		$this->connection = ssh2_connect($ftp_host, $this->ftp_info->ftp_port);
		if(!@ssh2_auth_password($this->connection, $this->ftp_info->ftp_user, $this->ftp_password)) return new Object(-1, 'msg_ftp_invalid_auth_info');
	...
	function _removeFile($path) {
		...
		if(!@ssh2_sftp_unlink($this->sftp, $target_path)) return new Object(-1, sprintf(Context::getLang('msg_delete_file_failed'), $path));
		return new Object();
	}
	...
	function _removeDir_real($path) {
		if(substr($path, 0, 2) == "./") $path = substr($path, 2);
		$target_path = $this->ftp_info->ftp_root_path . $path;
		if(!@ssh2_sftp_rmdir($this->sftp, $target_path)) return new Object(-1, sprintf(Context::getLang('msg_delete_dir_failed'), $path));
		return new Object();
	}
	...
	function _copyDir(&$file_list) {
		...
		if(is_array($file_list)) {
			foreach($file_list as $k => $file) {
				$org_file = $file;
				if($this->package->path == ".") $file = substr($file, 3);
				$path = FileHandler::getRealPath("./" . $this->target_path . "/" . $file);
				$pathname = dirname($target_dir . "/" . $file);
				if(!file_exists(FileHandler::getRealPath($real_path))) ssh2_sftp_mkdir($this->sftp, $pathname, 0755, TRUE);
...
class PHPFTPModuleInstaller extends ModuleInstaller
{
	...
	function _connect() {
		...
		if(!$this->connection) return new Object(-1, sprintf(Context::getLang('msg_ftp_not_connected'), $ftp_host));
		...
		$_SESSION['ftp_password'] = $this->ftp_password;
		if($this->ftp_info->ftp_pasv != "N") ftp_pasv($this->connection, TRUE);
		return new Object();
	}
	...
	function _removeFile($path) {
		if(substr($path, 0, 2) == "./") $path = substr($path, 2);
		$target_path = $this->ftp_info->ftp_root_path . $path;
		if(!@ftp_delete($this->connection, $target_path)) return new Object(-1, "failed to delete file " . $path);
		return new Object();
	}
	...
	function _removeDir_real($path) {
		if(substr($path, 0, 2) == "./") $path = substr($path, 2);
		$target_path = $this->ftp_info->ftp_root_path . $path;
		if(!@ftp_rmdir($this->connection, $target_path)) return new Object(-1, "failed to delete directory " . $path);
		return new Object();
	}
	...
	function _copyDir(&$file_list) {
		...
		if(substr($this->download_path, -1) == '/') $this->download_path = substr($this->download_path, 0, -1);
		$target_dir = $this->ftp_info->ftp_root_path . $this->target_path;
		if(is_array($file_list)) {
			foreach($file_list as $k => $file) {
				if(!$file) continue;
				...
				for($i = 0; $i < count($path_list); $i++) {
					...
					if(!file_exists(FileHandler::getRealPath($real_path))) {
						if(!@ftp_mkdir($this->connection, $ftp_path)) return new Object(-1, "msg_make_directory_failed");
						if(strtoupper(substr(PHP_OS, 0, 3)) !== 'WIN') {
							if(function_exists('ftp_chmod')) {
								if(!ftp_chmod($this->connecton, 0755, $ftp_path)) return new Object(-1, "msg_permission_adjust_failed");
							} else {
								if(!ftp_site($this->connection, "CHMOD 755 " . $ftp_path)) return new Object(-1, "msg_permission_adjust_failed");
							}
						}
					}
				}
...
class FTPModuleInstaller extends ModuleInstaller
{
	...
	function _connect() {
		...
		$this->oFtp = new ftp();
		if(!$this->oFtp->ftp_connect($ftp_host, $this->ftp_info->ftp_port)) return new Object(-1, sprintf(Context::getLang('msg_ftp_not_connected'), $ftp_host));
	...
	function _removeFile($path) {
		if(substr($path, 0, 2) == "./") $path = substr($path, 2);
		$target_path = $this->ftp_info->ftp_root_path . $path;
		if(!$this->oFtp->ftp_delete($target_path)) return new Object(-1, sprintf(Context::getLang('msg_delete_file_failed'), $path));
		return new Object();
	}
	...
	function _removeDir_real($path) {
		if(substr($path, 0, 2) == "./") $path = substr($path, 2);
		$target_path = $this->ftp_info->ftp_root_path . $path;
		if(!$this->oFtp->ftp_rmdir($target_path)) return new Object(-1, sprintf(Context::getLang('msg_delete_dir_failed'), $path));
		return new Object();
	}
...
class DirectModuleInstaller extends ModuleInstaller
{
	...
	function _removeFile($path) {
		...
		if(!FileHandler::removeFile($target_path)) return new Object(-1, sprintf(Context::getLang('msg_delete_file_failed'), $path));
		return new Object();
	}
		...
		if(is_array($file_list)) {
			foreach($file_list as $k => $file) {
				...
				for($i = 0; $i < count($path_list); $i++) {
					if($path_list == "") continue;
					$real_path .= $path_list[$i] . "/";
					if(!file_exists(FileHandler::getRealPath($real_path))) FileHandler::makeDir($real_path);
				}
