...
class ggmailingController extends ggmailing
{
	...
	function triggerNotiliteGgmailing(&$obj) {
		...
		switch($obj->type) {
			case 'D': $type = '글'; break;
			case 'C': $type = '댓글'; break;
			case 'E': $type = '쪽지'; break;
			case 'T': $type = '테스트'; break;
		}
		...
		switch($obj->target_type) {
			...
			break;
			case 'E':
				if(version_compare(__XE_VERSION__, '1.7.4', '>=')) $str = sprintf('<strong>%s</strong>님께서 <strong>"%s"</strong> 메세지를 보내셨습니다.',$target_member, $obj->target_summary);
				else $str = sprintf('<strong>%d</strong>개의 읽지 않은 <strong>메시지</strong>가 있습니다.', $obj->target_summary);
			break;
			case 'P':
			...
			break;
			case 'S':
				if($obj->target_browser) $str = sprintf('<strong>%s</strong>님이 <strong>"%s"</strong>게시판에 <strong>"%s"</strong>글을 남겼습니다.', $target_member, $obj->target_browser, $obj->target_summary);
				else $str = sprintf('<strong>%s</strong>님이 <strong>"%s"</strong>글을 남겼습니다.', $target_member, $obj->target_summary);
			break;
			case 'V':
		...
		if($obj->receive_nickname != '' && $obj->receive_email != '') $output = executeQuery('ggmailing.insertGgmailingAdminSend',$obj);
