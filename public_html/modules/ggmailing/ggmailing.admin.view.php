...
class ggmailingAdminView extends ggmailing
{
	...
	function dispGgmailingAdminDonotsend() {
		...
		if($args->search_type=='ggmailing_email') $args->ggmailing_email = $args->search_keyword;
		elseif($args->search_type=='ggmailing_nickname') $args->ggmailing_nickname = $args->search_keyword;
		elseif($args->search_type=='ggmailing_member_srl') $args->ggmailing_member_srl = $args->search_keyword;
	...
	function dispGgmailingAdminBoardMailing() {
		...
		if(!$args->page) $args->page = '1';
		Context::set('args',$args);
		if($args->search_type=='ggemail') $args->ggmailing_email = $args->search_keyword;
		elseif($args->search_type=='ggmid') $args->ggmailing_mid = $args->search_keyword;
	...
	function dispGgmailingAdminInsertmembers() {
		...
		if($args->search_type=='nickname') $args->ggmailing_nickname = $args->search_keyword;
		elseif($args->search_type=='email') $args->ggmailing_email = $args->search_keyword;
		$oGgmailingAdminModel = &getAdminModel('ggmailing');
	...
	function dispGgmailingAdminInsert() {
		...
		if($is_update) {
			...
			foreach($output->data as $key => $val) Context::set('args', $val);
			$output = $oGgmailingAdminModel->getGgmailingAdminGateway($args);
			foreach($output->data as $key => $val) Context::set('gw_args',$val);
		}
