...
class menuAdminModel extends menu
{
	...
	function getMenuAdminItemInfo() {
		...
		if(is_array($menuItem->group_srls) && count($menuItem->group_srls) > 0) {
			if($menuItem->group_srls[0] == -1) $menuItem->grant = 'member';
			else if($menuItem->group_srls[0] == -3) $menuItem->grant = 'manager';
			else $menuItem->grant = 'group';
		}
	...
	function getModuleListInSitemap($site_srl = 0) {
		...
		if(is_array($output->data)) {
			foreach($output->data as $value) {
				if($value->instanceCount > 1) $moduleList[] = $value->module;
			}
		}
	...
	public function getMenuAdminSiteMap() {
		...
		if($menuSrl) {
			...
			if(count($menu->list)>0) {
				foreach($menu->list AS $key=>$value) $this->_menuInfoSetting($menu->list[$key], $start_module, $isMenuFixed, $menuSrl,$siteSrl);
			}
			if($isMenuFixed) $oMenuAdminController->makeXmlFile($menuSrl);
			...
			if(is_array($menuListFromDB)) {
				...
				foreach($menuListFromDB AS $key=>$value) {
					if($value->title == $oAdmin->getAdminMenuName()) unset($output[$key]);
						...
						if(!file_exists($value->php_file)) $oMenuAdminController->makeXmlFile($value->menu_srl);
						include($value->php_file);
						$isMenuFixed = false;
						if(count($menu->list) > 0) {
							foreach($menu->list AS $key2=>$value2) $this->_menuInfoSetting($menu->list[$key2], $start_module, $isMenuFixed, $value->menu_srl,$siteSrl);
						}
						if($isMenuFixed) $oMenuAdminController->makeXmlFile($value->menu_srl);
						$menuItems = new stdClass();
						$menuItems->menuSrl = $value->menu_srl;
						$menuItems->title = $value->title;
						$menuItems->menuItems = $menu;
						if($value->menu_srl == $this->menuSrlWithinHome) $menuList[-1] = $menuItems;
	...
	public function getMenuAdminDetailSetup() {
		$menuItemSrl = Context::get('menu_item_srl');
		if(!$menuItemSrl) return new Object(-1, 'msg_invalid_request');
		$menuItemInfo = $this->getMenuItemInfo($menuItemSrl);
		if($menuItemInfo->is_shortcut == 'Y') return new Object(-1, 'msg_invalid_request');
		$oModuleModel = getModel('module');
		...
		if($moduleConfInfo->simple_setup_index_act) {
			...
			if($simpleSetupHtml) $this->add('simpleSetupHtml', $simpleSetupHtml);
		}
	...
	private function _menuInfoSetting(&$menu, &$start_module, &$isMenuFixed, $menuSrl,$siteSrl = 0) {
		...
		if($menu['is_shortcut'] != 'Y') {
			...
			if($midInfo) {
				...
				if($midInfo->page_type) $menu['module_type'] = $midInfo->page_type;
				else $menu['module_type'] = $midInfo->module;
			}
			if($moduleInfo->setup_index_act) $menu['setup_index_act'] = $moduleInfo->setup_index_act;
			else if($moduleInfo->default_index_act) $menu['setup_index_act'] = $moduleInfo->default_index_act;
