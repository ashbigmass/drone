...
class editorModel extends editor
{
	...
	function getEditorConfig($module_srl = null) {
		...
		if(!$editor_config->editor_height) $editor_config->editor_height = ($editor_default_config->editor_height) ? $editor_default_config->editor_height : 500;
		if(!$editor_config->comment_editor_height) $editor_config->comment_editor_height = ($editor_default_config->comment_editor_height) ? $editor_default_config->comment_editor_height : 120;
		if(!$editor_config->editor_skin) $editor_config->editor_skin = ($editor_default_config->editor_skin) ? $editor_default_config->editor_skin : 'ckeditor';
		if(!$editor_config->comment_editor_skin) $editor_config->comment_editor_skin = ($editor_default_config->comment_editor_skin) ? $editor_default_config->comment_editor_skin : 'ckeditor';
		if(!$editor_config->content_style) $editor_config->content_style = ($editor_default_config->content_style) ? $editor_default_config->content_style : 'ckeditor_light';
	...
	function getEditor($upload_target_srl = 0, $option = null) {
		if($upload_target_srl) $option->editor_sequence = $upload_target_srl;
		...
		if($skin=='dreditor') $this->loadDrComponents();
		if($enable_autosave) {
	...
	function getComponentList($filter_enabled = true, $site_srl=0, $from_db=false) {
		...
		include($cache_file);
		$logged_info = Context::get('logged_info');
		if($logged_info && is_array($logged_info->group_list)) $group_list = array_keys($logged_info->group_list);
			...
			foreach($component_list as $key => $val) {
				...
				if($val->target_group) {
					if(!$logged_info) {
						...
						$is_granted = false;
						foreach($group_list as $group_srl) if(in_array($group_srl, $val->target_group)) $is_granted = true;
	...
	function getComponentXmlInfo($component) {
		...
		if($extra_vars) {
			...
			if(!$extra_var_groups) $extra_var_groups = $extra_vars;
