...
class ncenterliteView extends ncenterlite
{
	...
	function dispNcenterliteUserConfig() {
		...
		if($logged_info->is_admin == 'Y') $member_info = $oMemberModel->getMemberInfoByMemberSrl($member_srl);
		if($logged_info->is_admin != 'Y' && $member_srl) {
			if($member_srl != $logged_info->member_srl) return new Object(-1, 'ncenterlite_stop_no_permission_other_user');
		}
		$oNcenterliteModel = getModel('ncenterlite');
