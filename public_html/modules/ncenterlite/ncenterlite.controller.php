...
class ncenterliteController extends ncenterlite
{
	function procNcenterliteUserConfig() {
		...
		if(!$output) $outputs = executeQuery('ncenterlite.insertUserConfig', $args);
		else $outputs = executeQuery('ncenterlite.updateUserConfig', $args);
	...
	function triggerAfterDeleteMember($obj) {
		...
		if(!$output->toBool()) {
			...
			if(file_exists($flag_path)) FileHandler::removeFile($flag_path);
		}
		return new Object();
	}
	...
	function triggerAfterInsertDocument(&$obj) {
		...
		if(!empty($mention_targets)) {
			if(!$mention_targets && !count($mention_targets) || !isset($config->use['mention'])) return new Object();
			foreach($mention_targets as $mention_member_srl) {
				...
				if($notify_member_config->mention_notify == 'N') continue;
				$args = new stdClass();
		...
		if(isset($config->use['admin_content']) && is_array($config->admin_notify_module_srls) && in_array($module_info->module_srl, $config->admin_notify_module_srls) && empty($mention_targets)) {
			foreach($admin_list as $admins) {
				if($logged_info->member_srl == $admins) continue;
				$args = new stdClass();
	...
	function triggerAfterInsertComment(&$obj) {
		...
		if(isset($config->use['admin_content']) && is_array($config->admin_notify_module_srls) && in_array($module_info->module_srl, $config->admin_notify_module_srls)) {
			foreach($admin_list as $admins) {
				...
				$args->notify = $this->_getNotifyId($args);
				$output = $this->_insertNotify($args, $is_anonymous);
				if($output->toBool()) $admin_comment_notify = true;
			}
		}
		if(isset($config->use['mention'])) {
			...
			foreach($mention_targets as $mention_member_srl) {
				...
				$notify_member_config = $target_member_config->data;
				if($notify_member_config->mention_notify == 'N') continue;
				if(is_array($admin_list) && in_array($mention_member_srl, $admin_list) && isset($config->use['admin_content']) && $admin_comment_notify == true) continue;
				$args = new stdClass();
				$args->member_srl = $mention_member_srl;
				$args->target_p_srl = $obj->comment_srl;
				$args->srl = $obj->document_srl;
		...
		if(!isset($config->use['comment'])) return new Object();
		if($parent_srl) {
	...
	function triggerAfterSendMessage(&$trigger_obj) {
		$oNcenterliteModel = getModel('ncenterlite');
		$config = $oNcenterliteModel->getConfig();
		if(!isset($config->use['message'])) return new Object();
	...
	function triggerAfterVotedupdate(&$obj) {
		...
		$oNcenterliteModel = getModel('ncenterlite');
		$config = $oNcenterliteModel->getConfig();
		if(!isset($config->use['vote'])) return new Object();
	...
	function triggerAfterDeleteComment(&$obj) {
		$oNcenterliteModel = getModel('ncenterlite');
		$config = $oNcenterliteModel->getConfig();
		if(empty($config->use)) return new Object();
		$args = new stdClass();
	...
	function triggerAfterModuleHandlerProc(&$oModule) {
		...
		if($oModule->module == 'beluxe' && Context::get('is_modal')) $this->_hide_ncenterlite = true;
		if($oModule->module == 'bodex' && Context::get('is_iframe')) $this->_hide_ncenterlite = true;
		if($oModule->getLayoutFile() == 'popup_layout.html') $this->_hide_ncenterlite = true;
			...
			if($comment_srl && $document_srl && $oDocument) {
				...
				if($_comment_list) {
					if(array_key_exists($comment_srl, $_comment_list)) {
						...
						if($cpage > 1) $url = getNotEncodedUrl('cpage', $cpage - 1) . '#comment_' . $comment_srl;
						else $url = getNotEncodedUrl('_comment_srl', '', 'cpage', '') . '#comment_' . $comment_srl;
					}
					$url = str_replace('&amp;', '&', $url);
					header('location: ' . $url);
					Context::close();
	...
	function _addFile() {
		if(file_exists(FileHandler::getRealPath($this->template_path . 'ncenterlite.css'))) Context::loadFile(array($this->template_path . 'ncenterlite.css', '', '', 100), true);
		$oNcenterliteModel = getModel('ncenterlite');
		$config = $oNcenterliteModel->getConfig();
		if(!Mobile::isFromMobilePhone()) {
		...
		if($config->zindex) Context::set('ncenterlite_zindex', ' style="z-index:' . $config->zindex . ';" ');
	}
	...
	function _getTemplate() {
		...
		$result = '';
		if(Mobile::isFromMobilePhone()) $path = sprintf('%sm.skins/%s/', $this->module_path, $config->mskin);
		else $path = sprintf('%sskins/%s/', $this->module_path, $config->skin);
	...
	function _insertNotify($args, $anonymous = FALSE) {
		...
		if (!$args->notify) $args->notify = $this->_getNotifyId($args);
		if (!$args->regdate) $args->regdate = date('YmdHis');
		if($anonymous == TRUE) {
		...
		if($output->toBool()) {
			...
			if(!$trigger_notify->toBool()) return $trigger_notify;
		}
		$flag_path = _XE_PATH_ . 'files/cache/ncenterlite/new_notify/' . getNumberingPath($args->member_srl) . $args->member_srl . '.php';
		if(file_exists($flag_path)) FileHandler::removeFile($flag_path);
		return $output;
	...
	public static function updateFlagFile($member_srl = null, $output) {
		...
		if(file_exists($flag_path)) return;
	...
	function _getMentionTarget($content) {
		...
		foreach ($mentions as $mention) {
			if (isset($members[$mention])) continue;
			if (count($members) >= $config->mention_limit) break;
			if ($config->mention_suffix_always_cut != 'Y') {
				if ($config->mention_names === 'id') $member_srl = $oMemberModel->getMemberSrlByUserID($mention);
				else $member_srl = $oMemberModel->getMemberSrlByNickName($mention);
			} else {
				$member_srl = null;
			}
			if (!$member_srl) {
				foreach ($config->mention_suffixes as $suffix) {
					if (($pos = strpos($mention, $suffix)) !== false && $pos > 0) $mention = substr($mention, 0, $pos);
				}
				if (isset($members[$mention])) continue;
				elseif ($config->mention_names === 'id') $member_srl = $oMemberModel->getMemberSrlByUserID($mention);
				else $member_srl = $oMemberModel->getMemberSrlByNickName($mention);
			}
			if (!$member_srl || ($logged_info && ($member_srl == $logged_info->member_srl))) continue;
