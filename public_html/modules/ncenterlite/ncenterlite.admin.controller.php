...
class ncenterliteAdminController extends ncenterlite
{
	function procNcenterliteAdminInsertConfig() {
		...
		$config_vars = array(
			'use', 'display_use', 'user_config_list', 'mention_names', 'mention_suffixes',
			'mention_suffix_always_cut', 'mention_limit', 'hide_module_srls', 'admin_notify_module_srls', 'skin',
			'mskin', 'mcolorset', 'colorset', 'zindex', 'anonymous_name',
			'document_read', 'layout_srl', 'mlayout_srl',
		);
		foreach($config_vars as $val) if($obj->{$val}) $config->{$val} = $obj->{$val};
		if ($obj->disp_act == 'dispNcenterliteAdminIndexConfig') if (!$obj->use) $config->use = array();
		if ($obj->disp_act == 'dispNcenterliteAdminAdvancedconfig') {
			if (!$config->mention_suffixes) $config->mention_suffixes = array();
			else if (!is_array($config->mention_suffixes)) $config->mention_suffixes = array_map('trim', explode(',', $config->mention_suffixes));
		}
		if ($obj->disp_act == 'dispNcenterliteAdminSeletedmid') {
			if (!$obj->hide_module_srls) $config->hide_module_srls = array();
			if (!$obj->admin_notify_module_srls) $config->admin_notify_module_srls = array();
		}
		$output = $oModuleController->insertModuleConfig('ncenterlite', $config);
		if(!$output->toBool()) return new Object(-1, 'ncenterlite_msg_setting_error');
	...
	function procNcenterliteAdminDeleteNofity() {
		...
		$args = new stdClass;
		if($old_date) $args->old_date = $old_date;
