...
class ncenterliteModel extends ncenterlite
{
	...
	function getConfig() {
		if(self::$config === NULL) {
			...
			if(!$config->use) $config->use = array('message' => 1);
			if(!$config->display_use) $config->display_use = 'all';
			if(!$config->mention_names) $config->mention_names = 'nick_name';
			if(!$config->mention_suffixes) $config->mention_suffixes = array('님', '様', 'さん', 'ちゃん');
			unset($config->mention_format);
			if(!isset($config->mention_limit)) $config->mention_limit = 20;
	...
	function replaceNotifyType($match) {
		if(!in_array($match[1],$this->notify_arguments)) return $match[0];
		if(!isset($this->notify_args->{$match[1]})) return $match[0];
	...
	function getMyNotifyList($member_srl=null, $page=1, $readed='N') {
		$act = Context::get('act');
		if($act=='dispNcenterliteNotifyList') $output = $this->getMyDispNotifyList($member_srl);
		else $output = $this->_getMyNotifyList($member_srl, $page, $readed);
	...
	function getMyNotifyListTpl() {
		...
		foreach($tmp->data as $key => $obj) $tmp->data[$key]->url = str_replace('&amp;', '&', $obj->url);
		$list = new stdClass();
		$list->data = $tmp->data;
	...
	function getMemberAdmins() {
		...
		$member_srl = array();
		foreach($output->data as $member) $member_srl[] = $member->member_srl;
		return $member_srl;
	}
		...
		switch ($notification->type) {
			...
			case 'Z':
				return vsprintf($lang->{$notification->target_body}, array(
					$notification->target_member_srl,
					$notification->target_nick_name,
					$notification->target_user_id,
					$notification->target_email_address,
					$notification->target_browser,
					$notification->target_summary,
	...
	function getAgo($datetime) {
		...
		for($i = 0; $i < 6; $i++) {
			...
			if($current[$p[$i]] - $date[$p[$i]] > 1) {
				...
				if($lang_type == 'en') return $value . ' ' . $display[$i] . (($value != 1) ? 's' : '') . ' ' . $ago;
