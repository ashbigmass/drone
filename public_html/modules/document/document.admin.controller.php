...
class documentAdminController extends document
{
	...
	function moveDocumentModule($document_srl_list, $module_srl, $category_srl) {
		...
		for($i=count($document_srl_list)-1;$i>=0;$i--) {
			...
			if($module_srl != $obj->module_srl) $oDocumentController->deleteDocumentAliasByDocument($obj->document_srl);
	...
	function copyDocumentModule($document_srl_list, $module_srl, $category_srl) {
		...
		if(is_array($extraVarsList->data)) {
			foreach($extraVarsList->data as $value) {
				if(!isset($extraVarsListByDocumentSrl[$value->document_srl])) $extraVarsListByDocumentSrl[$value->document_srl] = array();
		...
		for($i=count($document_srl_list)-1;$i>=0;$i--) {
			...
			if($module_srl == $obj->module_srl) {
				if(is_array($extraVars)) {
					foreach($extraVars as $extraItem) if($extraItem->var_idx >= 0) $obj->{'extra_vars'.$extraItem->var_idx} = $extraItem->value;
				}
			}
			$obj->module_srl = $module_srl;
			...
			if(is_array($extraVars)) {
				foreach($extraVars as $value) {
					if($value->idx >= 0 && $value->lang_code == Context::getLangType()) continue;
	...
	function deleteModuleDocument($module_srl) {
		...
		if(is_array($documents)) {
			foreach ($documents as $oDocument) $document_srl_list[] = $oDocument->document_srl;
		}
		$oCacheHandler = CacheHandler::getInstance('object');
		if($oCacheHandler->isSupport()) {
	...
	function procDocumentAdminInsertExtraVar() {
		...
		if(!$output->toBool() || $output->data->count) return new Object(-1, 'msg_extra_name_exists');
		$oDocumentController = getController('document');
	...
	function procDocumentAdminMoveToTrash() {
		...
		if(!$returnUrl) {
			...
			if($arrUrl['query']) {
				...
				if(isset($arrQuery['document_srl'])) unset($arrQuery['document_srl']);
				$searchArgs = new stdClass;
				foreach($arrQuery as $key=>$val) $searchArgs->{$key} = $val;
				if(!isset($searchArgs->sort_index)) $searchArgs->sort_index = $module_info->order_target;
				foreach($module_info as $key=>$val) {
					if(!isset($searchArgs->{$key})) $searchArgs->{$key} = $val;
				}
				$oDocumentModel = getModel('document');
				$output = $oDocumentModel->getDocumentList($searchArgs, $module_info->except_notice, TRUE, array('document_srl'));
				$cur_page = 1;
				if(isset($arrQuery['page'])) $cur_page = (int)$arrQuery['page'];
				if($cur_page>1 && count($output->data) == 0) $arrQuery['page'] = $cur_page - 1;
				$query = "?";
				foreach($arrQuery as $key=>$val) $query .= sprintf("%s=%s&", $key, $val);
