		...
		if($output->data) {
			...
			foreach($output->data as $file) {
				...
				if(!$file->upload_target_type) {
					if($document_list[$target_srl]) $file->upload_target_type = 'doc';
					else if($comment_list[$target_srl]) $file->upload_target_type = 'com';
					else if($module_list[$target_srl]) $file->upload_target_type = 'mod';
					else {
						...
						if($file_update_args->upload_target_type) executeQuery('file.updateFileTargetType', $file_update_args);
					}
					for($i = 0; $i < $com_srls_count; ++$i) if($comment_list[$com_srls[$i]]) delete($com_srls[$i]);
					for($i = 0; $i < $doc_srls_count; ++$i) if($document_list[$doc_srls[$i]]) delete($doc_srls[$i]);
					for($i = 0; $i < $mod_srls_count; ++$i) if($module_list[$mod_srls[$i]]) delete($mod_srls[$i]);
				}
				if($file->upload_target_type) {
					if(!in_array($file->upload_target_srl, ${$file->upload_target_type.'_srls'})) ${$file->upload_target_type.'_srls'}[] = $target_srl;
				}
			...
			if($doc_srls_count) {
				...
				if(is_array($document_output)) {
					foreach($document_output as $document) $document_list[$document->document_srl] = $document;
				}
			}
			...
			foreach($file_list as $srl => $file) {
				if($file->upload_target_type == 'com') $file_list[$srl]->target_document_srl = $comment_list[$file->upload_target_srl]->document_srl;
			}
		}
