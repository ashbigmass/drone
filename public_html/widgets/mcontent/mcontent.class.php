...
class mcontent extends WidgetHandler {
	...
	function proc($args) {
		...
		if($args->content_type == 'rss') {
			...
			for($i=0,$c=count($rss_urls);$i<$c;$i++) if($rss_urls[$i]) $args->rss_urls[] = $rss_urls[$i];
		} else {
			$obj = new stdClass();
			if(!$args->module_srls) {
	...
	function _getDocumentItems($args) {
		...
		if($output->toBool() && $output->data) {
			foreach($output->data as $key => $val) $category_lists[$val->module_srl][$val->category_srl] = $val;
		}
		$obj->module_srl = $args->module_srl;
		$obj->sort_index = $args->order_target;
