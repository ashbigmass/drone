...
class content extends WidgetHandler {
	...
	function proc($args) {
		...
		if($args->tab_type == 'none' || $args->tab_type == '') {
			switch($args->content_type) {
				case 'comment': $content_items = $this->_getCommentItems($args); break;
				case 'image': $content_items = $this->_getImageItems($args); break;
				case 'rss': $content_items = $this->getRssItems($args); break;
				case 'trackback': $content_items = $this->_getTrackbackItems($args); break;
				default: $content_items = $this->_getDocumentItems($args); break;
			}
		} else {
			$content_items = array();
			switch($args->content_type) {
	...
	function _getDocumentItems($args) {
		...
		if($output->toBool() && $output->data) foreach($output->data as $key => $val) $category_lists[$val->module_srl][$val->category_srl] = $val;
		$obj->module_srl = $args->module_srl;
		$obj->category_srl = $args->category_srl;
		$obj->sort_index = $args->order_target;
		if($args->order_target == 'list_order' || $args->order_target == 'update_order') $obj->order_type = $args->order_type=="desc"?"asc":"desc";
		else $obj->order_type = $args->order_type=="desc"?"desc":"asc";
		$obj->list_count = $args->list_count * $args->page_count;
	...
	function _getRssItems($args) {
		...
		if($xml_doc->rss) {
			...
			foreach ($items as $key => $value) {
				...
				$content_item->setContentsLink($rss->link);
				if($item->title) if(stripos($value->title->attrs->type, "html") === FALSE) $item->title = $value->title->body;
				$content_item->setTitle($item->title);
				$content_item->setNickName(max($item->author,$item->{'dc:creator'}));
	...
	function _getRssThumbnail($content) {
		...
		elseif($matches[3]) return $matches[3];
		else return NULL;
	}
