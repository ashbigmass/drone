...
class Net_URL2
{
	...
	public function setUserinfo($userinfo, $password = false) {
		if ($password !== false) $userinfo .= ':' . $password;
		if ($userinfo !== false) $userinfo = $this->_encodeData($userinfo);
	...
	public function getAuthority() {
		if (false === $this->_host) return false;
		$authority = '';
		if (strlen($this->_userinfo)) $authority .= $this->_userinfo . '@';
	...
	public function setAuthority($authority) {
		...
		if (!preg_match('(^(([^\@]*)\@)?(.+?)(:(\d*))?$)', $authority, $matches)) return $this;
		if ($matches[1]) $this->_userinfo = $this->_encodeData($matches[2]);
		$this->_host = $matches[3];
		if (isset($matches[5]) && strlen($matches[5])) $this->_port = $matches[5];
		return $this;
	}
	...
	public function getQueryVariables() {
		...
		for ($part = strtok($this->_query, $separator); strlen($part); $part = strtok($separator)) {
			...
			if ($encodeKeys) $key = rawurldecode($key);
	...
	private function _queryArrayByKey($key, $value, array $array = array()) {
		if (!strlen($key)) return $array;
		$offset = $this->_queryKeyBracketOffset($key);
		if ($offset === false) $name = $key;
		else $name = substr($key, 0, $offset);
	...
	private function _queryKeyBracketOffset($key) {
		if (false !== $open = strpos($key, '[') and false === strpos($key, ']', $open + 1)) $open = false;
		return $open;
	}
	...
	public function getURL() {
		...
		if ($authority === false && strtolower($this->_scheme) === 'file') $authority = '';
	...
	private function _buildAuthorityAndPath($authority, $path) {
		if ($authority === false) return $path;
	...
	public function normalize() {
		...
		foreach ($fields as &$field) {
			if ($field !== false) $field = $this->_normalize("$field");
		}
		unset($field);
		$this->_path = self::removeDotSegments($this->_path);
		if (false !== $this->_host && '' === $this->_path) $this->_path = '/';
	...
	protected function buildQuery(array $data, $separator, $key = null) {
		...
		foreach ($data as $name => $value) {
			if ($this->getOption(self::OPTION_ENCODE_KEYS) === true) $name = rawurlencode($name);
			if ($key !== null) {
			...
			if (is_array($value)) $query[] = $this->buildQuery($value, $separator, $name);
			else $query[] = $name . '=' . rawurlencode($value);
		}
