...
class HTTP_Request2_Adapter_Curl extends HTTP_Request2_Adapter
{
	...
	protected static $errorMap = array(
		CURLE_UNSUPPORTED_PROTOCOL  => array('HTTP_Request2_MessageException', HTTP_Request2_Exception::NON_HTTP_REDIRECT),
		...
		CURLE_WRITE_ERROR		   => array('HTTP_Request2_MessageException', HTTP_Request2_Exception::NON_HTTP_REDIRECT),
		CURLE_OPERATION_TIMEOUTED   => array('HTTP_Request2_MessageException', HTTP_Request2_Exception::TIMEOUT),
		...
		CURLE_LIBRARY_NOT_FOUND	 => array('HTTP_Request2_LogicException', HTTP_Request2_Exception::MISCONFIGURATION),
		CURLE_FUNCTION_NOT_FOUND	=> array('HTTP_Request2_LogicException', HTTP_Request2_Exception::MISCONFIGURATION),
		CURLE_ABORTED_BY_CALLBACK   => array('HTTP_Request2_MessageException', HTTP_Request2_Exception::NON_HTTP_REDIRECT),
		CURLE_TOO_MANY_REDIRECTS	=> array('HTTP_Request2_MessageException', HTTP_Request2_Exception::TOO_MANY_REDIRECTS),
		CURLE_SSL_PEER_CERTIFICATE  => array('HTTP_Request2_ConnectionException'),
		CURLE_GOT_NOTHING		   => array('HTTP_Request2_MessageException'),
		CURLE_SSL_ENGINE_NOTFOUND   => array('HTTP_Request2_LogicException', HTTP_Request2_Exception::MISCONFIGURATION),
		CURLE_SSL_ENGINE_SETFAILED  => array('HTTP_Request2_LogicException', HTTP_Request2_Exception::MISCONFIGURATION),
		CURLE_SEND_ERROR			=> array('HTTP_Request2_MessageException'),
		CURLE_RECV_ERROR			=> array('HTTP_Request2_MessageException'),
		CURLE_SSL_CERTPROBLEM	   => array('HTTP_Request2_LogicException', HTTP_Request2_Exception::INVALID_ARGUMENT),
	...
	protected static function wrapCurlError($ch) {
		...
		if (!isset(self::$errorMap[$nativeCode])) {
			...
			$code  = empty(self::$errorMap[$nativeCode][1]) ? 0 : self::$errorMap[$nativeCode][1];
	...
	public function sendRequest(HTTP_Request2 $request) {
		...
		try {
			if (false === curl_exec($ch = $this->createCurlHandle())) $e = self::wrapCurlError($ch);
		} catch (Exception $e) {
		}
		if (isset($ch)) {
		...
		if (!empty($e)) throw $e;
		if ($jar = $request->getCookieJar()) $jar->addCookiesFromResponse($response, $request->getUrl());
	...
	protected function createCurlHandle() {
		...
		if (!$this->request->getConfig('follow_redirects')) {
			...
			if (!@curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true)) {
				throw new HTTP_Request2_LogicException('Redirect support in curl is unavailable due to open_basedir or safe_mode setting',HTTP_Request2_Exception::MISCONFIGURATION);
			}
		...
		if ($ip = $this->request->getConfig('local_ip')) curl_setopt($ch, CURLOPT_INTERFACE, $ip);
		...
		switch ($this->request->getMethod()) {
			...
			case HTTP_Request2::METHOD_PUT: curl_setopt($ch, CURLOPT_UPLOAD, true); break;
		...
		if ($host = $this->request->getConfig('proxy_host')) {
			...
			if ($user = $this->request->getConfig('proxy_user')) {
				curl_setopt($ch, CURLOPT_PROXYUSERPWD, $user . ':' . $this->request->getConfig('proxy_password'));
				switch ($this->request->getConfig('proxy_auth_scheme')) {
					case HTTP_Request2::AUTH_BASIC: curl_setopt($ch, CURLOPT_PROXYAUTH, CURLAUTH_BASIC); break;
					case HTTP_Request2::AUTH_DIGEST: curl_setopt($ch, CURLOPT_PROXYAUTH, CURLAUTH_DIGEST);
				}
			}
			if ($type = $this->request->getConfig('proxy_type')) {
				switch ($type) {
					...
					default: throw new HTTP_Request2_NotImplementedException("Proxy type '{$type}' is not supported");
				}
			}
		}
		if ($auth = $this->request->getAuth()) {
			...
			switch ($auth['scheme']) {
				case HTTP_Request2::AUTH_BASIC: curl_setopt($ch, CURLOPT_HTTPAUTH, CURLAUTH_BASIC); break;
				case HTTP_Request2::AUTH_DIGEST: curl_setopt($ch, CURLOPT_HTTPAUTH, CURLAUTH_DIGEST);
			}
		}
		...
		if (($jar = $this->request->getCookieJar()) && ($cookies = $jar->getMatching($this->request->getUrl(), true))) {
		...
		if (isset($headers['content-length']) || isset($headers['transfer-encoding'])) $this->workaroundPhpBug47204($ch, $headers);
		$headersFmt = array();
	...
	protected function callbackReadBody($ch, $fd, $length) {
		...
		if (in_array($this->request->getMethod(), self::$bodyDisallowed) || 0 == $this->contentLength || $this->position >= $this->contentLength) {
			return '';
		}
	...
	protected function callbackWriteHeader($ch, $string) {
		...
		if (empty($this->response)) {
			$this->response = new HTTP_Request2_Response($string, false, curl_getinfo($ch, CURLINFO_EFFECTIVE_URL));
		} else {
			...
			if ('' == trim($string)) {
				...
				if ($this->request->getConfig('follow_redirects') && $this->response->isRedirect()) {
					...
					if ($jar = $this->request->getCookieJar()) {
						...
						if (!$redirectUrl->isAbsolute()) $redirectUrl = $this->request->getUrl()->resolve($redirectUrl);
						if ($cookies = $jar->getMatching($redirectUrl, true)) curl_setopt($ch, CURLOPT_COOKIE, $cookies);
					}
				}
