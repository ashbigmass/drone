...
class HTTP_Request2_Adapter_Socket extends HTTP_Request2_Adapter {
	...
	protected function connect() {
		...
		if ($ip = $this->request->getConfig('local_ip')) {
			$options['socket'] = array('bindto' => (false === strpos($ip, ':') ? $ip : '[' . $ip . ']') . ':0');
		}
		if ($secure || $tunnel) {
			$options['ssl'] = array();
			foreach ($this->request->getConfig() as $name => $value) {
				if ('ssl_' == substr($name, 0, 4) && null !== $value) {
					if ('ssl_verify_host' == $name) {
						if ($value) $options['ssl']['CN_match'] = $reqHost;
					} else {
		...
		if ($timeout = $this->request->getConfig('timeout')) $deadline = time() + $timeout;
		else $deadline = null;
	...
	protected function shouldUseServerDigestAuth(HTTP_Request2_Response $response) {
		...
		foreach ($prefixes as $prefix) {
			if (!empty(self::$challenges[$prefix]) && (empty($challenge['stale']) || strcasecmp('true', $challenge['stale']))) {
				$ret = false;
			}
	...
	protected function addAuthorizationHeader(&$headers, $requestHost, $requestUrl) {
		...
		switch ($auth['scheme']) {
			...
			case HTTP_Request2::AUTH_DIGEST:
				...
				$fullUrl = ('/' == $requestUrl[0]) ? $this->request->getUrl()->getScheme() . '://' . $requestHost . $requestUrl : $requestUrl;
			...
			break;
			default:
				throw new HTTP_Request2_NotImplementedException("Unknown HTTP authentication scheme '{$auth['scheme']}'");
		}
	}
	...
	protected function addProxyAuthorizationHeader(&$headers, $requestUrl) {
		...
		switch ($this->request->getConfig('proxy_auth_scheme')) {
			...
			break;
			default:
				throw new HTTP_Request2_NotImplementedException("Unknown HTTP authentication scheme '" .$this->request->getConfig('proxy_auth_scheme') . "'");
		}
	}
	...
	protected function prepareHeaders() {
		...
		if ('1.1' == $this->request->getConfig('protocol_version')) $this->updateExpectHeader($headers);
		else $this->expect100Continue = false;
	...
	protected function updateExpectHeader(&$headers) {
		...
		if (isset($headers['expect'])) {
			...
			$expectParam	 = ';\s*' . self::REGEXP_TOKEN . '(?:\s*=\s*(?:' . self::REGEXP_TOKEN . '|' . self::REGEXP_QUOTED_STRING . '))?\s*';
			$expectExtension = self::REGEXP_TOKEN . '(?:\s*=\s*(?:' . self::REGEXP_TOKEN . '|' . self::REGEXP_QUOTED_STRING . ')\s*(?:' . $expectParam . ')*)?';
			...
			while ($pos < $length) {
				...
				if (',' === substr($headers['expect'], $pos, 1)) {
					...
					throw new HTTP_Request2_LogicException("Cannot parse value '{$headers['expect']}' of Expect header", HTTP_Request2_Exception::INVALID_ARGUMENT);
				} else {
					$pos += strlen($m[0]);
					if (strcasecmp('100-continue', $m[0])) $expectations[]  = $m[0];
				}
			}
		}
		...
		if (empty($expectations)) unset($headers['expect']);
		else $headers['expect'] = implode(',', $expectations);
	}
	...
	protected function writeBody() {
		...
		while ($position < $this->contentLength) {
			...
			if (!$chunked) $this->socket->write($str);
			else $this->socket->write(dechex(strlen($str)) . "\r\n{$str}\r\n");
			$this->request->setLastEvent('sentBodyPart', strlen($str));
			$position += strlen($str);
		}
		if ($chunked) $this->socket->write("0\r\n\r\n");
	...
	protected function readResponse() {
		...
		do {
			try {
				...
				if (HTTP_Request2_Exception::TIMEOUT === $e->getCode() && $this->expect100Continue) return null;
				throw $e;
			}
			if ($this->expect100Continue && 100 == $response->getStatus()) return $response;
		...
		if ($chunked || null === $length || 0 < intval($length)) {
			...
			while (!$this->socket->eof() && (is_null($toRead) || 0 < $toRead)) {
				...
				if ('' == $data && (!$this->chunkLength || $this->socket->eof())) break;
				$hasBody = true;
	...
	protected function readChunked($bufferSize) {
		...
		if (0 == $this->chunkLength) $this->socket->readLine($bufferSize);
		return $data;
	}
}
