...
class HTTP_Request2 implements SplSubject
{
	...
	protected $lastEvent = array('name' => 'start', 'data' => null);
	protected $body = '';
	...
	public function __construct($url = null, $method = self::METHOD_GET, array $config = array()) {
		...
		$this->setHeader('user-agent', 'HTTP_Request2/2.2.1 ' . '(http://pear.php.net/package/http_request2) PHP/' . phpversion());
	}
	...
	public function setMethod($method) {
		if (preg_match(self::REGEXP_INVALID_TOKEN, $method)) {
			throw new HTTP_Request2_LogicException("Invalid request method '{$method}'", HTTP_Request2_Exception::INVALID_ARGUMENT);
		}
		$this->method = $method;
		return $this;
	}
	...
	public function getConfig($name = null) {
		if (null === $name) {
			...
			throw new HTTP_Request2_LogicException("Unknown configuration parameter '{$name}'", HTTP_Request2_Exception::INVALID_ARGUMENT);
		}
	...
	public function setAuth($user, $password = '', $scheme = self::AUTH_BASIC) {
		if (empty($user)) {
			...
			$this->auth = array('user'	 => (string)$user, 'password' => (string)$password, 'scheme'   => $scheme);
		}
		return $this;
	}
	...
	public function setHeader($name, $value = null, $replace = true) {
		if (is_array($name)) {
			foreach ($name as $k => $v) {
				if (is_string($k)) $this->setHeader($k, $v, $replace);
				else $this->setHeader($v, null, $replace);
			}
		} else {
			if (null === $value && strpos($name, ':')) list($name, $value) = array_map('trim', explode(':', $name, 2));
			if (preg_match(self::REGEXP_INVALID_TOKEN, $name)) {
				throw new HTTP_Request2_LogicException("Invalid header name '{$name}'", HTTP_Request2_Exception::INVALID_ARGUMENT);
			}
			...
			if (null === $value) {
				...
				if (!isset($this->headers[$name]) || $replace) $this->headers[$name] = $value;
	...
	public function setBody($body, $isFilename = false) {
		if (!$isFilename && !is_resource($body)) {
			...
			if (empty($this->headers['content-type'])) $this->setHeader('content-type', $fileData['type']);
		}
	...
	public function addUpload($fieldName, $filename, $sendFilename = null, $contentType = null) {
		if (!is_array($filename)) {
			...
			$this->uploads[$fieldName] = array(
				'fp'		=> $fileData['fp'],
				'filename'  => !empty($sendFilename)? $sendFilename :(is_string($filename)? basename($filename): 'anonymous.blob') ,
				'size'	  => $fileData['size'],
			...
			foreach ($filename as $f) {
				...
				$fps[]   = $fileData['fp'];
				$names[] = !empty($f[1])? $f[1] :(is_string($f[0])? basename($f[0]): 'anonymous.blob');
				$sizes[] = $fileData['size'];
	...
	public function setAdapter($adapter) {
		if (is_string($adapter)) {
			if (!class_exists($adapter, false)) {
				...
				if (!class_exists($adapter, false) && preg_match('/^HTTP_Request2_Adapter_([a-zA-Z0-9]+)$/', $adapter)) {
					include_once str_replace('_', DIRECTORY_SEPARATOR, $adapter) . '.php';
				}
				if (!class_exists($adapter, false)) {
					throw new HTTP_Request2_LogicException("Class {$adapter} not found", HTTP_Request2_Exception::MISSING_VALUE);
				}
			}
			$adapter = new $adapter;
		}
		if (!$adapter instanceof HTTP_Request2_Adapter) {
			throw new HTTP_Request2_LogicException('Parameter is not a HTTP request adapter', HTTP_Request2_Exception::INVALID_ARGUMENT);
		}
	...
	public function setCookieJar($jar = true) {
		if (!class_exists('HTTP_Request2_CookieJar', false)) require_once 'HTTP/Request2/CookieJar.php';
		if ($jar instanceof HTTP_Request2_CookieJar) {
			...
			throw new HTTP_Request2_LogicException('Invalid parameter passed to setCookieJar()',HTTP_Request2_Exception::INVALID_ARGUMENT);
		}
		return $this;
	}
	...
	protected function fopenWrapper($file, $detectType = false) {
		if (!is_string($file) && !is_resource($file)) {
			throw new HTTP_Request2_LogicException("Filename or file pointer resource expected", HTTP_Request2_Exception::INVALID_ARGUMENT);
		}
		$fileData = array('fp'   => is_string($file)? null: $file, 'type' => 'application/octet-stream', 'size' => 0);
		if (is_string($file)) {
			if (!($fileData['fp'] = @fopen($file, 'rb'))) {
				$error = error_get_last();
				throw new HTTP_Request2_LogicException($error['message'], HTTP_Request2_Exception::READ_ERROR);
			}
			if ($detectType) {
		...
		if (!($stat = fstat($fileData['fp']))) {
			throw new HTTP_Request2_LogicException("fstat() call failed", HTTP_Request2_Exception::READ_ERROR);
		}
