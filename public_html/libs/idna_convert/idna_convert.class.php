...
class idna_convert {
	...
	public function set_parameter($option, $value = false) {
		...
		foreach ($option as $k => $v) {
			switch ($k) {
				...
				break;
				case 'idn_version':
					if (in_array($v, array('2003', '2008'))) $this->_idn_version = $v;
					else $this->_error('Set Parameter: Unknown parameter ' . $v . ' for option ' . $k);
				break;
				case 'encode_german_sz':
					if (!$v) self::$NP['replacemaps'][0xDF] = array(0x73, 0x73);
					else unset(self::$NP['replacemaps'][0xDF]);
				break;
				default:
	...
	public function encode($decoded, $one_time_encoding = false) {
		...
		if ($last_begin) {
			...
			if (false !== ($output = $this->_encode($decoded))) return $output;
	...
	protected function _encode($decoded) {
		...
		while ($codecount < $deco_len) {
			for ($i = 0, $next_code = $this->_max_ucs; $i < $deco_len; $i++) {
				if ($decoded[$i] >= $cur_code && $decoded[$i] <= $next_code) $next_code = $decoded[$i];
			}
	...
	protected function _adapt($delta, $npoints, $is_first) {
		...
		for ($k = 0; $delta > (($this->_base - $this->_tmin) * $this->_tmax) / 2; $k += $this->_base) $delta = intval($delta / ($this->_base - $this->_tmin));
	...
	protected function _nameprep($input) {
		...
		foreach ($input as $v) {
			...
			if (0xAC00 <= $v && $v <= 0xD7AF) {
				...
				foreach ($this->_apply_cannonical_ordering(self::$NP['replacemaps_2003'][$v]) as $out) $output[] = (int) $out;
			} elseif (($this->_idn_version == '2008') && isset(self::$NP['replacemaps'][$v])) {
				foreach ($this->_apply_cannonical_ordering(self::$NP['replacemaps'][$v]) as $out) $output[] = (int) $out;
			} else {
				$output[] = (int) $v;
			}
		}
	...
	public function singleton($params = array()) {
		...
		if (!isset($instances[$signature])) $instances[$signature] = idna_convert::getInstance($params);
