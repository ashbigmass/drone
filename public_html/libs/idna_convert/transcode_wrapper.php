...
function encode_utf8($string = '', $encoding = 'iso-8859-1', $safe_mode = false) {
	...
	if (strtoupper($encoding) == 'UTF-8' || strtoupper($encoding) == 'UTF8') return $string;
	elseif (strtoupper($encoding) == 'ISO-8859-1') return utf8_encode($string);
	elseif (strtoupper($encoding) == 'WINDOWS-1252') return utf8_encode(map_w1252_iso8859_1($string));
	elseif (strtoupper($encoding) == 'UNICODE-1-1-UTF-7') $encoding = 'utf-7';
...
function decode_utf8($string = '', $encoding = 'iso-8859-1', $safe_mode = false)
	...
	if (strtoupper($encoding) == 'UTF-8' || strtoupper($encoding) == 'UTF8') return $string;
	elseif (strtoupper($encoding) == 'ISO-8859-1') return utf8_decode($string);
	elseif (strtoupper($encoding) == 'WINDOWS-1252') return map_iso8859_1_w1252(utf8_decode($string));
	elseif (strtoupper($encoding) == 'UNICODE-1-1-UTF-7') $encoding = 'utf-7';
	if (function_exists('mb_convert_encoding')) {
