...
class SMTP {
	...
	public function Connect($host,$port=0,$tval=30) {
		...
		if(empty($this->smtp_conn)) {
			...
			if($this->do_debug >= 1) echo "SMTP -> ERROR: " . $this->error["error"] . ": $errstr ($errno)" . $this->CRLF . '<br />';
			return false;
		}
		if(substr(PHP_OS, 0, 3) != "WIN") socket_set_timeout($this->smtp_conn, $tval, 0);
		$announce = $this->get_lines();
		if($this->do_debug >= 2) echo "SMTP -> FROM SERVER:" . $announce . $this->CRLF . '<br />';
		return true;
	}
	...
	public function StartTLS() {
		...
		if($this->do_debug >= 2) echo "SMTP -> FROM SERVER:" . $rply . $this->CRLF . '<br />';
		if($code != 220) {
			$this->error = array("error"	 => "STARTTLS not accepted from server", "smtp_code" => $code, "smtp_msg" => substr($rply,4));
			if($this->do_debug >= 1) echo "SMTP -> ERROR: " . $this->error["error"] . ": " . $rply . $this->CRLF . '<br />';
			return false;
		}
	...
	public function Quit($close_on_error=true) {
		...
		if($this->do_debug >= 2) echo "SMTP -> FROM SERVER:" . $byemsg . $this->CRLF . '<br />';
		$rval = true;
		$e = null;
		...
		if($code != 221) {
			...
			$rval = false;
			if($this->do_debug >= 1) echo "SMTP -> ERROR: " . $e["error"] . ": " . $byemsg . $this->CRLF . '<br />';
		}
	...
	private function get_lines() {
		...
		while($str = @fgets($this->smtp_conn,515)) {
			...
			$data .= $str;
			if($this->do_debug >= 4) echo "SMTP -> get_lines(): \$data is \"$data\"" . $this->CRLF . '<br />';
