...
class PHPMailer {
	...
	function Send() {
		...
		switch($this->Mailer) {
			...
			default: $result = $this->MailSend($header, $body); break;
		}
		return $result;
	}
	...
	function SendmailSend($header, $body) {
		...
		fputs($mail, $body);
		$result = pclose($mail);
		if (version_compare(phpversion(), '4.2.3') == -1) $result = $result >> 8 & 0xFF;
		if($result != 0) {
			$this->SetError($this->Lang('execute') . $this->Sendmail);
			return false;
		}
	...
	function MailSend($header, $body) {
		...
		if ($this->Sender != '' && strlen(ini_get('safe_mode')) < 1) {
			...
			if ($this->SingleTo === true && count($toArr) > 1) {
				foreach ($toArr as $key => $val) $rt = @mail($val, $this->EncodeHeader($this->SecureHeader($this->Subject)), $body, $header, $params);
			} else {
	...
	function CreateHeader() {
		...
		if((($this->Mailer == 'sendmail') || ($this->Mailer == 'mail')) && (count($this->cc) > 0)) $result .= $this->AddrAppend('Cc', $this->cc);
	...
	function CreateBody() {
		$result = '';
		if ($this->sign_key_file) $result .= $this->GetMailMIME();
		$this->SetWordWrap();
		switch($this->message_type) {
			case 'alt':
		...
		if($this->IsError()) {
			...
			if (@openssl_pkcs7_sign($file, $signed, "file://".$this->sign_cert_file, array("file://".$this->sign_key_file, $this->sign_key_pass), null)) {
				...
				$result = '';
				while(!feof($fp)) $result = $result . fread($fp, 1024);
				fclose($fp);
			} else {
	...
	function MsgHTML($message,$basedir='') {
		...
		if ( empty($this->AltBody) ) $this->AltBody = 'To view this email message, open the email in with HTML compatibility!' . "\n\n";
	}
	...
	function getFile($filename) {
		...
		if ($fp = fopen($filename, 'rb')) {
			while (!feof($fp)) $return .= fread($fp, 1024);
			fclose($fp);
			return $return;
		} else {
			return false;
