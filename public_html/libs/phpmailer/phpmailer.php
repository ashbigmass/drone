...
class PHPMailer {
	...
	public function IsSendmail() {
		if (!stristr(ini_get('sendmail_path'), 'sendmail')) $this->Sendmail = '/var/qmail/bin/sendmail';
		$this->Mailer = 'sendmail';
	}

	public function IsQmail() {
		if (stristr(ini_get('sendmail_path'), 'qmail')) $this->Sendmail = '/var/qmail/bin/sendmail';
		$this->Mailer	= 'sendmail';
	}
	...
	public function Send() {
		try {
			...
			if (empty($this->Body)) throw new phpmailerException($this->Lang('empty_message'), self::STOP_CRITICAL);
	...
	protected function SendmailSend($header, $body) {
		if ($this->Sender != '') $sendmail = sprintf("%s -oi -f %s -t", escapeshellcmd($this->Sendmail), escapeshellarg($this->Sender));
	...
	protected function MailSend($header, $body) {
		$toArr = array();
		foreach($this->to as $t) $toArr[] = $this->AddrFormat($t);
		$to = implode(', ', $toArr);
		...
		if(!$rt) throw new phpmailerException($this->Lang('instantiate'), self::STOP_CRITICAL);
		return true;
	}
	...
	protected function SmtpSend($header, $body) {
		...
		if(!$this->smtp->Mail($smtp_from)) throw new phpmailerException($this->Lang('from_failed') . $smtp_from, self::STOP_CRITICAL);
		...
		if(!$this->smtp->Data($header . $body)) throw new phpmailerException($this->Lang('data_not_accepted'), self::STOP_CRITICAL);
		if($this->SMTPKeepAlive == true) $this->smtp->Reset();
		return true;
	}

	public function SmtpConnect() {
		if(is_null($this->smtp)) $this->smtp = new SMTP();
		...
		try {
			while($index < count($hosts) && !$connection) {
				...
				if($this->smtp->Connect(($ssl ? 'ssl://':'').$host, $port, $this->Timeout)) {
					...
					if($tls) {
						if(!$this->smtp->StartTLS()) throw new phpmailerException($this->Lang('tls'));
						$this->smtp->Hello($hello);
					}
					$connection = true;
					if($this->SMTPAuth) {
						if(!$this->smtp->Authenticate($this->Username, $this->Password)) throw new phpmailerException($this->Lang('authenticate'));
					}
				}
				$index++;
	...
	public function AddrAppend($type, $addr) {
		$addr_str = $type . ': ';
		$addresses = array();
		foreach ($addr as $a) $addresses[] = $this->AddrFormat($a);
	...
	public function CreateHeader() {
		...
		if($this->Mailer != 'mail') {
			if ($this->SingleTo === true) {
				foreach($this->to as $t) $this->SingleToArray[] = $this->AddrFormat($t);
			} else {
	...
	public function CreateBody() {
		$body = '';
		if ($this->sign_key_file) $body .= $this->GetMailMIME();
		$this->SetWordWrap();
		switch($this->message_type) {
			case 'alt':
	...
	public function AddAttachment($path, $name = '', $encoding = 'base64', $type = 'application/octet-stream') {
		try {
			...
			if ($this->exceptions) throw $e;
			echo $e->getMessage()."\n";
			if ( $e->getCode() == self::STOP_CRITICAL ) return false;
		}
		return true;
	}
	...
	public function ClearCCs() {
		foreach($this->cc as $cc) unset($this->all_recipients[strtolower($cc[0])]);
		$this->cc = array();
	}

	public function ClearBCCs() {
		foreach($this->bcc as $bcc) unset($this->all_recipients[strtolower($bcc[0])]);
		$this->bcc = array();
	}
	...
	protected function SetError($msg) {
		...
		if ($this->Mailer == 'smtp' and !is_null($this->smtp)) {
			...
			if (!empty($lasterror) and array_key_exists('smtp_msg', $lasterror)) $msg .= '<p>' . $this->Lang('smtp_error') . $lasterror['smtp_msg'] . "</p>\n";
		}
		$this->ErrorInfo = $msg;
	}
	...
	public function MsgHTML($message,$basedir='') {
		...
		if ( !empty($textMsg) && empty($this->AltBody) ) $this->AltBody = html_entity_decode($textMsg);
	...
	public function DKIM_Sign($s) {
		...
		if (openssl_sign($s, $signature, $privKey)) return base64_encode($signature);
	}
	...
	public function DKIM_Add($headers_line,$subject,$body) {
		...
		foreach($headers as $header) {
			if (strpos($header,'From:') === 0) $from_header=$header;
