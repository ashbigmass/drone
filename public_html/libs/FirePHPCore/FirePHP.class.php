...
class FirePHP {
	...
	protected $options = array('maxObjectDepth' => 10, 'maxArrayDepth' => 20, 'useNativeJsonEncode' => true, 'includeLineNumbers' => true);
	...
	public static function getInstance($AutoCreate=false) {
		if($AutoCreate===true && !self::$instance) self::init();
	...
	public function errorHandler($errno, $errstr, $errfile, $errline, $errcontext) {
		...
		if (error_reporting() & $errno) {
			...
			if($this->throwErrorExceptions) throw $exception;
			else $this->fb($exception);
		}
	}
	...
	public function assertionHandler($file, $line, $code) {
		if($this->convertAssertionErrorsToExceptions) {
			...
			if($this->throwAssertionExceptions) throw $exception;
			else $this->fb($exception);
		} else {
	...
	public function group($Name, $Options=null) {
		...
		if($Options) {
			if(!is_array($Options)) throw $this->newException('Options must be defined as an array!');
			if(array_key_exists('Collapsed', $Options)) $Options['Collapsed'] = ($Options['Collapsed'])?'true':'false';
		}
	...
	public function detectClientExtension() {
		if(!@preg_match_all('/\sFirePHP\/([\.|\d]*)\s?/si',$this->getUserAgent(),$m) || !version_compare($m[1][0],'0.0.6','>=')) 	return false;
		return true;
	}
		...
		if($Object instanceof Exception) {
			...
			if($Type==self::TRACE) {
				...
				for( $i=0 ; $i<sizeof($trace) ; $i++ ) {
					if(isset($trace[$i]['class']) && isset($trace[$i]['file']) && ($trace[$i]['class']=='FirePHP' || $trace[$i]['class']=='FB')
						&& (substr($this->_standardizePath($trace[$i]['file']),-18,18)=='FirePHPCore/fb.php'
						|| substr($this->_standardizePath($trace[$i]['file']),-29,29)=='FirePHPCore/FirePHP.class.php')) {
					} else if(isset($trace[$i]['class']) && isset($trace[$i+1]['file']) && $trace[$i]['class']=='FirePHP'
				...
				if(isset($Object[0]) && is_string($Object[0])) $Object[1] = $this->encodeTable($Object[1]);
				else $Object = $this->encodeTable($Object);
				$skipFinalObjectEncode = true;
			} else if($Type==self::GROUP_START) {
				if(!$Label) throw $this->newException('You must specify a label for the group!');
			} else {
				if($Type===null) $Type = self::LOG;
			}
				...
				if(!isset($meta['file']) || !isset($meta['line'])) {
					...
					for( $i=0 ; $trace && $i<sizeof($trace) ; $i++ ) {
						if(isset($trace[$i]['class']) && isset($trace[$i]['file']) && ($trace[$i]['class']=='FirePHP'
							|| $trace[$i]['class']=='FB') && (substr($this->_standardizePath($trace[$i]['file']),-18,18)=='FirePHPCore/fb.php'
							|| substr($this->_standardizePath($trace[$i]['file']),-29,29)=='FirePHPCore/FirePHP.class.php')) {
						} else if(isset($trace[$i]['class']) && isset($trace[$i+1]['file']) && $trace[$i]['class']=='FirePHP'
							&& substr($this->_standardizePath($trace[$i+1]['file']),-18,18)=='FirePHPCore/fb.php') {
			...
			if($Type==self::DUMP) {
				...
				$msg_meta = $Options;
				$msg_meta['Type'] = $Type;
				if($Label!==null) $msg_meta['Label'] = $Label;
				if(isset($meta['file']) && !isset($msg_meta['File'])) $msg_meta['File'] = $meta['file'];
				if(isset($meta['line']) && !isset($msg_meta['Line'])) $msg_meta['Line'] = $meta['line'];
			...
			for( $i=0 ; $i<count($parts) ; $i++) {
				...
				if ($part) {
					...
					$this->messageIndex++;
					if ($this->messageIndex > 99999) throw $this->newException('Maximum number (99,999) of messages reached!');
				}
			}
	...
	protected function _escapeTrace($Trace) {
		...
		for( $i=0 ; $i<sizeof($Trace) ; $i++ ) {
			if(isset($Trace[$i]['file'])) $Trace[$i]['file'] = $this->_escapeTraceFile($Trace[$i]['file']);
			if(isset($Trace[$i]['args'])) $Trace[$i]['args'] = $this->encodeObject($Trace[$i]['args']);
		}
		return $Trace;
	}
	...
	public function jsonEncode($Object, $skipObjectEncode=false) {
		if(!$skipObjectEncode) $Object = $this->encodeObject($Object);
		if(function_exists('json_encode') && $this->options['useNativeJsonEncode']!=false) return json_encode($Object);
	...
	protected function encodeTable($Table) {
		...
		foreach($Table as $row) {
			if(is_array($row)) {
				$new_row = array();
				foreach($row as $item) $new_row[] = $this->encodeObject($item);
				$new_table[] = $new_row;
			}
		}
		return $new_table;
	...
	protected function encodeObject($Object, $ObjectDepth = 1, $ArrayDepth = 1) {
		...
		if (is_resource($Object)) {
			...
			if ($ObjectDepth > $this->options['maxObjectDepth']) return '** Max Object Depth ('.$this->options['maxObjectDepth'].') **';
			foreach ($this->objectStack as $refVal) {
				if ($refVal === $Object) return '** Recursion ('.get_class($Object).') **';
			}
			...
			$properties = array();
			foreach( $reflectionClass->getProperties() as $property) $properties[$property->getName()] = $property;
			$members = (array)$Object;
			foreach( $properties as $raw_name => $property ) {
				$name = $raw_name;
				if($property->isStatic()) $name = 'static:'.$name;
			...
			if ($ArrayDepth > $this->options['maxArrayDepth']) return '** Max Array Depth ('.$this->options['maxArrayDepth'].') **';
			...
			if(self::is_utf8($Object)) return $Object;
			else return utf8_encode($Object);
		}
		return $return;
	}
	...
	private function json_encode($var) {
		if(is_object($var)) {
			if(in_array($var,$this->json_objectStack)) return '"** Recursion **"';
		}
		switch (gettype($var)) {
			case 'boolean':
				...
				case 'array':
					if (is_array($var) && count($var) && (array_keys($var) !== range(0, sizeof($var) - 1))) {
						...
						foreach($properties as $property) {
							if($property instanceof Exception) return $property;
						}
					...
					foreach($elements as $element) {
						if($element instanceof Exception) return $element;
					}
	...
	private function json_name_value($name, $value) {
		...
		if($encoded_value instanceof Exception) return $encoded_value;
