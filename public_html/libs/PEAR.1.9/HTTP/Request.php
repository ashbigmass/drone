...
class HTTP_Request extends HTTP_Request2
{
	...
	public function getResponseCode() {
		if($this->response) return $this->response->getStatus();
	}

	public function getResponseHeader() {
		if($this->response) return $this->response->getHeader();
	}

	public function getResponseBody() {
		if($this->response) return $this->response->getBody();
	}

	public function getResponseCookies() {
		if($this->response) return $this->response->getCookies();
	}
