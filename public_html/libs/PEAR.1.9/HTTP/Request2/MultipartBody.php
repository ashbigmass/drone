...
class HTTP_Request2_MultipartBody
{
		...
		foreach ($uploads as $fieldName => $f) {
			if (!is_array($f['fp'])) {
				...
				for ($i = 0; $i < count($f['fp']); $i++) {
					$upload = array('name' => ($useBrackets? $fieldName . '[' . $i . ']': $fieldName));
					foreach (array('fp', 'filename', 'size', 'type') as $key) $upload[$key] = $f[$key][$i];
	...
	public function getLength() {
		...
		foreach ($this->_params as $p) $length += $headerParamLength + strlen($p[0]) + strlen($p[1]) + 2;
		foreach ($this->_uploads as $u) $length += $headerUploadLength + strlen($u['name']) + strlen($u['type']) + strlen($u['filename']) + $u['size'] + 2;
		return $length;
	}

	public function getBoundary() {
		if (empty($this->_boundary)) $this->_boundary = '--' . md5('PEAR-HTTP_Request2-' . microtime());
	...
	public function read($length) {
		...
		while ($length > 0 && $this->_pos[0] <= $paramCount + $uploadCount) {
			...
			if ($this->_pos[0] < $paramCount) {
				...
				$header = sprintf($this->_headerUpload, $boundary, $this->_uploads[$pos]['name'], $this->_uploads[$pos]['filename'], $this->_uploads[$pos]['type']);
				...
				if ($length > 0) {
					$start   = $this->_pos[1] + ($oldLength - $length) - strlen($header) - $this->_uploads[$pos]['size'];
					$ret	.= substr("\r\n", $start, $length);
			...
			if ($length > 0) $this->_pos	 = array($this->_pos[0] + 1, 0);
			else $this->_pos[1] += $oldLength;
		}
		return $ret;
	}
	...
	public function rewind() {
		$this->_pos = array(0, 0);
		foreach ($this->_uploads as $u) rewind($u['fp']);
	}
	...
	private static function _flattenArray($name, $values, $useBrackets) {
		if (!is_array($values)) {
			...
			foreach ($values as $k => $v) {
				...
				elseif ($useBrackets) $newName = $name . '[' . $k . ']';
				else $newName = $name;
