...
class PEAR {
	...
	function PEAR($error_class = null) {
		...
		if ($this->_debug) print "PEAR constructor called, class=$classname\n";
	...
	function _PEAR() {
		if ($this->_debug) printf("PEAR destructor called, class=%s\n", strtolower(get_class($this)));
	}

	function &getStaticProperty($class, $var) {
		static $properties;
		if (!isset($properties[$class])) $properties[$class] = array();
		if (!array_key_exists($var, $properties[$class])) $properties[$class][$var] = null;
	...
	function isError($data, $code = null) {
		if (!is_a($data, 'PEAR_Error')) return false;
		if (is_null($code)) return true;
		elseif (is_string($code)) return $data->getMessage() == $code;
	...
	function expectError($code = '*') {
		if (is_array($code)) array_push($this->_expected_errors, $code);
		else array_push($this->_expected_errors, array($code));
	...
	function delExpect($error_code) {
		...
		if ((is_array($error_code) && (0 != count($error_code)))) {
			foreach($error_code as $key => $error) {
				if ($this->_checkDelExpect($error)) $deleted =  true;
				else $deleted = false;
			}
			...
			else return PEAR::raiseError("The expected error you submitted does not exist");
		}
...
>>>>>>> origin/drone
		...
		if ($error_class !== null) $ec = $error_class;
		elseif (isset($this) && isset($this->_error_class)) $ec = $this->_error_class;
		else $ec = 'PEAR_Error';
		if (intval(PHP_VERSION) < 5) {
			include 'PEAR/FixPHP5PEARWarnings.php';
			return $a;
		}
		if ($skipmsg) $a = new $ec($code, $mode, $options, $userinfo);
...
>>>>>>> origin/drone
	...
	function _PEAR_call_destructors() {
		...
		if (is_array($_PEAR_destructor_object_list) && sizeof($_PEAR_destructor_object_list)) {
			...
			if (PEAR_ZE2) $destructLifoExists = PEAR5::getStaticProperty('PEAR', 'destructlifo');
			else $destructLifoExists = PEAR::getStaticProperty('PEAR', 'destructlifo');
			if ($destructLifoExists) $_PEAR_destructor_object_list = array_reverse($_PEAR_destructor_object_list);
		...
		if (isset($GLOBALS['_PEAR_shutdown_funcs']) AND is_array($GLOBALS['_PEAR_shutdown_funcs']) AND !empty($GLOBALS['_PEAR_shutdown_funcs'])) {
			foreach ($GLOBALS['_PEAR_shutdown_funcs'] as $value) call_user_func_array($value[0], $value[1]);
		}
	}
...
class PEAR_Error
{
	...
	function PEAR_Error($message = 'unknown error', $code = null, $mode = null, $options = null, $userinfo = null) {
		...
		if (!$skiptrace) {
			...
			if (isset($this->backtrace[0]) && isset($this->backtrace[0]['object'])) unset($this->backtrace[0]['object']);
		}
		if ($mode & PEAR_ERROR_CALLBACK) {
			$this->level = E_USER_NOTICE;
			$this->callback = $options;
		} else {
			if ($options === null) $options = E_USER_NOTICE;
			$this->level = $options;
			$this->callback = null;
		}
		if ($this->mode & PEAR_ERROR_PRINT) {
			if (is_null($options) || is_int($options)) $format = "%s";
			else $format = $options;
			printf($format, $this->getMessage());
		}
		if ($this->mode & PEAR_ERROR_TRIGGER) trigger_error($this->getMessage(), $this->level);
...
>>>>>>> origin/drone
		...
		if ($this->mode & PEAR_ERROR_CALLBACK) {
			if (is_callable($this->callback)) call_user_func($this->callback, $this);
		}
