...
if(!class_exists('AddonMemberCaptcha', false)) {
	...
	class AddonMemberCaptcha {
		...
		function before_module_init(&$ModuleHandler) {
			...
			if($_SESSION['XE_VALIDATOR_ERROR'] == -1) $_SESSION['member_captcha_authed'] = false;
			if($_SESSION['member_captcha_authed']) return false;
			$type = Context::get('captchaType');
			$this->target_acts = array();
			if($this->addon_info->apply_find_account == 'apply') $this->target_acts[] = 'procMemberFindAccount';
			if($this->addon_info->apply_resend_auth_mail == 'apply') $this->target_acts[] = 'procMemberResendAuthMail';
			if($this->addon_info->apply_signup == 'apply') $this->target_acts[] = 'procMemberInsert';
		...
		function createCaptchaAudio($string) {
			...
			for($i = 0, $c = strlen($string); $i < $c; $i++) {
				...
				$start = rand(5, 68);
				$datalen = strlen($_data) - $start - 256;
		...
		function compareCaptcha() {
			...
			if($_SESSION['member_captcha_authed']) return true;
...
if(method_exists($oAddonMemberCaptcha, $called_position))
	if(!call_user_func_array(array(&$oAddonMemberCaptcha, $called_position), array(&$this))) return false;
}
