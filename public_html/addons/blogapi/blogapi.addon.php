...
if($called_position == 'before_module_proc') {
	if(!$this->grant->write_document) printContent(getXmlRpcFailure(1, 'no permission'));
	...
	switch($method_name) {
		...
		case 'metaWeblog.newMediaObject' :
			...
			foreach($fileinfo as $key => $val) {
				...
				if($nodename == 'bits') $filedata = base64_decode((string)$val->value->base64);
				elseif($nodename == 'name') $filename = (string)$val->value->string;
			}
			...
			if(!$document_srl) {
				...
				if(!$oDocument->isExists() || !$oDocument->isGranted()) {
					...
					if($oDocument->get('category_srl')) {
						...
						if($category_list[$oDocument->get('category_srl')]) $category = $category_list[$oDocument->get('category_srl')]->title;
					}
					$content = sprintf(
		...
		case 'metaWeblog.newPost' :
			...
			foreach($info->value->struct->member as $val) {
				switch((string)$val->name) {
					case 'title' : $obj->title = (string)$val->value->string; break;
					case 'description' : $obj->content = (string)$val->value->string; break;
					case 'categories' :
					...
					case 'tagwords' :
						...
						foreach($tags as $tag) $tag_list[] = (string)$tag->string;
			...
			else $content = getXmlRpcResponse(strval($document_srl));
			FileHandler::removeDir($tmp_uploaded_path);
			printContent($content);
		break;
		case 'metaWeblog.editPost' :
			$tmp_val = (string)$params[0]->value->string;
			if(!$tmp_val) $tmp_val = (string)$params[0]->value->i4;
			if(!$tmp_val) {
