...
function _getEncodedVal($val, $is_sub_set = false) {
	if(preg_match('/^\<\!\[CDATA\[/',$val)) $buff = sprintf("<value>%s</value>", $val);
	elseif(is_int($val)) $buff = sprintf("<value><i4>%d</i4></value>", $val);
	...
	elseif(is_object($val)) {
		...
		$buff = "<value><struct>";
		foreach($values as $k => $v) $buff .= sprintf("<member>\n<name>%s</name>\n%s</member>\n", htmlspecialchars($k, ENT_COMPAT | ENT_HTML401, 'UTF-8', false), _getEncodedVal($v, true));
		$buff .= "</struct></value>\n";
	} elseif(is_array($val)) {
		$val_count = count($val);
		$buff = "<value><array>\n<data>";
		for($i = 0; $i < $val_count; $i++) $buff .= _getEncodedVal($val[$i], true);
		$buff .= "</data>\n</array></value>";
	} else {
