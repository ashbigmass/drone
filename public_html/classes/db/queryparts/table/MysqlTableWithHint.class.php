...
class MysqlTableWithHint extends Table
{
	...
	function toString() {
		...
		if($use_index_hint != '') $result .= ' USE INDEX (' . substr($use_index_hint, 0, -2) . ') ';
		if($force_index_hint != '') $result .= ' FORCE INDEX (' . substr($force_index_hint, 0, -2) . ') ';
		if($ignore_index_hint != '') $result .= ' IGNORE INDEX (' . substr($ignore_index_hint, 0, -2) . ') ';
		return $result;
	}
}
