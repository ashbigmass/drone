...
class Query extends Object
{
	...
	function Query($queryID = NULL, $action = NULL, $columns = NULL, $tables = NULL, $conditions = NULL, $groups = NULL, $orderby = NULL, $limit = NULL, $priority = NULL) {
	...
	function getClickCountColumns() {
		...
		foreach($this->columns as $column) {
			if($column->show() && is_a($column, 'ClickCountExpression')) $click_count_columns[] = $column;
		}
	...
	function getSelectString($with_values = TRUE) {
		foreach($this->columns as $column) {
			if($column->show()) {
				if($column->isSubquery()) $select[] = $column->toString($with_values) . ' as ' . $column->getAlias();
				else $select[] = $column->getExpression($with_values);
			}
		}
	...
	function getUpdateString($with_values = TRUE) {
		foreach($this->columns as $column) {
			if($column->show()) $update[] = $column->getExpression($with_values);
		}
		if(!$update) return;
	...
	function getInsertString($with_values = TRUE) {
		$columnsList = '';
		if($this->subquery) {
			foreach($this->columns as $column) $columnsList .= $column->getColumnName() . ', ';
	...
	function getWhereString($with_values = TRUE, $with_optimization = TRUE) {
		...
		foreach($this->conditions as $conditionGroup) {
			if($condition_count === 0) $conditionGroup->setPipe("");
	...
	function getGroupByString() {
		$groupBy = '';
		if($this->groups) {
			if($this->groups[0] !== "") $groupBy = implode(', ', $this->groups);
		}
		return $groupBy;
	}
	...
	function getOrderByString() {
		if(!$this->_orderByString) {
			...
			$orderBy = '';
			foreach($this->orderby as $order) $orderBy .= $order->toString() . ', ';
