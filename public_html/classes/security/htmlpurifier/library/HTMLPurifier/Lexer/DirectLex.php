...
class HTMLPurifier_Lexer_DirectLex extends HTMLPurifier_Lexer
{
	...
	public function tokenizeHTML($html, $config, $context) {
		...
		if ($maintain_line_numbers === null) 	$maintain_line_numbers = $config->get('Core.CollectErrors');
		...
		while(++$loops) {
			...
			if (!$inside_tag && $position_next_lt !== false) {
				...
				if ($position_first_space >= $strlen_segment) {
					if ($is_self_closing) $token = new HTMLPurifier_Token_Empty($segment);
					else $token = new HTMLPurifier_Token_Start($segment);
					if ($maintain_line_numbers) {
						$token->rawPosition($current_line, $current_col);
						$current_line += $this->substrCount($html, $nl, $cursor, $position_next_gt - $cursor);
					}
				...
				if ($attribute_string) $attr = $this->parseAttributeString($attribute_string, $config, $context);
				else $attr = array();
				if ($is_self_closing) $token = new HTMLPurifier_Token_Empty($type, $attr);
				else $token = new HTMLPurifier_Token_Start($type, $attr);
				if ($maintain_line_numbers) {
					$token->rawPosition($current_line, $current_col);
					$current_line += $this->substrCount($html, $nl, $cursor, $position_next_gt - $cursor);
				}
	...
	protected function substrCount($haystack, $needle, $offset, $length) {
		static $oldVersion;
		if ($oldVersion === null) $oldVersion = version_compare(PHP_VERSION, '5.1', '<');
		if ($oldVersion) {
