...
class HTML5
{
	...
	public function __construct($data) {
		...
		$this->state = 'data';
		while($this->state !== null) $this->{$this->state.'State'}();
	}
	...
	private function character($s, $l = 0) {
		if($s + $l < $this->EOF) {
			if($l === 0) return $this->data[$s];
			else return substr($this->data, $s, $l);
		}
	}
	...
	private function dataState() {
		...
		if($char === '&' && ($this->content_model === self::PCDATA || $this->content_model === self::RCDATA)) {
			...
			$this->char >= 3 && $this->character($this->char - 4, 4) === '<!--') $this->escape = true;
			$this->emitToken(array('type' => self::CHARACTR,'data' => $char));
		} elseif($char === '<' && ($this->content_model === self::PCDATA || (($this->content_model === self::RCDATA ||
			$this->content_model === self::CDATA) && $this->escape === false))) {
			$this->state = 'tagOpen';
		} elseif($char === '>') {
			if(($this->content_model === self::RCDATA ||
			$this->content_model === self::CDATA) && $this->escape === true &&
			$this->character($this->char, 3) === '-->') $this->escape = false;
			$this->emitToken(array('type' => self::CHARACTR,'data' => $char));
	...
	private function bogusCommentState() {
		...
		$this->state = 'data';
		if($this->char === $this->EOF) $this->char = $this->EOF - 1;
	}
			...
			default:
				...
				for($c = 1; $c <= $len; $c++) {
					...
					if(in_array($id, $this->entities)) {
						if ($e_name[$c-1] !== ';') if ($c < $len && $e_name[$c] == ';') $this->char++;
						$entity = $id;
						break;
					}
				}
	...
	private function emitToken($token) {
		...
		if(is_int($emit)) $this->content_model = $emit;
...
class HTML5TreeConstructer
{
	...
	private function mainPhase($token) {
		if($token['type'] === HTML5::DOCTYPE) {
			...
			foreach($token['attr'] as $attr) {
				if(!$this->stack[0]->hasAttribute($attr['name'])) $this->stack[0]->setAttribute($attr['name'], $attr['value']);
			}
	...
	private function inHead($token) {
		...
		($token['type'] === HTML5::ENDTAG && $token['name'] !== 'html')) {
			...
			if($this->head_pointer->isSameNode(end($this->stack))) $this->inHead(array('name' => 'head','type' => HTML5::ENDTAG));
			else $this->mode = self::AFTER_HEAD;
		...
		switch($token['type']) {
			...
			switch($token['name']) {
				...
				case 'li': case 'dd': case 'dt':
					...
					for($n = $stack_length; 0 <= $n; $n--) {
						...
						if($token['name'] === $node->tagName ||	($token['name'] !== 'li'
							...
							for($x = $stack_length; $x >= $n ; $x--) array_pop($this->stack);
							break;
						}
				...
				case 'h1': case 'h2': case 'h3': case 'h4': case 'h5': case 'h6':
					...
					while($this->elementInScope(array('h1', 'h2', 'h3', 'h4', 'h5', 'h6'))) array_pop($this->stack);
					$this->insertElement($token);
				break;
				case 'a':
			...
			switch($token['name']) {
				...
				case 'menu': case 'ol': case 'pre': case 'ul':
					if($this->elementInScope($token['name'])) {
						...
						for($n = count($this->stack) - 1; $n >= 0; $n--) {
							if($this->stack[$n]->nodeName === $token['name']) $n = -1;
							array_pop($this->stack);
						}
					}
				break;
				case 'form':
					if($this->elementInScope($token['name'])) $this->generateImpliedEndTags();
				...
				case 'p':
					if($this->elementInScope('p')) {
						...
						for($n = count($this->stack) - 1; $n >= 0; $n--) {
							if($this->elementInScope('p')) array_pop($this->stack);
							else break;
						}
					}
				break;
				...
				case 'h1': case 'h2': case 'h3': case 'h4': case 'h5': case 'h6':
					...
					if($this->elementInScope($elements)) {
						$this->generateImpliedEndTags();
						while($this->elementInScope($elements)) array_pop($this->stack);
					}
				break;
				...
				case 'strong': case 'tt': case 'u':
					while(true) {
						...
						for($s = $fe_s_pos + 1; $s < $length; $s++) {
							...
							if($category !== self::PHRASING && $category !== self::FORMATTING) $furthest_block = $this->stack[$s];
						}
						if(!isset($furthest_block)) {
							for($n = $length - 1; $n >= $fe_s_pos; $n--) array_pop($this->stack);
							unset($this->a_formatting[$fe_af_pos]);
							$this->a_formatting = array_merge($this->a_formatting);
							break;
						}
						...
						while(true) {
							...
							elseif($last_node === $furthest_block) $bookmark = array_search($node, $this->a_formatting, true) + 1;
							...
							if($last_node->parentNode !== null) $last_node->parentNode->removeChild($last_node);
				...
				case 'button': case 'marquee': case 'object':
					if($this->elementInScope($token['name'])) {
						...
						for($n = count($this->a_formatting) - 1; $n > $marker; $n--) array_pop($this->a_formatting);
					}
				break;
				case 'area': case 'basefont': case 'bgsound': case 'br':
				...
				default:
					for($n = count($this->stack) - 1; $n >= 0; $n--) {
						...
						if($token['name'] === $node->nodeName) {
							$this->generateImpliedEndTags();
							for($x = count($this->stack) - $n; $x >= $n; $x--) array_pop($this->stack);
						} else {
	...
	private function inTable($token) {
		...
		$token['name'] === 'table') {
			...
			array('body', 'caption', 'col', 'colgroup', 'html', 'tbody', 'td', 'tfoot', 'th', 'thead', 'tr'))) {
		} else {
	...
	private function inCaption($token) {
		...
		$token['name'] === 'table')) {
			...
			array('body', 'col', 'colgroup', 'html', 'tbody', 'tfoot', 'th','thead', 'tr'))) {
		} else {
			$this->inBody($token);
		}
	}
	...
	private function inSelect($token) {
		...
		$token['name'] === 'optgroup') {
			...
			if($this->stack[$elements_in_stack - 1] === 'optgroup') array_pop($this->stack);
		} elseif($token['type'] === HTML5::ENDTAG &&
		$token['name'] === 'option') {
			if(end($this->stack)->nodeName === 'option') array_pop($this->stack);
		} elseif($token['type'] === HTML5::ENDTAG &&
		$token['name'] === 'select') {
			if(!$this->elementInScope($token['name'], true)) {
			} else {
	...
	private function appendToRealParent($node) {
		if($this->foster_parent === null) {
			...
			else $this->foster_parent->appendChild($node);
	...
	private function elementInScope($el, $table = false) {
		if(is_array($el)) {
			foreach($el as $element) {
				if($this->elementInScope($element, $table)) return true;
			}
			return false;
		}
	...
	private function reconstructActiveFormattingElements() {
		...
		if($formatting_elements === 0) return false;
		$entry = end($this->a_formatting);
		if($entry === self::MARKER || in_array($entry, $this->stack, true)) return false;
		for($a = $formatting_elements - 1; $a >= 0; true) {
			...
			if($entry === self::MARKER || in_array($entry, $this->stack, true)) break;
		}
		...
		while(true) {
			...
			if(end($this->a_formatting) !== $clone) $step_seven = true;
			else break;
		}
	}
	...
	private function generateImpliedEndTags($exclude = array()) {
		...
		while(in_array(end($this->stack)->nodeName, $elements)) array_pop($this->stack);
	}
	...
	private function getElementCategory($node) {
		...
		else return self::PHRASING;
	}

	private function clearStackToTableContext($elements) {
		while(true) {
			$node = end($this->stack)->nodeName;
			if(in_array($node, $elements)) break;
	...
	private function resetInsertionMode() {
		...
		for($n = $leng - 1; $n >= 0; $n--) {
			$node = $this->stack[$n];
			if($this->stack[0]->isSameNode($node)) $last = true;
