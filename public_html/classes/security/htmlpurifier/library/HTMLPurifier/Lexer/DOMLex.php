...
class HTMLPurifier_Lexer_DOMLex extends HTMLPurifier_Lexer
{
	...
	protected function tokenizeDOM($node, &$tokens) {
		...
		do {
			while (!empty($nodes[$level])) {
				...
				if ($needEndingTag) $closingNodes[$level][] = $node;
				if ($node->childNodes && $node->childNodes->length) {
					$level++;
					$nodes[$level] = array();
					foreach ($node->childNodes as $childNode) array_push($nodes[$level], $childNode);
				}
			}
			$level--;
			if ($level && isset($closingNodes[$level])) {
				while($node = array_pop($closingNodes[$level])) $this->createEndNode($node, $tokens);
			}
		} while ($level > 0);
	}
	...
	protected function createStartNode($node, &$tokens, $collect) {
		...
		if (!$node->childNodes->length) {
			if ($collect) $tokens[] = $this->factory->createEmpty($node->tagName, $attr);
			return false;
		} else {
			if ($collect) $tokens[] = $this->factory->createStart($tag_name = $node->tagName,$attr);
			return true;
		}
	}
	...
	protected function transformAttrToAssoc($node_map) {
		...
		$array = array();
		foreach ($node_map as $attr) $array[$attr->name] = $attr->value;
		return $array;
	}
