...
class HTMLPurifier_URIFilter_MakeAbsolute extends HTMLPurifier_URIFilter
{
	...
	public function filter(&$uri, $config, $context) {
		...
		if (!is_null($uri->scheme)) {
			...
			if (!$scheme_obj->hierarchical) return true;
		}
		if (!is_null($uri->host)) return true;
		if ($uri->path === '') {
			...
			if ($new_stack[0] !== '' && !is_null($this->base->host)) array_unshift($new_stack, '');
		...
		for ($i = 0; isset($stack[$i]); $i++) {
			...
			if ($stack[$i] == '..') {
				if (!empty($result)) {
					...
					if ($segment === '' && empty($result)) $result[] = '';
					elseif ($segment === '..') $result[] = '..';
				} else {
					$result[] = '..';
				}
				$is_folder = true;
