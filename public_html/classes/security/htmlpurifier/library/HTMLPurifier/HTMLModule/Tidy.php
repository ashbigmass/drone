...
class HTMLPurifier_HTMLModule_Tidy extends HTMLPurifier_HTMLModule
{
	...
	public function setup($config) {
		...
		foreach ($fixes as $name => $fix) {
			if (isset($remove_fixes[$name]) || (!isset($add_fixes[$name]) && !isset($fixes_lookup[$name]))) unset($fixes[$name]);
		}
		$this->populate($fixes);
	}

	public function getFixesForLevel($level) {
		if ($level == $this->levels[0]) return array();
		...
		foreach ($activated_levels as $level) {
			foreach ($this->fixesForLevel[$level] as $fix) $ret[$fix] = true;
		}
		return $ret;
	}
			...
			switch ($type) {
				...
				case 'attr_transform_post':
					...
					if (isset($params['element'])) {
						...
						if (empty($this->info[$element])) $e = $this->addBlankElement($element);
						else $e = $this->info[$element];
					} else {
						$type = "info_$type";
						$e = $this;
					}
	...
	public function getFixType($name) {
		...
		if (is_null($property)) return array('tag_transform', $params);
