...
class HTMLPurifier_Strategy_RemoveForeignElements extends HTMLPurifier_Strategy
{
	public function execute($tokens, $config, $context) {
		...
		if ($remove_script_contents === true) $hidden_elements['script'] = true;
		elseif ($remove_script_contents === false && isset($hidden_elements['script'])) unset($hidden_elements['script']);
		...
		foreach($tokens as $token) {
			if ($remove_until) {
				if (empty($token->is_tag) || $token->name !== $remove_until) continue;
			}
			if (!empty( $token->is_tag )) {
				...
				if ($textify_comments !== false) {
					...
					if ($e) {
						if (substr($token->data, -1) == '-') $trailing_hyphen = true;
					}
					...
					if ($trusted || !empty($comment_lookup[trim($token->data)]) || ($comment_regexp !== NULL && preg_match($comment_regexp, trim($token->data)))) {
						if ($e) {
							if ($trailing_hyphen) $e->send(E_NOTICE, 'Strategy_RemoveForeignElements: Trailing hyphen in comment removed');
							if ($found_double_hyphen) $e->send(E_NOTICE, 'Strategy_RemoveForeignElements: Hyphens in comment collapsed');
						}
					} else {
