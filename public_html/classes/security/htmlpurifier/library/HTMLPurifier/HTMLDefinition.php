...
class HTMLPurifier_HTMLDefinition extends HTMLPurifier_Definition
{
	...
	public function addAttribute($element_name, $attr_name, $def) {
		...
		if (!isset($module->info[$element_name])) $element = $module->addBlankElement($element_name);
		else $element = $module->info[$element_name];
	...
	protected function setupConfigStuff($config) {
		...
		if (isset($this->info_content_sets['Block'][$block_wrapper])) $this->info_block_wrapper = $block_wrapper;
		else trigger_error('Cannot use non-block element as block wrapper', E_USER_ERROR);
		...
		if (!is_array($allowed_elements) && !is_array($allowed_attributes)) {
			...
			if (is_string($allowed)) list($allowed_elements, $allowed_attributes) = $this->parseTinyMCEAllowedList($allowed);
		}
		if (is_array($allowed_elements)) {
			...
			foreach ($allowed_elements as $element => $d) {
				$element = htmlspecialchars($element, ENT_COMPAT | ENT_HTML401, 'UTF-8', false);
		...
		if (is_array($allowed_attributes)) {
			foreach ($this->info_global_attr as $attr => $x) {
				...
				$delete = true;
				foreach ($keys as $key) {
					if ($delete && isset($allowed_attributes[$key])) $delete = false;
					if (isset($allowed_attributes_mutable[$key])) unset($allowed_attributes_mutable[$key]);
				}
		...
		foreach ($forbidden_attributes as $key => $v) {
			...
			if ($key[0] != '*') continue;
			if ($key[1] == '.') trigger_error("Error with $key: *.attr syntax not supported for HTML.ForbiddenAttributes; use attr instead", E_USER_WARNING);
		}
	...
	public function parseTinyMCEAllowedList($list) {
		...
		foreach ($chunks as $chunk) {
			...
			$attr = explode('|', $attr);
			foreach ($attr as $key) $attributes["$element.$key"] = true;
		}
