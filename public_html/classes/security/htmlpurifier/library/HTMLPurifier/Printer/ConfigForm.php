...
class HTMLPurifier_Printer_ConfigForm extends HTMLPurifier_Printer
{
	...
	public function render($config, $allowed = true, $render_controls = true) {
		...
		$ret .= $this->end('tr');
		$ret .= $this->end('thead');
		foreach ($all as $ns => $directives) $ret .= $this->renderNamespace($ns, $directives);
		if ($render_controls) {
	...
	protected function renderNamespace($ns, $directives) {
		...
		foreach ($directives as $directive => $value) {
			...
			$ret .= $this->element('label',$directive_disp,$attr);
			...
			if (!isset($this->fields[$type])) $type = 0;
			$type_obj = $this->fields[$type];
			if ($allow_null) $type_obj = new HTMLPurifier_Printer_ConfigForm_NullDecorator($type_obj);
		...
		if (is_array($value)) {
			switch ($type) {
				case HTMLPurifier_VarParser::LOOKUP:
					$array = $value;
					$value = array();
					foreach ($array as $val => $b) $value[] = $val;
				case HTMLPurifier_VarParser::ALIST:
					$value = implode(PHP_EOL, $value);
					break;
				case HTMLPurifier_VarParser::HASH:
					$nvalue = '';
					foreach ($value as $i => $v) $nvalue .= "$i:$v" . PHP_EOL;
					$value = $nvalue;
					break;
				default:
					$value = '';
