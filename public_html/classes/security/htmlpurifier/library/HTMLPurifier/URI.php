...
class HTMLPurifier_URI
{
	...
	public function validate($config, $context) {
		...
		if (!is_null($this->scheme) && is_null($this->host) || $this->host === '') {
			$def = $config->getDefinition('URI');
			if ($def->defaultScheme === $this->scheme) $this->scheme = null;
		}
		if (!is_null($this->userinfo)) {
			$encoder = new HTMLPurifier_PercentEncoder($chars_sub_delims . ':');
			$this->userinfo = $encoder->encode($this->userinfo);
		}
		if (!is_null($this->port)) if ($this->port < 1 || $this->port > 65535) $this->port = null;
		$path_parts = array();
		...
		if (!is_null($this->host)) {
			...
			if ($this->path[0] === '/') {
				if (strlen($this->path) >= 2 && $this->path[1] === '/') $this->path = '';
				else $this->path = $segments_encoder->encode($this->path);
		...
		if (!is_null($this->query)) $this->query = $qf_encoder->encode($this->query);
		if (!is_null($this->fragment)) $this->fragment = $qf_encoder->encode($this->fragment);
		return true;
	}
	...
	public function isBenign($config, $context) {
		...
		if ($current_scheme_obj->secure) {
			if (!$scheme_obj->secure) return false;
		}
		return true;
	}
}
