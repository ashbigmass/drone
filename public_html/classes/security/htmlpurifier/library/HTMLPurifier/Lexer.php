...
class HTMLPurifier_Lexer
{
	...
	public static function create($config) {
		...
		if (is_object($lexer)) {
			...
			if (is_null($lexer)) {
				do {
					if ($needs_tracking) {$lexer = 'DirectLex';break;}
					if (class_exists('DOMDocument') && method_exists('DOMDocument', 'loadHTML') && !extension_loaded('domxml')) $lexer = 'DOMLex';
					else $lexer = 'DirectLex';
				} while(0);
			}
			switch ($lexer) {
				case 'DOMLex': $inst = new HTMLPurifier_Lexer_DOMLex(); break;
				case 'DirectLex': $inst = new HTMLPurifier_Lexer_DirectLex(); break;
				case 'PH5P': $inst = new HTMLPurifier_Lexer_PH5P(); break;
				default:
	...
	public function normalize($html, $config, $context) {
		...
		if ($config->get('HTML.Trusted')) $html = $this->escapeCommentedCDATA($html);
		$html = $this->escapeCDATA($html);
		$html = $this->removeIEConditional($html);
		if ($config->get('Core.ConvertDocumentToFragment')) {
			$e = false;
			if ($config->get('Core.CollectErrors')) $e =& $context->get('ErrorCollector');
			$new_html = $this->extractBody($html);
			if ($e && $new_html != $html) $e->send(E_WARNING, 'Lexer: Extracted body');
			$html = $new_html;
		}
		$html = $this->_entity_parser->substituteNonSpecialEntities($html);
		$html = HTMLPurifier_Encoder::cleanUTF8($html);
		if ($config->get('Core.RemoveProcessingInstructions')) $html = preg_replace('#<\?.+?\?>#s', '', $html);
		return $html;
	}
