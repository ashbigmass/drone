...
class HTMLPurifier_Config
{
	...
	public static function create($config, $schema = null) {
		...
		if (!$schema) $ret = HTMLPurifier_Config::createDefault();
		else $ret = new HTMLPurifier_Config($schema);
	...
	public function get($key, $a = null) {
		...
		if (!isset($this->def->info[$key])) {
			$this->triggerError('Cannot retrieve value of undefined directive ' . htmlspecialchars($key, ENT_COMPAT | ENT_HTML401, 'UTF-8', false), E_USER_WARNING);
			return;
		}
	...
	public function getSerial() {
		if (empty($this->serial)) $this->serial = md5(serialize($this->getAll()));
		return $this->serial;
	}
	...
	public function set($key, $value, $a = null) {
		...
		if (is_string($value) && is_object($def)) {
			if (isset($def->aliases[$value])) $value = $def->aliases[$value];
		...
		if ($namespace == 'HTML' || $namespace == 'CSS' || $namespace == 'URI') $this->definitions[$namespace] = null;
	...
	public function getDefinition($type, $raw = false, $optimized = false) {
		if ($optimized && !$raw) throw new HTMLPurifier_Exception("Cannot set optimized = true when raw = false");
		if (!$this->finalized) $this->autoFinalize();
		$lock = $this->lock;
		$this->lock = null;
	...
	private function initDefinition($type) {
		if ($type == 'HTML') $def = new HTMLPurifier_HTMLDefinition();
		elseif ($type == 'CSS') $def = new HTMLPurifier_CSSDefinition();
		elseif ($type == 'URI') $def = new HTMLPurifier_URIDefinition();
		else throw new HTMLPurifier_Exception("Definition of $type type not supported");
		$this->definitions[$type] = $def;
		return $def;
	}
	...
	public function loadArray($config_array) {
		...
		foreach ($config_array as $key => $value) {
			...
			if (strpos($key, '.') !== false) {
				...
				foreach ($namespace_values as $directive => $value) $this->set($namespace .'.'. $directive, $value);
			}
		}
	}

	public static function getAllowedDirectivesForForm($allowed, $schema = null) {
		if (!$schema) $schema = HTMLPurifier_ConfigSchema::instance();
		if ($allowed !== true) {
			 ...
			 foreach ($allowed as $ns_or_directive) {
				 if (strpos($ns_or_directive, '.') !== false) {
					 if ($ns_or_directive[0] == '-') $blacklisted_directives[substr($ns_or_directive, 1)] = true;
	...
	public function isFinalized($error = false) {
		if ($this->finalized && $error) $this->triggerError($error, E_USER_ERROR);
		return $this->finalized;
	}

	public function autoFinalize() {
		if ($this->autoFinalize) $this->finalize();
		else $this->plist->squash(true);
	}
	...
	protected function triggerError($msg, $no) {
		...
		if ($this->chatty) {
			...
			for ($i = 0, $c = count($trace); $i < $c - 1; $i++) {
				if ($trace[$i + 1]['class'] === 'HTMLPurifier_Config') continue;
				$frame = $trace[$i];
