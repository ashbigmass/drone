...
class HTMLPurifier_ContentSets
{
	...
	public function __construct($modules) {
		...
		foreach ($modules as $module_i => $module) {
			foreach ($module->content_sets as $key => $value) {
				...
				if (isset($this->lookup[$key])) $this->lookup[$key] = array_merge($this->lookup[$key], $temp);
				else $this->lookup[$key] = $temp;
			}
		}
		$old_lookup = false;
		...
		foreach ($this->lookup as $key => $lookup) $this->info[$key] = implode(' | ', array_keys($lookup));
	...
	protected function convertToLookup($string) {
		...
		$ret = array();
		foreach ($array as $i => $k) $ret[$k] = true;
		return $ret;
	}
}
