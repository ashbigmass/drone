...
class HTMLPurifier_UnitConverter
{
	...
	protected static $units = array(
		self::ENGLISH => array('px' => 3, 'pt' => 4, 'pc' => 48, 'in' => 288, self::METRIC => array('pt', '0.352777778', 'mm'),),
	...
	public function convert($length, $to_unit) {
		...
		$n	= $length->getN();
		$unit = $length->getUnit();
		if ($n === '0' || $unit === false) return new HTMLPurifier_Length('0', false);
		...
		for ($i = 0; $i < 2; $i++) {
			if ($dest_state === $state) $dest_unit = $to_unit;
			else $dest_unit = self::$units[$state][$dest_state][0];
			if ($dest_unit !== $unit) {
