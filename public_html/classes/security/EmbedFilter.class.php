...
class EmbedFilter
{
	...
	function getInstance() {
		if(!isset($GLOBALS['__EMBEDFILTER_INSTANCE__'])) $GLOBALS['__EMBEDFILTER_INSTANCE__'] = new EmbedFilter();
				...
				while($parser->parse()) {
					if(is_array($parser->iNodeAttributes)) {
						foreach($parser->iNodeAttributes AS $attrName => $attrValue) {
							...
							if(strtolower($attrName) == 'type' && $attrValue) $isWhiteMimetype = $this->isWhiteMimetype($attrValue);
						}
					}
				}
	...
	function isWhiteDomain($urlAttribute) {
		if(is_array($this->whiteUrlList)) {
			foreach($this->whiteUrlList AS $key => $value) {
				if(preg_match('@^' . preg_quote($value) . '@i', $urlAttribute)) return TRUE;
			}
		}
		return FALSE;
	}
	...
	function isWhiteMimetype($mimeType) {
		if(isset($this->mimeTypeList[$mimeType])) return TRUE;
		return FALSE;
	}

	function isWhiteExt($ext) {
		if(isset($this->extList[$ext])) return TRUE;
		return FALSE;
	}
	...
	function _checkAllowScriptAccess($m) {
		...
		if($m[1] == 'param') {
			...
			if(stripos($m[0], 'allowscriptaccess')) $m[0] = preg_replace('/always|samedomain/i', 'never', $m[0]);
			else $m[0] = preg_replace('/\<embed/i', '<embed allowscriptaccess="never"', $m[0]);
		}
		return $m[0];
	}
	...
	function _makeWhiteDomainList($whitelist = NULL) {
		...
		$isMake = FALSE;
		if(!file_exists($whiteUrlCacheFile)) $isMake = TRUE;
		if(file_exists($whiteUrlCacheFile) && filemtime($whiteUrlCacheFile) < filemtime($whiteUrlXmlFile)) $isMake = TRUE;
		if(gettype($whitelist) == 'array' && gettype($whitelist['object']) == 'array' && gettype($whitelist['iframe']) == 'array') $isMake = FALSE;
		if(isset($whitelist) && gettype($whitelist) == 'object') $isMake = TRUE;
		if($isMake) {
			$whiteUrlList = array();
			...
			if(gettype($whitelist->object) == 'array' && gettype($whitelist->iframe) == 'array') {
				...
				foreach($embedDomainList AS $key => $value) {
					...
					if(is_array($patternList)) {
						foreach($patternList AS $key => $value) $whiteUrlList[] = $value->body;
					} else {
				...
				foreach($iframeDomainList AS $key => $value) {
					$patternList = $value->pattern;
					if(is_array($patternList)) {
						foreach($patternList AS $key => $value) $whiteIframeUrlList[] = $value->body;
					} else {
			...
			if($db_info->embed_white_object) $whiteUrlList = array_merge($whiteUrlList, $db_info->embed_white_object);
			if($db_info->embed_white_iframe) $whiteIframeUrlList = array_merge($whiteIframeUrlList, $db_info->embed_white_iframe);
