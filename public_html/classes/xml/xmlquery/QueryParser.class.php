...
class QueryParser
{
	...
	function QueryParser($query = NULL, $isSubQuery = FALSE) {
		if($query) $this->queryTag = new QueryTag($query, $isSubQuery);
	}
	...
	function getTableInfo($query_id, $table_name) {
		...
		if(count($id_args) == 2) {
			...
			if(!isset($targetList[$target])) return;
			$module = $id_args[1];
			$id = $id_args[2];
		}
		...
		if(file_exists($table_file)) {
			...
			if($table_obj->table) {
				if(isset($table_obj->table->column) && !is_array($table_obj->table->column)) $table_obj->table->column = array($table_obj->table->column);
				foreach($table_obj->table->column as $k => $v) $column_type[$v->attrs->name] = $v->attrs->type;
			}
		}
		return $column_type;
	}
