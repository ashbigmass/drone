...
class QueryTag
{
	...
	function getBuff() {
		...
		$buff .= $this->preBuff;
		if($this->columns) $buff .= '$query->setColumns(' . $this->columns->toString() . ');' . PHP_EOL;
		$buff .= '$query->setTables(' . $this->tables->toString() . ');' . PHP_EOL;
		if($this->action == 'insert-select') $buff .= '$query->setSubquery(' . $this->subquery->toString() . ');' . PHP_EOL;
	...
	function getSubquery() {
		if($this->query->query) $this->subquery = new QueryTag($this->query->query, true);
	}
	...
	function getGroups() {
		if($this->query->groups) return $this->groups = new GroupsTag($this->query->groups->group);
		else return $this->groups = new GroupsTag(NULL);
	}
	...
	function getArguments() {
		$arguments = array();
		if($this->columns) $arguments = array_merge($arguments, $this->columns->getArguments());
		if($this->action == 'insert-select') $arguments = array_merge($arguments, $this->subquery->getArguments());
