...
class TablesTag
{
	...
	function toString() {
		...
		foreach($this->tables as $table) {
			if(is_a($table, 'QueryTag')) $output_tables .= $table->toString() . PHP_EOL . ',';
			else $output_tables .= $table->getTableString() . PHP_EOL . ',';
		}
	...
	function getArguments() {
		$arguments = array();
		foreach($this->tables as $table) $arguments = array_merge($arguments, $table->getArguments());
		return $arguments;
	}
}
