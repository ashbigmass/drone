...
class ConditionsTag
{
	...
	function ConditionsTag($xml_conditions) {
		...
		if(!$xml_conditions) return;
		$xml_condition_list = array();
		if($xml_conditions->condition) 		$xml_condition_list = $xml_conditions->condition;
		...
		if($xml_condition_list) $this->condition_groups[] = new ConditionGroupTag($xml_condition_list);
		$xml_groups = $xml_conditions->group;
		if($xml_groups) {
			if(!is_array($xml_groups)) $xml_groups = array($xml_groups);
			foreach($xml_groups as $group) $this->condition_groups[] = new ConditionGroupTag($group->condition, $group->attrs->pipe);
		}
	}

	function toString() {
		$output_conditions = 'array(' . PHP_EOL;
		foreach($this->condition_groups as $condition) $output_conditions .= $condition->getConditionGroupString() . PHP_EOL . ',';
	...
	function getArguments() {
		$arguments = array();
		foreach($this->condition_groups as $condition) $arguments = array_merge($arguments, $condition->getArguments());
		return $arguments;
	}
}
