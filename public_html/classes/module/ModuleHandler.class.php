...
class ModuleHandler extends Handler
{
	...
	function ModuleHandler($module = '', $act = '', $mid = '', $document_srl = '', $module_srl = '') {
		...
		if($entry = Context::get('entry')) $this->entry = Context::convertEncodingStr($entry);
		$isInvalid = NULL;
	...
	public static function xeErrorLog($errnumber, $errormassage, $errorfile, $errorline, $errorcontext) {
		if(($errnumber & 3) == 0 || error_reporting() == 0) return false;
		...
		if(count($backtrace) > 1 && $backtrace[1]['function'] === 'xeErrorLog' && !$backtrace[1]['class']) array_shift($backtrace);
	...
	function shutdownHandler() {
		...
		if ($errinfo === null || ($errinfo['type'] != 1 && $errinfo['type'] != 4)) return false;
		set_error_handler(function() { }, ~0);
		$debug_file = _XE_PATH_ . 'files/_debug_message.php';
		if(!file_exists($debug_file)) $print[] = '<?php exit() ?>';
	...
	function init() {
		...
		foreach($urls as $url) {
			...
			if($host && ($host != $defaultHost && $host != $site_module_info->domain)) throw new Exception('msg_default_url_is_null');
		}
		...
		if(!$module_info && $this->mid) $module_info = $oModuleModel->getModuleInfoByMid($this->mid, $site_module_info->site_srl);
	...
	function procModule() {
		...
		if($this->error) {
			...
			if($this->httpStatusCode) $oMessageObject->setHttpStatusCode($this->httpStatusCode);
		...
		if($type == 'controller') {
			...
			if(!$allowedMethod) $allowedMethodList[0] = 'POST';
			else $allowedMethodList = explode('|', strtoupper($allowedMethod));
		...
		if(!empty($ruleset)) {
			...
			if(!empty($rulesetFile)) {
				if($_SESSION['XE_VALIDATOR_ERROR_LANG']) {
					...
					foreach($errorLang as $key => $val) Context::setLang($key, $val);
		...
		if($type == "view" && $kind != 'admin') {
			...
			if($module_config->htmlFooter) Context::addHtmlFooter($module_config->htmlFooter);
			if($module_config->siteTitle) {
				$siteTitle = Context::getBrowserTitle();
				if(!$siteTitle) Context::setBrowserTitle($module_config->siteTitle);
			}
		}
		$this->_setInputErrorToContext();
		...
		if(!$oModule->stop_proc && !isset($methodList[Context::getRequestMethod()])) {
			...
			if(Context::get('xeVirtualRequestMethod') != 'xml') $_SESSION['XE_VALIDATOR_RETURN_URL'] = $redirectUrl;
		}
		unset($logged_info);
		return $oModule;
	}
	...
	function _setInputErrorToContext() {
		...
		if($_SESSION['XE_VALIDATOR_ID'] && !Context::get('XE_VALIDATOR_ID')) Context::set('XE_VALIDATOR_ID', $_SESSION['XE_VALIDATOR_ID']);
		if(count($_SESSION['INPUT_ERROR'])) Context::set('INPUT_ERROR', $_SESSION['INPUT_ERROR']);
	...
	function displayContent($oModule = NULL) {
		...
		if(!isset($methodList[Context::getRequestMethod()])) {
			...
			if(Mobile::isFromMobilePhone()) $layout_srl = $oModule->module_info->mlayout_srl;
			else $layout_srl = $oModule->module_info->layout_srl;
			if($layout_srl == -1) {
				$viewType = (Mobile::isFromMobilePhone()) ? 'M' : 'P';
				$oLayoutAdminModel = getAdminModel('layout');
				...
				if($layout_info) {
					if($layout_info->extra_var_count) {
						foreach($layout_info->extra_var as $var_id => $val) {
							if($val->type == 'image') {
								if(strncmp('./files/attach/images/', $val->value, 22) === 0) $val->value = Context::getRequestUri() . substr($val->value, 2);
							}
					...
					if($layout_info->menu_count) {
						foreach($layout_info->menu as $menu_id => $menu) {
							if(!$menu->menu_srl || $menu->menu_srl == -1) {
								...
								if(FileHandler::exists($homeMenuCacheFile)) include($homeMenuCacheFile);
								if(!$menu->menu_srl) {
	...
	function &getModuleInstance($module, $type = 'view', $kind = '') {
		...
		if(!isset($GLOBALS['_loaded_module'][$module][$type][$kind])) {
			...
			if(!class_exists($module, true)) return NULL;
			if(!class_exists($instance_name, true)) return NULL;
			$oModule = new $instance_name();
			if(!is_object($oModule)) return NULL;
			Context::loadLang($class_path . 'lang');
			if($extend_module) Context::loadLang(ModuleHandler::getModulePath($parent_module) . 'lang');
	...
	function triggerCall($trigger_name, $called_position, &$obj) {
		...
		if(!$triggers || count($triggers) < 1) return new Object();
		$before_trigger_time = NULL;
		if(__LOG_SLOW_TRIGGER__> 0) $before_trigger_time = microtime(true);
