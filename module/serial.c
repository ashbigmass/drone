...
#include <string.h>
	...
	enum SerialBaud {
		SPABAUD_50 = B50, SPABAUD_110 = B110, SPABAUD_300 = B300, SPABAUD_600 = B600, SPABAUD_1200 = B1200,
		SPABAUD_2400 = B2400, SPABAUD_4800 = B4800, SPABAUD_9600 = B9600, SPABAUD_19200 = B19200,
	...
	enum SerialBaud {
		SPABAUD_50 = 50, SPABAUD_110 = 110, SPABAUD_300 = 300, SPABAUD_600 = 600, SPABAUD_1200 = 1200,
		SPABAUD_2400 = 2400, SPABAUD_4800 = 4800, SPABAUD_9600 = 9600, SPABAUD_19200 = 19200,
...
static enum SerialParity SerialGetParity(const char *buf, int *ressize) {
	...
	if(!strncasecmp(buf, "none", 4)) { r = 4; p = SPAPARITY_NONE; }
	else if(!strncasecmp(buf, "no", 2)) { r = 2; p = SPAPARITY_NONE; }
	else if(!strncasecmp(buf, "odd", 3)){ r = 3; p = SPAPARITY_ODD; }
	else if(!strncasecmp(buf, "even", 4)) { r = 4; p = SPAPARITY_EVEN; }
	else if(*buf == 'N' || *buf == 'n') { r = 1; p = SPAPARITY_NONE; }
	else if(*buf == 'O' || *buf == 'o') { r = 1; p = SPAPARITY_ODD; }
	else if(*buf == 'E' || *buf == 'e') { r = 1; p = SPAPARITY_EVEN; }
...
static enum SerialProtocol SerialGetProtocol(const char *buf, int *ressize) {
	...
	else if(*buf == 'r' || *buf == 'R' || *buf == 'c' || *buf == 'C') {r = 1; Protocol=SPAPROTOCOL_RTS_CTS;}
	...
	static const char * SerialInit(
		struct serial *sn, const char *Device,
		enum SerialBaud Baud, enum SerialStopbits StopBits,
		enum SerialProtocol Protocol, enum SerialParity Parity,
		enum SerialDatabits DataBits, int dowrite
	) {
		char mydevice[50];
		if(Device[0] != '\\') snprintf(mydevice, sizeof(mydevice), "\\\\.\\%s", Device);
		else mydevice[0] = 0;

		if((sn->Stream = CreateFile(mydevice[0] ? mydevice : Device, dowrite ?
			GENERIC_WRITE|GENERIC_READ : GENERIC_READ, 0, 0, OPEN_EXISTING, 0, 0)) == INVALID_HANDLE_VALUE)
		...
		char str[100];
		snprintf(
			str,sizeof(str), "baud=%d parity=%c data=%d stop=%d xon=%s octs=%s rts=%s", Baud, Parity, DataBits, StopBits,
			Protocol == SPAPROTOCOL_XON_XOFF ? "on" : "off", Protocol == SPAPROTOCOL_RTS_CTS ? "on" : "off",
	...
	static int SerialRead(struct serial *sn, char *buffer, size_t size) {
		DWORD j = 0;
		if(!ReadFile(sn->Stream, buffer, size, &j, 0)) return -1;
		return j;
	}

	static int SerialWrite(struct serial *sn, const char *buffer, size_t size) {
		DWORD j = 0;
		if(!WriteFile(sn->Stream, buffer, size, &j, 0)) return -1;
		return j;
	}
